...
**No depth bound, and why there is nothing to bound.** A general JSON compiler needs one, and the case list drafted for this stage carried one. This one does not: `RunPlan` and the inputs it is compiled from are closed at every level — `spec/run-plan.schema.json` sets `additionalProperties: false` on the document and on all fifteen `$defs`, and no field is recursive or carries free-form JSON. There is no input a caller can nest deeply, so a bound would refuse nothing and a case for it would observe nothing. Recorded rather than left as a case nobody can write.
