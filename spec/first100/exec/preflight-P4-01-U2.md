...
## Deviations and readiness notes, recorded at the end of U2's implementation

Four, each measured rather than anticipated.

**1. `verifying` is occupied by an end decision, and P7-05 owns what it will mean.** The delegate ruled (note 13) that the terminal path passes through `verifying` because the table admits no shortcut from `running` to `succeeded`, and that the step's meaning here is the end decision over this plugin's failure ledger — explicitly **not** output verification. The owner of the real meaning is **P7-05** (`AcceptanceGate 与 OutcomePackage：只有被证明的结果才能完成 Run`), whose must[0] adds `accepted`, `rejected`, `needs-human` and `compensating` beside this state, whose must[1] says execution-completed may only enter `verifying`, and whose acceptance[1] forbids reporting `completed: true` when verification failed. P4-01 is among its predecessors.

**Readiness note, `landsIn` P7-05: extend the `verifying` step.** The transition shape U2 freezes survives; the decision inside it is replaced by the AcceptanceGate. Not a defect and not numbered — a successor with an owner.

**2. The decision input is not in the log, and the right home is P7-05's.** The ruling's first form asked the `→ succeeded` entry to carry its input (`failures = 0`). `RunEvent` carries `fromState`, `toState`, `occurredAt` and `references`, where a reference is `{kind, id}` over a closed `RunEntityKind` — a count is not an entity, so carrying it needs a new event field, which is an event-vocabulary change and therefore outside this slice (the three adapt dispositions are read-only only while U2 changes no vocabulary). The delegate withdrew that half and took the alternative: **the transition entry is the record, and the input belongs in P7-05's `OutcomePackage.failures`**, which is content-addressed and signed. Recorded as a limitation in `packages/run/run/README.md`.

**3. A clean unload parks the Run and cannot hand back the work item — and the second half is BLOCKED-197.** The delegate rejected both of the shapes lane B put forward (leave it, or terminate it) and ruled a third: a clean unload is neither a crash nor a completion, so the Run goes `running → paused` — the one legal Run state nothing had ever reached, and the one that says "not crashed, not finished, resumable". Terminating would have claimed work that is merely unfinished, and it would also have falsified P4-01's own frozen `restores the Runs an earlier mount left behind, rather than starting empty`, whose premise is that a clean unload leaves something restorable; `paused` keeps that case true.

The ruling's other half — release the lease, so a restart needs no lapse — **is not implementable from that disposer**, measured: the provider's handle accessor throws, because its teardown clears the handle synchronously while this disposer is awaiting the transition before the release — and a fiber unload runs every disposer concurrently, so there is no order to rely on either (`vendor/cordis/src/fiber.ts`'s single `Promise.all`). The message that throw carried said the opposite (`used BEFORE its mount opened the database`) and is corrected in `@deepseek-ai/dsh-lease-sqlite`. The delegate withdrew that half. Two things the measurement turned up incidentally: the rejection was **swallowed** (the disposer's promise rejected and `fiber.dispose()` resolved anyway, so the parking case stayed green — without a case depending on the release, the exception would have been thrown and discarded at every clean shutdown), and a local `try`/`catch` was refused because the in-memory provider would not throw, making the degradation **provider-dependent**.

**4. Continuing a Run needs a stable session id**4. Continuing a Run needs a stable session id, so the shipped-profile observation needed a second fixture.** Adoption is keyed on the session. `dsh-agent-loop`'s `agents[].sessionId` is documented as an optional stable identity whose remounts resume the materialized history; without it every boot mints a session and each correctly gets its own Run. The existing fixture has no id — and a frozen P4-01 case asserts exactly that its two boots are two sessions — so `tests/first100/fixtures/loader/p4-01-run-resumed/` is a second composition differing by that one line. No frozen case was touched to make the observation.

## One frozen assertion changed, with the delegate's approval

`packages/run/run/tests/task-profile.spec.ts`'s `moves the Run accepted → planning carrying the profile as a task-profile reference` — a title in P4-02's live U freeze entry — read `expect(run?.state).toBe('planning')` after a full turn. U2 drives `planning → running` at that same step, so the assertion was falsified.

The title's subject is the ENTRY, not the state afterwards; reading the state was incidental and was true only because nothing advanced the Run. The replacement pins `fromState === 'accepted'` on the entry whose `toState` is `planning` and pins the reference **on that entry** rather than anywhere in the log — strictly stronger, and immune to the three transitions U2 adds. The title is unchanged, P4-02's U cell is `NOT_RUN`, so nothing green was retroactively affected. Approved in the delegate's note 14 before the edit was made, because "edit an assertion until it is green again" is formally indistinguishable from writing a case to fit a run already seen.
