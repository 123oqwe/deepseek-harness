...
**3. A Run does not terminate when the PROCESS ends, and that is a real gap rather than a test artefact.** Terminal transitions are driven from `agent/disposed`; at whole-process teardown Cordis unloads this plugin's listeners before the agent registry disposes its agents, so `finish` never runs — the Run keeps its state and the lease is not released. Measured two ways: the Loader fixture's warm boot reports `restored 1 non-terminal Run(s)` after a boot whose root agent never stepped, and the lease's survival is why the in-process restart case over a shared store had to use `leaseMs: 1`. A session disposed while the harness runs does terminate, which is what the package-suite cases observe. **Consequence: `listNonTerminal` still grows per boot for any session a later boot does not continue.** Recorded as a limitation; closing it means a teardown ordering guarantee this slice does not own.

**4. Continuing a Run needs a stable session id, so the shipped-profile observation needed a second fixture.** Adoption is keyed on the session. `dsh-agent-loop`'s `agents[].sessionId` is documented as an optional stable identity whose remounts resume the materialized history; without it every boot mints a session and each correctly gets its own Run. The existing fixture has no id — and a frozen P4-01 case asserts exactly that its two boots are two sessions — so `tests/first100/fixtures/loader/p4-01-run-resumed/` is a second composition differing by that one line. No frozen case was touched to make the observation.
