...
- **新冻结条目的 `expectCases` 一律取 `vitest list --json` 的全名**(`' > '` 换空格;补记 236):不要照抄同 epic 的老条目——BLOCKED-104 迁移前的条目持裸 `it` 标题,`frozen-titles-in-tree` 两种拼法都放行、`frozen-titles-resolvable` 不比拼法,只有观测记录按全名匹配,拼法错会一路走到观测才暴露(P4-02.U.2 一例)。
