...
- **肯定结果的探针同样要证工具答了**(补记 143/144):`cmd | tail; echo $?` 取的是 tail 的退出码(78 一例);vitest 4 对未知选项(如 `--minWorkers`)`CACError` 即死、0 测试,尾巴 `echo` 仍报 0——**没有 `Test Files` 这类工具自己的 summary 行就是没跑**(补记 240);`printf "%s" "$(basename x)" "$?"` 里的 `$?` 取到的是前一个 `$(…)` 子命令的状态而非目标命令(lane A 一例,三门全报 0 实际两红);`vitest list --json <path>` exit 0 写 0 字节(`--json` 吃掉路径,须 `vitest list <path> --json`);非空但 JSON 不可解析的 chunk 在"非空即成功"的分支溜走——收集产物须可解析且非空才算过,任一缺失即拒绝报数。
...
- **改 docs/README 或删被引用文件的提交,报 tip 前跑 `verify-md-links` 比对断链集合改前后**(补记 245):它是 doc-quick 的叶子、不在八项/九项里,所以"删源文件 + 改 README"能全绿溜过(`02ceda9fc8` 留了 12 条断链、八项全绿)。不是硬门(全仓 46 条既有债会挡每次 tip),口径同 CI 只查变更文件:确认**你没新增**断链即可。
- **两执行 session 绝不静默 idle**(用户令 2026-09-12,补记 246):它们并行是为更快 + 保质,所以除非在等 delegate 回复或等一次已报告的集体验证,否则不得停。已装进各 session `Stop` hook(`executor-stop-guard.sh`,`stop_hook_active` 守护、每次拦一次):将 idle 时注入"立即 SendMessage 一行状态 + 推荐下一步,由 delegate 裁,再继续"。delegate 侧不变量:收 lane 完成/idle 信号即主动裁+派(继续/改/等),不被动 hold;修 hook 只在用户明令下、写 per-worktree `settings.local.json`、有备份、不碰产品仓。
- **已被观测过的冻结条目,其 `expectCases` 只有在属性真的变了时才动**(补记 246):`verify-freeze-in-candidate-tree` 按 `(epic,stage,seq,排序 expectCases)` 匹配观测树,改标题=改观测身份、会红;"更精确/无害"不是理由(78 令 P4-02.U.2 裸→全名,破门,已撤)。
- **切分支后新包遗留的 ignored `lib/` 会让 `constraints` 报假红**(补记 246):切到带新包的分支再切回,该包留下只有构建产物、无 `package.json` 的目录 → `constraints` 报"expected a package here";`rm -rf` 该 ignored 目录后重跑。`verify-import-integrity`(7 条未声明运行时 import)、`verify-translation-pairing`(48 条)是上游既有红,非 CI 阻塞门,归属按 diff 交集实测。
- **绝不 block-wait 在 OOM-prone 的全仓任务上**(补记 243):`verify-frozen-titles-resolvable`、全套 snapshot、`build:lib:host` 在这台内存紧的机器上会 40 分钟起或被杀;把它们挂成阻塞后台任务再 `TaskOutput` block-wait,turn 会在等待中结束、session 停成 idle(lane B 一晚多次)。要么内联跑并接受可能被杀,要么记"云上 step 11/门 补",不要 block-wait。tip 报告一写好立即 SendMessage,不留在自己终端。delegate 见 lane 连续两次心跳 idle 且 dirty 树有未提交冻结/tip 材料:先查转录尾是否 `TaskOutput`/`TaskStop` + 进程表单文件 vitest(= 卡在全仓门),对症解,不是再派活。
...
- **三条流程陷阱**(补记 219):① 用 Python 改仓库 JSON 时 `json.dumps(..., ensure_ascii=False)`——默认会把字面非 ASCII 全改成 `\uXXXX`(772 行一例);② 包级 `tsconfig.json` 多只 include `src`,`tsc -p … --noEmit` 不看 `tests/`,vitest 也不做类型检查——改了 spec、或**改动了导出的 `Config`/公共接口**(会打断别处手写调用点),报 tip 前必跑宿主编译(`tsc -b tsconfig.host.json`),补记 240 一例(`waitForJobsMs` 必填打断八处);③ `verify-files-overlay.mjs --write` 才有 CLI,`files-overlay.mjs` 是纯库、直接跑静默退出 0。
