...
**What does withdraw delegated authority, corrected on 2026-09-25 (BLOCKED-331 condition 5):** explicit revocation, and it is final: a revoked token or lineage is never re-issued, and the provider's durable record keeps a restarted mount from issuing the session another token. Expiry bounds the life of one token, not the authority. A session token that has expired is re-issued under the same policy at the session's next dispatch. A delegated token is derived from the parent's current token, expires when that token does, and is re-derived from the parent's current token when it expires; attenuating from an expired parent is refused as `parent-expired`. No separate child TTL exists. The production owner of explicit revocation is **P2-12's emergency stop** — `cancel run` / `kill execution world` revoking the run's session roots, which cascade to descendants through the recorded lineage. That is where acceptance[1] gets a launched-profile subject; it is that clause's home rather than a deferral.
