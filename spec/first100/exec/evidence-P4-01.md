...
**LANDED 2026-09-11.** U.1 and U.2 were observed passing in CI run 34652643903 at candidate `93d52207338066bd65494f6044f1ec5920d63cb8`, and all ten citations below are now in `acceptance-coverage.json`; `checkCoverageClosure('P4-01')` returns valid with no unverified citations. Both stale U citations were struck in the same edit — the delegate call this page asked for was given. One thing the table did not say and the edit needed: a citation for a SUPPLEMENT case must carry `supplementSeq`, or closure resolves it against the BASE entry and reports it unverified even though the supplement observed it. Written without that field first, all ten came back unverified and the index went red — the field is what makes a supplement citation resolvable.
