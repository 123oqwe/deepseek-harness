...
| 5 | it can be re-attached by run id to observe, await, or cancel | `WorkflowEngine.attach(runId)`, declared on the SEAM so a caller reaches it without the engine's concrete type; live run → the same handle, settled detached run → its outcome collected once; frozen as four cases plus the `resume`-refused control (supplement U.6) | closes |
...
**Semantic 5 was measured OPEN and is now built** (§12.46-B does not apply). The split is for a producer owned by a scheduled later epic — `hedge` to P5-04, `trace` to P7-07, `inputs` to P6-09 — and re-attachment has no other owner: it is must[2]'s own half. Before the build, the engine's surface was `start`, `startDetached`, `resume`, `registerDefinition`, `startNested`, and grepping `packages/workflow/*/src` for a re-attach path returned 0.

It matters more than the other five semantics: a detached run's whole point is outliving the turn that launched it, so the handle that turn received is precisely what a later observer does not have. Unbuilt, the mechanism was reachable in the case where it does not matter and unreachable in the case where it does.

Two routes, because a live run and a settled one are different questions, and `resume` is neither: resuming a live run starts a SECOND worker under one run id — two masters, which P4-07's lease refuses. A case asserts that refusal rather than assuming it. A settlement is kept only for a DETACHED run and delivered once; an ordinary run's caller held its handle all along, and keeping those would grow the map with every run the engine ever executed.
