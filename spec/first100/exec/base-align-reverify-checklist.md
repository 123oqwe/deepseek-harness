...
**How to read a RE-OBSERVED marker.** A row's action column may end with one or more markers of the form `**RE-OBSERVED** <cell> <tip> exit <n> cases <k/m> <UTC> R<nn>`, written by lane A's `checklist-writeback.mjs` (kept outside this tree) after that cell's frozen command was re-run **and its output read**. `<cell>` is named because a row can carry several cells and they are re-run separately. `<tip>` is the tree the command actually ran against. `<k/m>` is matched-over-frozen cases, and `<n>` is the real exit; a clean exit with `k < m` is refused by the tool rather than recorded, because it reads as success in a table. **`R<nn>` is a command number from lane A's `argv-rerun-table.md` (kept outside this tree), not a CI run id** — these are local runs and have none; cells sharing a byte-identical `argv` share one command number and therefore one run, and each still gets its own marker. A marker records a claim; it does not verify one.
...
|---|---|---|---|---|---|
| MEDIUM | `apps/cli/src/profile-boot.ts` | +91/-6 | +413/-6 | 4 — P0-05.U, P1-01.U, P1-03.U.1, P1-08.U | **ADJUDICATED 2026-09-13 — host intact, re-observe.** 307 (base) → 392 (upstream) lines; upstream deleted **no** top-level declaration and added one (`initializeProfileFromDefault`), so all 413 of our lines attach to surviving code. Textual merge + import fixes for the relocated manifest types, then re-run each cell's argv and re-verify **4.4a**/**4.4d**. `http-proxy` is newly wired here upstream. **RE-OBSERVED** P0-05.U 191aa1e502ff7e96a7ce306cdf57ccb742c745fb exit 0 cases 20/20 2026-09-15T16:19:49Z R01 **RE-OBSERVED** P1-01.U 191aa1e502ff7e96a7ce306cdf57ccb742c745fb exit 0 cases 48/48 2026-09-15T16:19:49Z R05 **RE-OBSERVED** P1-03.U.1 191aa1e502ff7e96a7ce306cdf57ccb742c745fb exit 0 cases 9/9 2026-09-15T16:19:49Z R06 **RE-OBSERVED** P1-08.U 191aa1e502ff7e96a7ce306cdf57ccb742c745fb exit 0 cases 12/12 2026-09-15T16:19:49Z R08 |
...
**That file carries three identities at once, and a single red has three readings with three different repairs** — the authority is `base-align-v3-preflight.md`'s **§6/S9-S10 supplement**, which the delegate also names aloud when dispatching this step. Summarised here so the order travels with the step: **(1)** a flake → re-run and take the second reading; **(2)** the re-anchor broke the harness path → fix the merge or the wiring; **(3)** P9-06.P's subject relocated, i.e. STALE-by-design → re-record per S8b/S10 and **do not touch the tree**. Read them in that order: re-run first to rule out (1), and only then read the diff to tell a broken path from a legitimately relocated subject.
