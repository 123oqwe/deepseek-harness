...
**That file carries three identities at once, and a single red has three readings with three different repairs** — the order to read them in is below. (The delegate also names it when dispatching S9/S10, and ruled it into `base-align-v3-preflight.md`'s S9/S10 section — **as of `e0a28769be` that section does not yet carry it**, so treat this list as the operative one until it does.) **(1)** a flake → re-run and take the second reading; **(2)** the re-anchor broke the harness path → fix the merge or the wiring; **(3)** P9-06.P's subject relocated, i.e. STALE-by-design → re-record per S8b/S10 and **do not touch the tree**. Read them in that order: re-run first to rule out (1), and only then read the diff to tell a broken path from a legitimately relocated subject.
