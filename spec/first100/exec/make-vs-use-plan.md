...
**派生自** `spec/first100/exec/make-vs-use-ledger.json`(sha256 前 16 位 `45434d476f4f54a1`)+ `ledger.json` 状态 + `p9-verification.json`;**生成时间** 2026-09-06T13:59-04:00;生成器源码在文末 `<details>`。**不要手改本文件**——改账本 JSON 或整改令,重新生成。
...
**社区参考(不采用)**:dsh-eval-harness 14★ 覆盖≈30% — YAML cases → headless overlay-patch fork → session.jsonl asserts → baseline gate; unlicensed, single author, LLM-judge;
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-harbor 24★ 覆盖≈30% — cordis-plugin·进程内·无 key \| 缺口：No manifestVersion=2 or structured schema (services/tools/skills/MCP/events/fs/network/pro
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-provenance 1★ 覆盖≈15% — SRI integrity vs registry, pin check, parses SLSA attestation but its own source says it does NOT verify the sigstore si
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-plugin-mall 5★ 覆盖≈20% — Snapshots profile load-bearing files pre-install, restores on failure, pending-install marker (~20% of transaction seman
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-plugin-marketplace 15★ 覆盖≈35% — bundle·进程内·无 key \| 缺口：No read-only quarantine directory: pnpm mutates node_modules in place. No build sandbox: consente
...
**社区参考(不采用)**:dsh-plugin-vet 3★ 覆盖≈35% — TS-compiler AST, 15 rule modules, capability inference, OSV lookup, opt-in runtime guard + honeypot; alarm-only — best f
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:hol-guard 514★ 覆盖≈20% — external-product-with-dsh-integration·非插件·无 key \| 缺口：External wrapper/hook model, not an in-harness workspace state mac
**裁决叠加(整改令)**:
- §7.1 已验收核验:OK
...
**社区参考(不采用)**:dsh-web-plugin-manager 67★ 覆盖≈20% — bundle·进程内·无 key \| 缺口：No manifest-declared runtime API range / schema ranges / required-optional capabilities, no graph
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-web-plugin-manager 67★ 覆盖≈25% — bundle·进程内·无 key \| 缺口：Post-hoc static detection only (README admits dynamic names are missed); no PluginIdentity/namesp
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-backup 15★ 覆盖≈25% — Pre-upgrade snapshots on host version change, sha256 verify, auto-rollback, corrupt-log quarantine — snapshot/rollback h
...
**社区参考(不采用)**:dsh-harmony 17★ 覆盖≈15% — cordis-plugin·进程内·无 key \| 缺口：Zero governance: patches auto-apply at boot, no proposal artifact, scan, permission diff,
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-plugin-marketplace 20★ 覆盖≈35% — cordis-plugin·进程内·无 key \| 缺口：No `dsh plugin verify` CLI and no per-plugin report bundle (manifest/signature/SBOM/compat
**裁决叠加(整改令)**:
- §3.4/R1 消费者:in-toto/DSSE 信封 import `packages/attestation/envelope`,不再声明一份
...
**社区参考(不采用)**:dsh-awiki 14★ 覆盖≈15% — ANP/DID identities for agent-to-agent messaging — network identity, not harness principal (<15%)
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-auto-mode 146★ 覆盖≈30% — cordis-plugin·进程内·无 key \| 缺口：No formal class enum (read/local-reversible/internal-write/external-communication/destruct
...
**社区参考(不采用)**:dsh-permission-rules 102★ 覆盖≈30% — PEP-side ordered allow/deny/ask YAML on tools/pre-execute; first-match semantics (nearer user allow overrides baseline d
**裁决叠加(整改令)**:
- §2.G 开工前设计决定已定(见该 epic 行)
...
**社区参考(不采用)**:dsh-auto-mode 146★ 覆盖≈25% — cordis-plugin·进程内·无 key \| 缺口：No ActionManifest digest or canonicalized-argument hash; binding is by string equality of
...
**社区参考(不采用)**:hol-guard 514★ 覆盖≈30% — external-product-with-dsh-integration·非插件·无 key \| 缺口：Not dsh's workflow `waiting_for_approval` + scheduler wake; reques
**裁决叠加(整改令)**:
- §2.B 子句不动、加实现约束(见该 epic 小节)
...
**社区参考(不采用)**:dsh-approve-for-me 14★ 覆盖≈20% — cordis-plugin·进程内·无 key \| 缺口：No expiry, revocation, usage counters, amount/count/time-window, environment or resource p
**裁决叠加(整改令)**:
- §3.1 共用引擎 Cedar(@cedar-policy/cedar-wasm 4.12.0)在 P2-05 开工前作为 infra slice 接入;本 epic 只写 adapter/PEP/explain
...
**社区参考(不采用)**:dsh-permission-rules 102★ 覆盖≈45% — YAML rules with JSON Schema, enforce:false dry-run, hot reload keeping previous set on bad edit, /rules test explain, ma
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-auto-mode 146★ 覆盖≈20% — cordis-plugin·进程内·无 key \| 缺口：Profile is only sandbox+approval; no execution-world/fs/network/process/secrets/risk-thres
...
**社区参考(不采用)**:dsh-im-gateway 45★ 覆盖≈25% — cordis-plugin·进程内·无 key \| 缺口：No pause-new-actions / cancel-run / kill-execution-world / resume commands; no kernel-broa
...
**社区参考(不采用)**:axern 59★ 覆盖≈25% — not-a-plugin·非插件·无 key \| 缺口：Does not define dsh's WorldSpec/WorldHandle/WorldAttestation/WorldSnapshot; no snapshot/res
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:axern 59★ 覆盖≈35% — not-a-plugin·非插件·无 key \| 缺口：No FileSystemPolicy/ProcessPolicy/IpcPolicy/DevicePolicy/SecretPolicy vocabulary; filesyste
...
**社区参考(不采用)**:axern 59★ 覆盖≈30% — not-a-plugin·非插件·无 key \| 缺口：No per-execution policy_denied outcome: egress denials are REFUSED/dropped on the wire and
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-browser 13★ 覆盖≈15% — cordis-plugin·进程内·无 key \| 缺口：Not a proxy: Playwright owns the socket, so DNS rebinding / TOCTOU is explicitly unmitigat
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:axern 59★ 覆盖≈35% — not-a-plugin·非插件·无 key \| 缺口：Linux node runtime only: no Seatbelt, no Windows restricted token/job object; no explicit p
**裁决叠加(整改令)**:
- §2.D 热区改接法:[B] 热区文件换 [N] 新 rung / contribution
- §3.2 共用引擎 sandbox-runtime(@anthropic-ai/sandbox-runtime,exact pin)在 W7 前作为 sandbox-srt rung 接入;该 slice 同时把 sandbox-local 的 PLATFORM_CHAINS 表改成 contribution point
...
**社区参考(不采用)**:dsh-config-manager 63★ 覆盖≈20% — AES-256-GCM export of config — at-rest encryption, not brokered leases (<20%)
**裁决叠加(整改令)**:
- §3.2 共用引擎 sandbox-runtime(@anthropic-ai/sandbox-runtime,exact pin)在 W7 前作为 sandbox-srt rung 接入;该 slice 同时把 sandbox-local 的 PLATFORM_CHAINS 表改成 contribution point
...
**社区参考(不采用)**:hol-guard 514★ 覆盖≈30% — external-product-with-dsh-integration·非插件·无 key \| 缺口：Attestation is not bound to OS/kernel/provider version tuple in wh
**裁决叠加(整改令)**:
- §2.D 热区改接法:[B] 热区文件换 [N] 新 rung / contribution
- §3.2 共用引擎 sandbox-runtime(@anthropic-ai/sandbox-runtime,exact pin)在 W7 前作为 sandbox-srt rung 接入;该 slice 同时把 sandbox-local 的 PLATFORM_CHAINS 表改成 contribution point
- §3.4/R1 消费者:in-toto/DSSE 信封 import `packages/attestation/envelope`,不再声明一份
...
**社区参考(不采用)**:dsh-worlds 3★ 覆盖≈50% — 本会话复核：frozo-ai/dsh-worlds 仓库里没有 LICENSE 文件（gh api license=null），无授权即不可依赖，直接出局；且原审计已记它安全 MUST 覆盖 0%（强制 danger-full-access
...
**社区参考(不采用)**:axern 59★ 覆盖≈45% — not-a-plugin·非插件·无 key \| 缺口：No microVM provider yet (Firecracker named only as future direction); attestation is node-c
**裁决叠加(整改令)**:
- §2.B 子句不动、加实现约束(见该 epic 小节)
- §3.4/R1 消费者:in-toto/DSSE 信封 import `packages/attestation/envelope`,不再声明一份
...
**社区参考(不采用)**:dsh-plugin-container 4★ 覆盖≈20% — cordis-plugin·进程内·无 key·一次性提交 \| 缺口：All limits are opt-in per call (no default budget), only per-container not per actio
...
**社区参考(不采用)**:dsh-filesnap 32★ 覆盖≈35% — cordis-plugin·进程内·无 key \| 缺口：Not ExecutionWorld-aware at all: no WorldSpec/WorldHandle/provider metadata, no container/
**裁决叠加(整改令)**:
- §2.B 子句不动、加实现约束(见该 epic 小节)
...
**社区参考(不采用)**:dsh-docs 13★ 覆盖≈30% — cordis-plugin·进程内·无 key \| 缺口：MUST-2 mostly unmet: MIME is derived from file extension only (mediaTypeForPath), no conte
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-trajectory-governor 13★ 覆盖≈30% — cordis-plugin·进程内·无 key \| 缺口：No per-inference source/confidence (only a scalar complexity score); no question emission
...
**社区参考(不采用)**:dsh_workflow 113★ 覆盖≈20% — cordis-plugin·进程内·无 key·一次性提交 \| 缺口：The plan is not data: the executable part is arbitrary JavaScript run in QuickJS, di
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:allinluna 51★ 覆盖≈20% — cordis-plugin·进程内·无 key \| 缺口：Hash is not a signature (any party can recompute); no kernel signing, no PlanAmendment pro
**裁决叠加(整改令)**:
- §3.4/R1 消费者:in-toto/DSSE 信封 import `packages/attestation/envelope`,不再声明一份
- §7.3 JCS 消费者:argumentsHash/plan id/contract hash 复用 P2-03 的 canonicalizer(差分 oracle 已证),不再写一份
...
**社区参考(不采用)**:allinluna 51★ 覆盖≈35% — cordis-plugin·进程内·无 key \| 缺口：No lease epoch on transitions, no stale-worker write rejection (no fencing), lifecycle is
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:allinluna 51★ 覆盖≈35% — cordis-plugin·进程内·无 key \| 缺口：No dead-letter, no per-message priority/deadline, no consumer-side epoch dedup, no tenant
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:allinluna 51★ 覆盖≈30% — cordis-plugin·进程内·无 key \| 缺口：No heartbeat/renew API, no epoch/fencing token carried on state writes or actions (grep he
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh_workflow 113★ 覆盖≈35% — resume-run via call-seq cache + events.jsonl proves the pattern; own file store above ctx.subagents, compiled lib/, sing
**裁决叠加(整改令)**:
- §7.1 已验收核验:OK
...
**社区参考(不采用)**:allinluna 51★ 覆盖≈35% — cordis-plugin·进程内·无 key \| 缺口：Budgets are concurrency slots only: no tokens/cost/time/tool-call/world budgets, no parent
**裁决叠加(整改令)**:
- §6:REUSE_UPSTREAM 已按 gap-over-upstream 缩范围,开工时读 spec/first100/sources/base-align-v2/23-partial-rescope-spec.md 对应条目
...
**社区参考(不采用)**:allinluna 51★ 覆盖≈25% — cordis-plugin·进程内·无 key \| 缺口：No prepared/sent/confirmed/ambiguous/compensated ledger states, no native-provider-key pas
...
**社区参考(不采用)**:allinluna 51★ 覆盖≈15% — cordis-plugin·进程内·无 key \| 缺口：No observeState/compareExpected/compensate declarations for tools, no StateDiff/repair opt
...
**社区参考(不采用)**:dsh-harness-one 17★ 覆盖≈35% — bundle·进程内·无 key \| 缺口：Zero coverage of partial-turn checkpoint boundaries (model request / tool call / tool result / as
**裁决叠加(整改令)**:
- §2.C 缩范围:files[] 删与上游/已有实现重复的 [N] 文件,residual 收窄
...
**社区参考(不采用)**:ruflo 70,209★ 覆盖≈30% — not-a-plugin·非插件·无 key \| 缺口：Strategy vocabulary is execution-shape over subtasks, not answer-only/single-react/plan-exe
...
**社区参考(不采用)**:dsh-plugin-subagent-director 14★ 覆盖≈25% — Static per-role provider/model + 4-level fallback; proves the agent/request seam works for subagents
...
**社区参考(不采用)**:dsh-vision-router 1,051★ 覆盖≈10% — 'Unsupported image → explicit vision degrade' is the one negotiation case the community built; core already has projectI
**裁决叠加(整改令)**:
- §2.C 缩范围:files[] 删与上游/已有实现重复的 [N] 文件,residual 收窄
...
**社区参考(不采用)**:dsh-llm-fallbacks 20★ 覆盖≈38% — Provider/model fallback chains on agent/request-error after llm-retry, cooldown, half-open recovery, per-role chains — ~
**裁决叠加(整改令)**:
- §2.G 开工前设计决定已定(见该 epic 行)
...
**社区参考(不采用)**:dsh_workflow 113★ 覆盖≈40% — cordis-plugin·进程内·无 key·一次性提交 \| 缺口：No capability token, tenant/principal, WorldSpec beyond shared-cwd/worktree, artifac
**裁决叠加(整改令)**:
- §2.D 热区改接法:[B] 热区文件换 [N] 新 rung / contribution
...
**社区参考(不采用)**:allinluna 51★ 覆盖≈40% — cordis-plugin·进程内·无 key \| 缺口：No usage/cost, tool-trace refs, action-receipt list, or continuation token. JSON-Schema +
**裁决叠加(整改令)**:
- §2.D 热区改接法:[B] 热区文件换 [N] 新 rung / contribution
...
**社区参考(不采用)**:dsh-cli-bridge 2★ 覆盖≈40% — cordis-plugin·进程内·无 key \| 缺口：Approval requests are NOT routed to the parent policy: it forces `-c approval_policy="neve
**裁决叠加(整改令)**:
- §2.C 缩范围:files[] 删与上游/已有实现重复的 [N] 文件,residual 收窄
...
**社区参考(不采用)**:dsh-cli-bridge 2★ 覆盖≈45% — cordis-plugin·进程内·无 key \| 缺口：No unified SubagentResult conformance (its own RunEnd/DelegationSnapshot shape); no parent
**裁决叠加(整改令)**:
- §2.C 缩范围:files[] 删与上游/已有实现重复的 [N] 文件,residual 收窄
...
**社区参考(不采用)**:dsh-plugin-product-subagents 19★ 覆盖≈20% — cordis-plugin·进程内·无 key \| 缺口：No identity/tenant negotiation, no resume token + event cursor (loadSession failure silent
**裁决叠加(整改令)**:
- §6:REUSE_UPSTREAM 已按 gap-over-upstream 缩范围,开工时读 spec/first100/sources/base-align-v2/23-partial-rescope-spec.md 对应条目
...
**社区参考(不采用)**:dsh-Visual-Workflow 14★ 覆盖≈25% — cordis-plugin·进程内·无 key \| 缺口：Control messages are in-memory only (no durability, epoch, or idempotency key); interrupt
**裁决叠加(整改令)**:
- §2.F planError 已解决,记录即可
...
**社区参考(不采用)**:dsh-agent-teams 1,285★ 覆盖≈48% — Atomic claim + attempt_id + revoke/quiesce + cold-restart recovery + durable jsonl mailbox + captain/roles + quality gat
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-cli-bridge 2★ 覆盖≈35% — cordis-plugin·进程内·无 key \| 缺口：Verifier is optional LLM review, not a required verification gate; no coordination guard (
...
**社区参考(不采用)**:dsh-memento 80★ 覆盖≈35% — Zero-dep node:sqlite provider defining ctx.memory seam, approval-gated writes, adapter registry, conformance suite + JSO
**裁决叠加(整改令)**:
- §2.G 开工前设计决定已定(见该 epic 行)
...
**社区参考(不采用)**:dsh-memento 80★ 覆盖≈25% — MemoryEntry (track/scope/workspaceKey/agentKey/text/source/tags/version/recallCount); no evidence links or supersedes/di
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-memento 80★ 覆盖≈40% — Approval gate, budgets, consolidate, adapter export; no evidence requirement, tombstones, erase propagation, TTL
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-capability-menu 84★ 覆盖≈15% — bundle·进程内·无 key \| 缺口：Zones are process-global, not declared per agent in a RunPlan (MetaSearchOptions.scope is explici
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:dsh-compaction-instant 14★ 覆盖≈40% — cordis-plugin·进程内·无 key \| 缺口：No structured CompactionResult fields: compaction/summary still carries only shadowedRange
**裁决叠加(整改令)**:
- §6:REUSE_UPSTREAM 已按 gap-over-upstream 缩范围,开工时读 spec/first100/sources/base-align-v2/23-partial-rescope-spec.md 对应条目
...
**社区参考(不采用)**:Cambium 294★ 覆盖≈20% — external-product-with-dsh-integration·非插件·无 key \| 缺口：No encryption at rest, no per-tenant envelope keys/KMS, no residen
**裁决叠加(整改令)**:
- §2.G 开工前设计决定已定(见该 epic 行)
- §3.4/R1 消费者:in-toto/DSSE 信封 import `packages/attestation/envelope`,不再声明一份
...
**社区参考(不采用)**:deepseek-harness-reliability-governor 2★ 覆盖≈40% — cordis-plugin·进程内·无 key \| 缺口：No RunPlan or PlanAmendment binding (P4-03/04 do not exist upstream); contract is opt-in p
**裁决叠加(整改令)**:
- §3.4/R1 消费者:in-toto/DSSE 信封 import `packages/attestation/envelope`,不再声明一份
- §7.3 JCS 消费者:argumentsHash/plan id/contract hash 复用 P2-03 的 canonicalizer(差分 oracle 已证),不再写一份
...
**社区参考(不采用)**:dsh-research-report 42★ 覆盖≈25% — Content-addressed ledger objects/<sha256> + JSONL journals, sealed manifest hash, claim↔evidence binding, SARIF verifier
**裁决叠加(整改令)**:
- §3.4/R1 消费者:in-toto/DSSE 信封 import `packages/attestation/envelope`,不再声明一份
...
**社区参考(不采用)**:deepseek-harness-reliability-governor 2★ 覆盖≈35% — cordis-plugin·进程内·无 key \| 缺口：No VerifierProvider seam — check evaluation is hard-coded in src/governor.ts; no model-bac
...
**社区参考(不采用)**:grove 22★ 覆盖≈25% — external-product-with-dsh-integration·非插件·无 key \| 缺口：No ContradictionEdge/conflicted state or automatic conflict propag
**裁决叠加(整改令)**:
- §3.4/R1 消费者:in-toto/DSSE 信封 import `packages/attestation/envelope`,不再声明一份
- §7.2 R3:W3C PROV-DM 词汇(wasDerivedFrom/wasGeneratedBy/wasAttributedTo)由本 epic 首次对外使用;P5-11/P6-02 内部名保留 + 映射
...
**社区参考(不采用)**:deepseek-harness-reliability-governor 2★ 覆盖≈35% — cordis-plugin·进程内·无 key \| 缺口：No Run-level state machine (verifying/accepted/rejected/needs-human/compensating) — the tu
**裁决叠加(整改令)**:
- §3.4/R1 消费者:in-toto/DSSE 信封 import `packages/attestation/envelope`,不再声明一份
- §7.3 JCS 消费者:argumentsHash/plan id/contract hash 复用 P2-03 的 canonicalizer(差分 oracle 已证),不再写一份
...
**社区参考(不采用)**:deepseek-harness-reliability-governor 2★ 覆盖≈40% — cordis-plugin·进程内·无 key \| 缺口：Only attempt count is capped — no token, wall-time or external-write budgets; no RepairPla
...
**社区参考(不采用)**:dsh-observe 4★ 覆盖≈45% — OTLP traces+metrics (JSON) + Langfuse ingestion, sanitize, bounded durable offline spool over storage-domain, determinis
**裁决叠加(整改令)**:
- §2.C 缩范围:files[] 删与上游/已有实现重复的 [N] 文件,residual 收窄
- §3.3 OTel:在既有 packages/session/session-telemetry-otel 上加 TracerProvider pipeline;不建 otel-exporter 包;gen_ai.* 用 @opentelemetry/semantic-conventions
...
**社区参考(不采用)**:dsh-repro 2★ 覆盖≈15% — /repro exports minimal secret-scrubbed replayable bundle — overlap with ReplayBundle packaging
**裁决叠加(整改令)**:
- §6:REUSE_UPSTREAM 已按 gap-over-upstream 缩范围,开工时读 spec/first100/sources/base-align-v2/23-partial-rescope-spec.md 对应条目
...
**社区参考(不采用)**:dsh-eval-harness 14★ 覆盖≈30% — YAML cases → headless fork → session.jsonl asserts (turn_end, tools_called, output_contains, max_steps/max_tokens) → bas
...
**社区参考(不采用)**:oh-my-knowledge 18★ 覆盖≈40% — cordis-plugin·进程内·需 key/服务 \| 缺口：No shadow/canary against live traffic, no replicated read-only stream for candidates, n
**裁决叠加(整改令)**:
- §3.4/R1 消费者:in-toto/DSSE 信封 import `packages/attestation/envelope`,不再声明一份
...
**社区参考(不采用)**:dsh-multica-runtime 60★ 覆盖≈20% — cordis-plugin·进程内·无 key \| 缺口：Exact-match version only, no range negotiation; no schema fingerprints; no mandatory/optio
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:sandbase-harness 641★ 覆盖≈10% — Exposes manage agents/sessions, stream turns, inspect artifacts, cancel over stdio MCP — competing surface on a differen
**裁决叠加(整改令)**:
- §2.F planError 已解决,记录即可
...
**社区参考(不采用)**:dsh-multica-runtime 60★ 覆盖≈15% — cordis-plugin·进程内·无 key \| 缺口：No pause/fork/retry/reconcile/close; no commandId/idempotencyKey/expectedRevision/reason c
**裁决叠加(整改令)**:
- §6:REUSE_UPSTREAM 已按 gap-over-upstream 缩范围,开工时读 spec/first100/sources/base-align-v2/23-partial-rescope-spec.md 对应条目
...
**社区参考(不采用)**:dsh-im-gateway 45★ 覆盖≈25% — cordis-plugin·进程内·无 key \| 缺口：No durable requestId, no action-hash binding (prompt shows only toolName + reason, never a
...
**社区参考(不采用)**:dsh-passwords 40★ 覆盖≈25% — Separate express+jsonwebtoken gateway, admin/user, per-account permissions + quotas, ACME HTTPS; patches dsh files; GPL
**裁决叠加(整改令)**:
- §2.G 开工前设计决定已定(见该 epic 行)
- §3.1 共用引擎 Cedar(@cedar-policy/cedar-wasm 4.12.0)在 P2-05 开工前作为 infra slice 接入;本 epic 只写 adapter/PEP/explain
...
**社区参考(不采用)**:iPolloWork 5,273★ 覆盖≈35% — external-product-with-dsh-integration·非插件·无 key \| 缺口：No Agent/Workflow DAG, budget/cost, Policy decisions, Evidence/Ver
**裁决叠加(整改令)**:
- §2.F planError 已解决,记录即可
...
**社区参考(不采用)**:tingly-box 345★ 覆盖≈15% — external-product-with-dsh-integration·非插件·无 key \| 缺口：No monotonic org->workspace->run policy hierarchy (child may only
**裁决叠加(整改令)**:
- §2.F planError 已解决,记录即可
- §3.1 共用引擎 Cedar(@cedar-policy/cedar-wasm 4.12.0)在 P2-05 开工前作为 infra slice 接入;本 epic 只写 adapter/PEP/explain
- §7.3 JCS 消费者:argumentsHash/plan id/contract hash 复用 P2-03 的 canonicalizer(差分 oracle 已证),不再写一份
...
**社区参考(不采用)**:dsh-undo-savepoint 142★ 覆盖≈15% — Auto-snapshots config/plugins/settings, undo/redo, secret redaction, safe mode, out-of-process recovery UI/CLI — config-
**裁决叠加(整改令)**:
- §3.4/R1 消费者:in-toto/DSSE 信封 import `packages/attestation/envelope`,不再声明一份
- §6:REUSE_UPSTREAM 已按 gap-over-upstream 缩范围,开工时读 spec/first100/sources/base-align-v2/23-partial-rescope-spec.md 对应条目
...
**社区参考(不采用)**:dsh-agy 28★ 覆盖≈15% — cordis-plugin·进程内·需 key/服务 \| 缺口：Tests are adapter-specific, not a reusable kit; no mock server package; no visible usag
...
**社区参考(不采用)**:dsh-plugin-subscriptions 312★ 覆盖≈25% — Registers vendor-specific routes into the native picker; per-vendor specialisation is an explicit non-goal
...
**社区参考(不采用)**:dsh-multica-runtime 60★ 覆盖≈25% — cordis-plugin·进程内·无 key \| 缺口：Not a `--model <route:model>` CLI flag on headless/interactive paths; failure does not lis
...
**社区参考(不采用)**:dsh-multica-runtime 60★ 覆盖≈40% — cordis-plugin·进程内·无 key \| 缺口：Lives in its own 'multica' profile behind --stdio, not on `dsh --profile headless` flags;
...
**社区参考(不采用)**:dsh-quota-meter 16★ 覆盖≈30% — bundle·进程内·无 key \| 缺口：No maxTurns/maxSteps budget at all. No typed `budget-exceeded` event in the session log (rejectio
...
**社区参考(不采用)**:dsh-eval-harness 14★ 覆盖≈30% — Closest shape (YAML cases, trials, baseline gate) but unlicensed, LLM-judge, no real-repo tasks — reference only
**裁决叠加(整改令)**:
...
**社区参考(不采用)**:oh-my-knowledge 18★ 覆盖≈44% — cordis-plugin·进程内·需 key/服务 \| 缺口：Guideline v1 content itself (half the epic) is authoring work, not tooling. Significanc
**裁决叠加(整改令)**:
...
for r in rows:
    ...
    if r['community']:
        top = max(r['community'], key=lambda c: c.get('coverage') or 0)
        if (top.get('coverage') or 0) > 0:
            w(f'**社区参考(不采用)**:{top["name"]}' + (f' {top["stars"]:,}★' if top.get('stars') else '') + f' 覆盖≈{top["coverage"]}%' + (f' — {esc(top.get("note"))[:120]}' if top.get('note') else ''))
    ovl = list(OV.get(eid, []))
