...
`Reproducible audit baseline + repo fingerprint (DONE)` · 层 L6_QUALIFICATION · 判决 **CONSUMER_WRITE + CONTRACT_WRITE** · 可省代码 0% · 社区插件最高覆盖 dsh-plugin-vetting 5★ 覆盖≈0% · 状态 **ACCEPTED (W1)**
...
**用什么 · 标准(绑定词汇)**:in-toto Statement v1 subject[]/ResourceDescriptor (本 epic 未采用/不采用;所有者 SLICE-3.4,见裁决叠加)
**备注 · 还得自写(residual)**:Everything except the hash primitive stays (291-line walker + spec); only representational alignment to in-toto subject[] remains.
**备注 · 风险(risk)**:None; keep as built and do not add a dependency for sha256 of ~100 files.
...
`Minimal Immutable Trust Kernel boundary (DONE)` · 层 L0_KERNEL · 判决 **KERNEL_WRITE** · 可省代码 0% · 社区插件最高覆盖 dsh-gov 1★ 覆盖≈0% · 状态 **ACCEPTED (W2)**
...
**用什么 · 标准(绑定词汇)**:SPIFFE-style URI ids (本 epic 未采用/不采用;所有者 P3-09,见裁决叠加)
**备注 · 还得自写(residual)**:100% hand-written by design: root identity, signature roots, policy entrypoint, audit append, secret-broker handle, attestation verifier (442 lines built).
**备注 · 风险(risk)**:Any OSS policy engine (OPA/Cedar/Casbin) belongs behind the enforcement entrypoint as a provider, never in the kernel.
**备注 · 计划错误(planError)**:validation[1] 'kernel must not depend on Cordis product packages' should be enforced by the P0-04 layer rule, not by review.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Capability-seam architecture checker (ACCEPTED)` · 层 L6_QUALIFICATION · 判决 **PROVIDER_ADAPT + PROVIDER_WRITE** · 可省代码 50% · 社区插件最高覆盖 dsh-tool-lens 6★ 覆盖≈0% · 状态 **ACCEPTED (W3)**
...
**备注 · 还得自写(residual)**:Family roles as rule-generator input, dated/owned allowlist (owner, removalDate), and the definition+provider fixture+composition test+rollback test presence check.
**备注 · 风险(risk)**:Already accepted as hand-rolled (~700 lines); only worth revisiting if P0-04 adopts dependency-cruiser (shared engine, one config); use --cache on ~300 packages.
**备注 · 计划错误(planError)**:Should have been PROVIDER_ADAPT(dependency-cruiser); actual is a hand-rolled scanner with 0% realized deletion today.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Layered dependency + no-cycle rules (TODO)` · 层 L6_QUALIFICATION · 判决 **PROVIDER_ADAPT + REUSE_UPSTREAM** · 可省代码 60-70% · 社区插件最高覆盖 — · 状态 **ACCEPTED (W4)**
...
**备注 · 还得自写(residual)**:docs/architecture/layering.md + layer manifest, ADR-required exemption format (adr: + owner), global-singleton-bypass grep check, 10 s budget test + 3 cycle fixtures; package-level cycles reuse scripts/package-graph.ts sinkCycles.
**备注 · 风险(risk)**:dependency-cruiser is JS/CJS-flavoured but runs fine on ESM/TS monorepos; pin exact version (hygiene gate).
**裁决叠加(整改令)**:
- §7.1 已验收核验:沉没成本,不重写(R6/§7.9)
- §7.2 R6 + §7.9:账本 leverage2 建议退掉手写扫描器换 dependency-cruiser,**推翻**——两者均为 TS AST 实现、三通道覆盖动态 import/require/path alias、规则逻辑(dated allowlist/ADR 豁免/kernel-vendor 绑定)dependency-cruiser 不提供;无下游传播;不为行数重写在跑的门
...
`Shadow/Enforce feature gates (ACCEPTED/in-flight)` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + CONTRACT_WRITE** · 可省代码 0-15% · 社区插件最高覆盖 dsh-evolution-lab 1★ 覆盖≈0% · 状态 **ACCEPTED (W3)**
...
**用什么 · 标准(绑定词汇)**:OpenFeature evaluation API (optional) (**本 epic 是形状所有者**)
**备注 · 还得自写(residual)**:Tri-state off\|shadow\|enforce semantics, redacted shadow-diff record, owner/introducedVersion/removalVersion, kernel-gated non-downgrade of enforce, --dump-config provenance chain (>80% dsh-specific; 611 lines built).
**备注 · 风险(risk)**:OpenFeature adds an SDK in the boot path for little deletion and cannot model 'run legacy AND new, record diff'; keep hand-rolled.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Unified schema registry + compatibility rules (ACCEPTED/in-flight)` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE** · 可省代码 20-30% · 社区插件最高覆盖 dsh-config-manager 63★ 覆盖≈0% · 状态 **ACCEPTED (W2)**
...
`Release Evidence Package + unforgeable completion gate (DONE)` · 层 L6_QUALIFICATION · 判决 **CONTRACT_WRITE + PROVIDER_ADAPT** · 可省代码 25% · 社区插件最高覆盖 loopx 覆盖≈0% · 状态 **ACCEPTED (W3)**
...
**用什么 · 标准(绑定词汇)**:in-toto Attestation Statement v1 (本 epic 未采用/不采用;所有者 SLICE-3.4,见裁决叠加) · DSSE envelope (本 epic 未采用/不采用;所有者 SLICE-3.4,见裁决叠加) · in-toto test-result/v0.1 predicate (本 epic 未采用/不采用;所有者 SLICE-3.4,见裁决叠加) · SLSA Provenance v1 (本 epic 未采用/不采用;所有者 SLICE-3.4,见裁决叠加)
**备注 · 还得自写(residual)**:Gate manifest, blocking-gate completeness rule (accepted=true impossible with skipped gate), tamper fixtures, pnpm evidence:* scripts; implement signature as DSSE via @sigstore/sign local Signer rather than a second envelope.
**备注 · 风险(risk)**:Re-shaping persisted JSON now is a P0-06 major bump; acceptable because evidence packages are per-run artifacts.
**备注 · 计划错误(planError)**:Built bespoke envelope (grep in-toto/dsse/slsa = 0 hits) with signature reserved but unimplemented and no envelope standard chosen.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`BENCHMARK.md → harness capability benchmark framework (TODO)` · 层 L6_QUALIFICATION · 判决 **QUALIFICATION_REUSE + REUSE_UPSTREAM** · 可省代码 40-50% · 社区插件最高覆盖 dsh-eval-harness 14★ 覆盖≈30% · 状态 **ACCEPTED (W4)**
...
**备注 · 还得自写(residual)**:benchmarks/harness-capability/manifest.yml, lane runner + JSON/Markdown report with CI, metric definitions (duplicate side effect, policy bypass, recovery success, verification precision, router regret) computed from session-log projections.
**备注 · 风险(risk)**:Do not build a new deterministic engine — runner.ts must orchestrate existing session-snapshot (4197 lines), llm-mock-server, llm-replay and 13 golden headless scenarios; real-model lane reports BLOCKED when keyless.
**备注 · 计划错误(planError)**:Deterministic lane already exists upstream (session-snapshot, seeded mock server, 13 golden scenarios) — the epic must orchestrate, not rebuild.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Plugin Manifest v2 (DONE)` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE** · 可省代码 5% · 社区插件最高覆盖 dsh-harbor 24★ 覆盖≈30% · 状态 **ACCEPTED (W3)**
...
`Signing, provenance, SBOM` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + CONTRACT_WRITE** · 可省代码 55% · 社区插件最高覆盖 dsh-provenance 1★ 覆盖≈15% · 状态 **ACCEPTED (W4)**
...
`Reproducible plugin lockfile` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + PROVIDER_WRITE** · 可省代码 40% · 社区插件最高覆盖 dsh-plugin-mall 5★ 覆盖≈20% · 状态 **BLOCKED_ON_ACCEPTANCE (W5)**
...
**备注 · 还得自写(residual)**:Overlay schema (.dsh/plugins.lock.json keyed by pnpm-lock digest: manifest digest, signature identity, source commit, load order, granted capabilities), candidate→verify→atomic-rename transaction, cross-process lock, boot-time node_modules↔lock digest check, race tests.
**备注 · 风险(risk)**:Do NOT invent a second resolver; pnpm's virtual store is the truth; parse pnpm-lock.yaml with the existing yaml dep + @pnpm/lockfile.types.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Isolated install, no lifecycle scripts` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + REUSE_UPSTREAM** · 可省代码 45% · 社区插件最高覆盖 dsh-plugin-marketplace 15★ 覆盖≈35% · 状态 **NOT_RUN (W8)**
...
**备注 · 还得自写(residual)**:Read-only quarantine layout, gate ordering (manifest→signature→SBOM→scanner), zip-bomb byte cap on tar entry stream (~30 lines), atomic promote via rename, crash-recovery journal, byte-level restore, malicious npm fixtures.
**备注 · 风险(risk)**:pnpm allowBuilds is per-profile config a market plugin can rewrite → installer must pass --ignore-scripts explicitly and treat any allowBuilds edit as policy; Windows pnpm symlink EPERM reports.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Static/dynamic plugin scanner` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + QUALIFICATION_REUSE** · 可省代码 40% · 社区插件最高覆盖 dsh-plugin-vet 3★ 覆盖≈35% · 状态 **NOT_RUN (W9)**
...
`Out-of-process host for untrusted plugins` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + PROVIDER_ADAPT** · 可省代码 20% · 社区插件最高覆盖 dsh-plugin-vet 3★ 覆盖≈0% · 状态 **NOT_RUN (W8)**
...
`Workspace trust boundary` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + CONTRACT_WRITE** · 可省代码 0-5% · 社区插件最高覆盖 hol-guard 514★ 覆盖≈20% · 状态 **ACCEPTED (W4)**
...
`ABI/capability/schema compat solver` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + PROVIDER_WRITE** · 可省代码 20% · 社区插件最高覆盖 dsh-web-plugin-manager 67★ 覆盖≈20% · 状态 **ACCEPTED (W4)**
...
**备注 · 还得自写(residual)**:Manifest fields (runtime API range, schema ranges, required/optional caps, provider constraints), backtracking solver + conflict set (~250 lines), load-plan determinism, --dump-config/inventory output, no-silent-degrade rule.
**备注 · 风险(risk)**:Keep solver tiny and property-tested; unsat core via explicit conflict-set tracking, not a SAT engine.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Namespace ownership & collision` · 层 L3_CONSUMER · 判决 **CONSUMER_WRITE + QUALIFICATION_REUSE** · 可省代码 0% · 社区插件最高覆盖 dsh-web-plugin-manager 67★ 覆盖≈25% · 状态 **ACCEPTED (W4)**
...
`Plugin data migrations & rollback` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + PROVIDER_ADAPT** · 可省代码 15% · 社区插件最高覆盖 dsh-backup 15★ 覆盖≈25% · 状态 **NOT_RUN (W8)**
...
**备注 · 还得自写(residual)**:Migration DAG + preconditions in manifest, freeze→snapshot→migrate-in-quarantine→validate→atomic-switch→health, irreversible-migration approval + export, crash-injection campaign, digest reconciliation; storage-sqlite currently has no migrations at all.
**备注 · 风险(risk)**:Local-only constraint kills durable-execution engines; decide at design whether umzug's linear model earns the dep.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Extension Proposal Pipeline (replace live cordis self-modification)` · 层 L2_PROVIDER · 判决 **CONSUMER_WRITE + REUSE_UPSTREAM** · 可省代码 10% · 社区插件最高覆盖 dsh-harmony 17★ 覆盖≈15% · 状态 **NOT_RUN (W10)**
...
**用什么 · 标准(绑定词汇)**:in-toto Statement as the proposal artifact (subject = definition digest) (所有者 SLICE-3.4:R1:envelope slice 在 P4-04(W9)前落地)
**备注 · 还得自写(residual)**:Proposal store, stage deadlines + durable state (storage-sqlite / jobs / agent-team task DAG CAS), canary, rollback to previous version, 'no browser → no hang'; stages compose P1-05/P1-02/P1-06/P2-06/P1-03.
**备注 · 风险(risk)**:tool-cordis/cordis-host-runner are upstream hot zones → add a proposal stage before startHostHalf, don't rewrite the runner.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Official verifier & trust levels` · 层 L6_QUALIFICATION · 判决 **CONTRACT_WRITE + QUALIFICATION_REUSE** · 可省代码 25% · 社区插件最高覆盖 dsh-plugin-marketplace 20★ 覆盖≈35% · 状态 **NOT_RUN (W10)**
...
`Unified Principal / Tenant / Run / Actor identity context` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE** · 可省代码 5% · 社区插件最高覆盖 dsh-awiki 14★ 覆盖≈15% · 状态 **ACCEPTED (W3)**
...
`Attenuable capability tokens + subagent delegation` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT** · 可省代码 60-70% · 社区插件最高覆盖 — · 状态 **NOT_RUN (W4)**
...
**备注 · 还得自写(residual)**:dsh fact vocabulary as Datalog facts+checks (~150–250 LOC), revocation-id store + propagation (sqlite), TrustKernel root keypair behind signatureRoots, attachment to ToolRunContext/SubagentRequest/plugin RPC/ExecutionWorld, digest-only logging, property tests.
**备注 · 风险(risk)**:Node 22 launcher needs --experimental-wasm-modules via re-exec/NODE_OPTIONS or a byte-instantiation loader (~1 day); small JS-binding community (29★) but Eclipse-governed core; keep behind ctx.capabilityTokens so the format is swappable.
**裁决叠加(整改令)**:
...
**用什么 · 标准(绑定词汇)**:RFC 8785 JCS (**本 epic 是形状所有者**) · in-toto Statement envelope (所有者 SLICE-3.4:R1:envelope slice 在 P4-04(W9)前落地) · MCP ToolAnnotations as side-effect hint input (本 epic 未采用/不采用;所有者 P2-04,见裁决叠加) · AuthZEN subject/action/resource/context (+ MCP profile / COAZ-MCP binding) (本 epic 未采用/不采用;所有者 P2-05,见裁决叠加)
**备注 · 还得自写(residual)**:Manifest type (actionId/runId/actor/capability/target/argumentsHash/sideEffectClass/idempotencyKey/preconditions/expectedDiff/compensation/evidence), durable append before tools/pre-execute at prepareExecution (~line 1450), code-mode/PTC and plugin-RPC coverage, replay pairing test.
**备注 · 风险(risk)**:JCS forbids non-finite numbers and big ints — define the argument value domain (JSON only) explicitly; do not write a second canonicalizer.
**备注 · 计划错误(planError)**:Today the tools/pre-execute waterfall runs before any durable record.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Universal side-effect & risk taxonomy` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE** · 可省代码 10-15% · 社区插件最高覆盖 dsh-auto-mode 146★ 覆盖≈30% · 状态 **NOT_RUN (W5)**
...
`Policy Decision Service + monotonic deny` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT** · 可省代码 50% · 社区插件最高覆盖 dsh-permission-rules 102★ 覆盖≈30% · 状态 **NOT_RUN (W6)**
...
**用什么 · 标准(绑定词汇)**:AuthZEN request/response vocabulary (**本 epic 是形状所有者**)
**备注 · 还得自写(residual)**:ctx.policy Service Definition decide(manifest, identity, token, world, facts), Cedar entity/schema design, PEP unification (prepareExecution + code-mode + workflow + SDK + subagent), kernel enforcement replacing the inert policyEnforcement body pinned like pinTrustKernel, plugin forbid-only API, version drift marking on replay, explain redaction.
**备注 · 风险(risk)**:BLOCKED-011: vendored Cordis Fiber fix (Option A) is a documented hard prerequisite before wiring a real enforcement point; Cedar package is heavy (13 MB) and CJS-loaded — fine for host, not browser.
**备注 · 计划错误(planError)**:P2-05 is gated on BLOCKED-011 which the wave-6 schedule does not show.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Approval bound to canonical args, resources, preconditions` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE** · 可省代码 5% · 社区插件最高覆盖 dsh-auto-mode 146★ 覆盖≈25% · 状态 **NOT_RUN (W7)**
...
**用什么 · 标准(绑定词汇)**:ACP RequestPermissionRequest.toolCall as the wire projection (**本 epic 是形状所有者**)
**备注 · 还得自写(residual)**:Extend ApprovalRequest with manifest digest + redacted view + riskClass + expectedDiff + expiresAt, preconditions.ts (inode/mtime/etag/remote version capture and re-check), re-validate digest+token+policy version before dispatch, invalidate-on-change, one-to-one approval↔action audit link.
**备注 · 风险(risk)**:TOCTOU/precondition semantics are dsh-specific; Unicode-confusable display via NFC + highlighting, hash stays over raw canonical value.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Durable cross-turn/cross-process approval queue` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE** · 可省代码 0-10% · 社区插件最高覆盖 hol-guard 514★ 覆盖≈30% · 状态 **NOT_RUN (W8)**
...
**用什么 · 标准(绑定词汇)**:AuthZEN Access Request & Approval Profile Draft 1 (所有者 P2-05,import 其定义) · ACP allow_always/reject_always option kinds (所有者 P2-06,import 其定义)
**备注 · 还得自写(residual)**:sqlite table (requested/approved/denied/expired/revoked/consumed, digest, policy version, actor, deadline), CAS consume, waiting_for_approval handoff to P4-01 run service, multi-client subscription, SDK reconnect, crash-injection tests; design so the sqlite store is one provider.
**备注 · 风险(risk)**:storage-sqlite has no explicit transactions — the approval store needs BEGIN IMMEDIATE locally in approval-store/src/sqlite.ts; do not widen the storage seam.
**备注 · 计划错误(planError)**:Durable-execution engines cannot be P2-07's default (server/Postgres) — only optional P4-01 providers.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Reusable grants, scope rules, expiry, revocation` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + PROVIDER_WRITE** · 可省代码 35% · 社区插件最高覆盖 dsh-approve-for-me 14★ 覆盖≈20% · 状态 **NOT_RUN (W9)**
...
**备注 · 还得自写(residual)**:Grant store (sqlite): counters, usage log, revocation flag, expiry sweep, bounded propagation to workers with fail-closed default; match.ts shrinks to selecting candidate templates for the principal.
**备注 · 风险(risk)**:Cedar is stateless: usage counts/revocation fed as context/entity attributes per evaluation; 'no unscoped permanent grant' needs a dsh-written linter.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Multi-party approval & separation of duties` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE** · 可省代码 10% · 社区插件最高覆盖 review-gate 2★ 覆盖≈0% · 状态 **NOT_RUN (W9)**
...
**备注 · 还得自写(residual)**:Quorum spec type returned by policy (roles, count, mutual exclusion, order, timeout), initiator ≠ approver via P2-01 ids, identity dedupe across sessions, all-approvals-still-valid check at execute time, revocation race handling, audit report (~300 LOC over P2-07 store).
**备注 · 风险(risk)**:No maintained JS library implements m-of-n human approval state.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Policy-as-code, explain, dry run` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT** · 可省代码 60% · 社区插件最高覆盖 dsh-permission-rules 102★ 覆盖≈45% · 状态 **NOT_RUN (W7)**
...
**备注 · 风险(risk)**:Cedar has no built-in test runner; 13 MB package weight; wasm boot ~tens of ms (sync); policy text is model-hidden by design.
**备注 · 计划错误(planError)**:Do not ship the planned packages/policy/policy-language/src/{parser,compiler}.ts — a homemade policy language.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Permission preset → full Policy Profile` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + CONSUMER_WRITE** · 可省代码 0% · 社区插件最高覆盖 dsh-auto-mode 146★ 覆盖≈20% · 状态 **NOT_RUN (W9)**
...
`Global emergency stop + generic human channel` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + CONTRACT_WRITE** · 可省代码 0-10% · 社区插件最高覆盖 dsh-im-gateway 45★ 覆盖≈25% · 状态 **NOT_RUN (W7)**
...
**用什么 · 标准(绑定词汇)**:MCP elicitation/create (**本 epic 是形状所有者**) · ACP session/request_permission / elicitation_create (所有者 P2-06,import 其定义) · ACP session/cancel (**本 epic 是形状所有者**)
**备注 · 还得自写(residual)**:control-plane (persisted stop state, kernel-broadcast event, lease/action gate every worker checks, explicit audited resume, cross-surface projection), in-flight policy (terminate vs reconciliation-required via P4-06 outbox), human-channel Service Definition askQuestion/requestApproval/notify implemented by MCP-elicitation, ACP, CLI, web and IM answerers.
**备注 · 风险(risk)**:The seam is the deliverable: today every remote plugin re-implements approval+question transport on approval/request and user-questions/request.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`ExecutionWorld capability seam` · 层 L1_CONTRACT · 判决 **REUSE_UPSTREAM + CONTRACT_WRITE** · 可省代码 30% · 社区插件最高覆盖 axern 59★ 覆盖≈25% · 状态 **NOT_RUN (W7)**
...
`Full-dimension sandbox policy vocabulary` · 层 L2_PROVIDER · 判决 **CONTRACT_WRITE + PROVIDER_ADAPT** · 可省代码 40% · 社区插件最高覆盖 axern 59★ 覆盖≈35% · 状态 **NOT_RUN (W8)**
...
`Typed out-of-band denial & execution outcomes` · 层 L1_CONTRACT · 判决 **REUSE_UPSTREAM + CONTRACT_WRITE** · 可省代码 50% · 社区插件最高覆盖 axern 59★ 覆盖≈30% · 状态 **NOT_RUN (W8)** · ◐ 上游已部分实现
...
`Egress proxy + destination policy` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + REUSE_UPSTREAM** · 可省代码 65% · 社区插件最高覆盖 dsh-browser 13★ 覆盖≈15% · 状态 **NOT_RUN (W9)**
...
**备注 · 还得自写(residual)**:dsh EgressPolicy (scheme/host/port/path/method, bandwidth/response-size), DNS pinning for direct-IP/rebinding at the proxy reusing web-fetch-http network.ts, byte accounting + ActionManifest evidence via commandId, wiring for browser/MCP (HTTP(S)_PROXY/ALL_PROXY + CA bundle).
**备注 · 风险(risk)**:srt is 'beta research preview' 0.0.x — pin exact version, vendor-fork risk; deps @pondwader/socks5-server, node-forge, zod3, commander (8.5 MB incl. seccomp helpers + Java agent jar).
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Process / syscall / IPC / device isolation` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + PROVIDER_ADAPT** · 可省代码 45% · 社区插件最高覆盖 axern 59★ 覆盖≈35% · 状态 **NOT_RUN (W9)** · ◐ 上游已部分实现
...
`Secrets broker: short-lived, least-scope, non-echoing leases` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + PROVIDER_ADAPT** · 可省代码 35% · 社区插件最高覆盖 dsh-config-manager 63★ 覆盖≈20% · 状态 **NOT_RUN (W8)**
...
`Cross-platform fail-closed hardening + attestation` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + CONTRACT_WRITE** · 可省代码 30% · 社区插件最高覆盖 hol-guard 514★ 覆盖≈30% · 状态 **NOT_RUN (W10)** · ◐ 上游已部分实现
...
**用什么 · 标准(绑定词汇)**:in-toto Statement v1 + DSSE envelope for the attestation artifact (所有者 SLICE-3.4:R1:envelope slice 在 P4-04(W9)前落地) · SLSA-style predicate (所有者 SLICE-3.4:R1:envelope slice 在 P4-04(W9)前落地)
**备注 · 还得自写(residual)**:Predicate schema (os, kernel, provider, version, supportedPolicyFeatures, enforcement), requested ⊆ supported check at each execute, explicit dev-degrade flag, conformance e2e across 3 platforms; verified by the trust-kernel sandboxAttestationVerifier stub.
**备注 · 风险(risk)**:sigstore-js adds ~2 MB deps; hand-rolling DSSE is acceptable but keep the FORMAT.
**备注 · 计划错误(planError)**:P3-07 lists B sandbox-local/src edits — hot zone; upstream already has functional probes and refuse-not-warn.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Container ExecutionWorld provider` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT** · 可省代码 55% · 社区插件最高覆盖 dsh-worlds 3★ 覆盖≈50% · 状态 **NOT_RUN (W10)**
...
**用什么 · 标准(绑定词汇)**:OCI runtime-spec HostConfig knobs (ReadonlyRootfs, Tmpfs, SecurityOpt no-new-privileges/seccomp, CapDrop ALL, PidsLimit, Memory, NanoCpus) (所有者 P3-01,import 其定义)
**备注 · 还得自写(residual)**:WorldSpec→HostConfig mapping, egress-proxy wiring (network=none + unix-socket or proxy sidecar sharing netns — design decision), secret-lease injection (tmpfs file/FD, not env), attestation predicate (image digest, runtime, rootless), validator rejecting docker.sock/$HOME mounts, conformance provider tests.
**备注 · 风险(risk)**:Docker/Podman daemon optional (macOS CI has no Docker by default → provider reports unavailable, fail-closed).
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`MicroVM / remote world + attestation` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + CONTRACT_WRITE** · 可省代码 50% · 社区插件最高覆盖 axern 59★ 覆盖≈45% · 状态 **NOT_RUN (W8)**
...
**用什么 · 标准(绑定词汇)**:in-toto/DSSE attestation (所有者 SLICE-3.4:R1:envelope slice 在 P4-04(W9)前落地) · SPIFFE ID format for tenant/world identity (**本 epic 是形状所有者**)
**备注 · 还得自写(residual)**:Reconnect/reconciliation (idempotent action-lease ledger), tenant-scoped attach tokens, partition tests, provider-signed in-toto Statement (image digest, policy hash, tenant, proxy config hash, secret-injection mode) with hardware: none\|sev-snp\|tdx field.
**备注 · 风险(risk)**:TPM/SEV-SNP/TDX attestation out of scope (no provider surfaces it; Go-only tooling); microsandbox needs KVM/HVF → GitHub Linux runners without nested virt fall to unavailable (fail-closed).
**备注 · 计划错误(planError)**:TPM/SEV-SNP attestation is not achievable with any keyless/local default — declare software attestation with explicit hardware:none.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Resource quotas (CPU/mem/disk/time/pids/net)` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + PROVIDER_ADAPT** · 可省代码 35% · 社区插件最高覆盖 dsh-plugin-container 4★ 覆盖≈20% · 状态 **NOT_RUN (W8)**
...
`World snapshot / restore / rollback` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + CATALOG_ADOPT** · 可省代码 50% · 社区插件最高覆盖 dsh-filesnap 32★ 覆盖≈35% · 状态 **NOT_RUN (W11)**
...
**备注 · 还得自写(residual)**:WorldSnapshot manifest (provider metadata, running-action boundary, secret REFERENCES not values, lineage), new-world-identity-on-restore (token rotation, lease non-reuse), corruption/version-mismatch typed outcomes, leak test loop.
**备注 · 风险(risk)**:CRIU Linux-only/root, breaks with TCP sockets/GPU — optional; container commit snapshots fs only → mark consistent: fs-only.
**备注 · 计划错误(planError)**:CRIU process-state snapshots are not achievable as a keyless/local default — declare fs-only snapshots as the contract.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Workspace path, attachment admission, malicious-input hardening` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + PROVIDER_ADAPT** · 可省代码 40% · 社区插件最高覆盖 dsh-docs 13★ 覆盖≈30% · 状态 **NOT_RUN (W8)** · ◐ 上游已部分实现
...
`(not in ledger)` · 层 L2_PROVIDER · 判决 **未判定** · 可省代码 None% · 社区插件最高覆盖 — · 状态 **NOT_RUN (W9)**

**SDD · 规格**:见 registry(P3-13 由 C8 收录);TDD 未冻结。
**用什么(OSS / 标准)**:— 不在账本内,开工时补判
**备注 · 还得自写(residual)**:账本 2026-09-02 生成时本 epic 同日才收录;开工时按 §4.1 补一次单条 make-vs-use 判断并写进本卡(delegate 维护 overlay)。
**备注 · 风险(risk)**:—
**裁决叠加(整改令)**:
- §3.2 共用引擎 sandbox-runtime(@anthropic-ai/sandbox-runtime,exact pin)在 W7 前作为 sandbox-srt rung 接入;该 slice 同时把 sandbox-local 的 PLATFORM_CHAINS 表改成 contribution point
...
`Run service + append-only run event log` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + CONTRACT_WRITE** · 可省代码 0-5% · 社区插件最高覆盖 dsh_workflow 113★ · 状态 **ACCEPTED (W4)**
...
**用什么 · 标准(绑定词汇)**:CloudEvents attribute names (id/source/type/time/subject/datacontenttype) (本 epic 未采用/不采用;所有者 P4-06,见裁决叠加) · A2A TaskState / MCP TaskStatus names at surfaces (本 epic 未采用/不采用;所有者 P4-05,见裁决叠加)
**备注 · 还得自写(residual)**:10-state table (~80 LOC), sqlite runs/run_events tables via node:sqlite, restart scan of non-terminal runs, cross-refs to sessions.
**备注 · 风险(risk)**:No engine can be the keyless local default (server / Postgres / BSL / SSPL) and a Run owned by an external engine breaks 'session log is source of truth' — recommend no engine provider even as optional.
**备注 · 计划错误(planError)**:run/* event types must be added to core/session known-event-types (closed list) or the run log lives in its own sqlite tables with session refs — decide explicitly; plugin-side event registration does not work (dsh-llm-fallbacks #52).
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`TaskProfile (generic compiled task profile)` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE** · 可省代码 0% · 社区插件最高覆盖 dsh-trajectory-governor 13★ 覆盖≈30% · 状态 **NOT_RUN (W6)**
...
**用什么 · 标准(绑定词汇)**:JSON Schema for the profile via zod (none exists for the concept) (所有者 P0-06,import 其定义)
**备注 · 还得自写(residual)**:Pure dsh vocabulary: goal ref, hard/soft constraints with source+confidence, side-effect class (unknown ≠ none), open questions routed to packages/interaction/user-questions; vitest golden fixtures.
**备注 · 风险(risk)**:Vertical creep is the risk, not OSS; keep fields generic per must#1.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`RunPlan (executable plan data)` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE** · 可省代码 5% · 社区插件最高覆盖 dsh_workflow 113★ 覆盖≈20% · 状态 **NOT_RUN (W8)**
...
**用什么 · 标准(绑定词汇)**:RFC 8785 JCS + sha256 for deterministic plan id (所有者 P2-03,import 其定义)
**备注 · 还得自写(residual)**:Satisfiability over typed constraints (capability ⊆ available, budget ≤ cap, policy allow) + deletion-based minimal conflict set (~100 LOC); verificationContractRef as versioned opaque ref (P7-01 fills it).
**备注 · 风险(risk)**:Keep verificationContractRef opaque and versioned.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Plan freeze, kernel signature, amendment CAS` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE** · 可省代码 10-15% · 社区插件最高覆盖 allinluna 51★ 覆盖≈20% · 状态 **NOT_RUN (W9)**
...
`Agent lifecycle state machine (PARTIAL)` · 层 L3_CONSUMER · 判决 **REUSE_UPSTREAM + CONSUMER_WRITE** · 可省代码 0% · 社区插件最高覆盖 allinluna 51★ 覆盖≈35% · 状态 **NOT_RUN (W5)** · ◐ 上游已部分实现
...
**用什么 · 标准(绑定词汇)**:A2A TaskState / MCP TaskStatus mapping at surfaces (waiting_human ↔ input_required) (**本 epic 是形状所有者**)
**备注 · 还得自写(residual)**:10-state validated transition table with {reason, runId, leaseEpoch}, stale-epoch rejection, orphan reclaim on restart, back-compat mapping to idle\|running (~150 LOC + fast-check); upstream already has inbox projection, consumed-work, holder model.
**备注 · 风险(risk)**:Wave inversion: P4-05 (wave 5) needs leaseEpoch from P4-07 (wave 6) — declare leaseEpoch as opaque number now, enforce fencing in P4-07.
**备注 · 计划错误(planError)**:P4-05 depends on lease epoch from P4-07 scheduled a wave later — declare the field early.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Durable inbox/outbox, effectively-once handoff (PARTIAL)` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + PROVIDER_WRITE** · 可省代码 0-5% · 社区插件最高覆盖 allinluna 51★ 覆盖≈35% · 状态 **BLOCKED_ON_ACCEPTANCE (W5)** · ◐ 上游已部分实现
...
**用什么 · 标准(绑定词汇)**:CloudEvents attribute names; dedup on id (**本 epic 是形状所有者**)
**备注 · 还得自写(residual)**:Outbox tables + BEGIN IMMEDIATE transactional write (cannot use KV seam), dispatcher receipts, consumer dedup by (messageId, epoch), priority/deadline/DLQ/backpressure, tenant column checks (~400 LOC + fault matrix).
**备注 · 风险(risk)**:'Exactly-once' is at-least-once + idempotent consumer; say so in the contract.
**备注 · 计划错误(planError)**:P4-06 should say 'at-least-once + idempotent consumer' not 'exactly-once'; transactional outbox cannot use the storage KV seam (single-statement by design).
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Worker lease, heartbeat, fencing token` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + QUALIFICATION_REUSE** · 可省代码 0% · 社区插件最高覆盖 allinluna 51★ 覆盖≈30% · 状态 **ACCEPTED (W6)**
...
`Workflow journal + step-level resume` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE** · 可省代码 0% · 社区插件最高覆盖 dsh_workflow 113★ 覆盖≈35% · 状态 **ACCEPTED (W7)**
...
**备注 · 还得自写(residual)**:Journal record {runId, scriptDigest, seq, kind, inputDigest, receipt\|artifactRef, phase, epoch} on top of protocol.ts + logged tool-workflow/agent-start\|end events; replay returns recorded receipt; digest mismatch ⇒ refuse resume (~400–600 LOC); side effects hand off to P4-12/13.
**备注 · 风险(risk)**:Adopting Temporal would replace workflow-worker-thread (1987 LOC, adjacent to hot zones) and impose determinism rules on model scripts — a rewrite, not a delete.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Detached / saved / versioned / nested workflows` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE** · 可省代码 0-5% · 社区插件最高覆盖 dsh_workflow 113★ · 状态 **BLOCKED_ON_ACCEPTANCE (W8)**
...
**用什么 · 标准(绑定词汇)**:sha256 content digest + DSSE signature (P4-04) for saved definitions (所有者 SLICE-3.4:R1:envelope slice 在 P4-04(W9)前落地)
**备注 · 还得自写(residual)**:Signed-artifact load path that never executes unverified code, detached ownership by the Run service, nested budget/capability/trace decay, recursion/cycle rejection at compile, parent-cancel propagation (~400 LOC).
**备注 · 风险(risk)**:Detached runs need durable job records — jobs-local is in-memory and can't own them; the Run service must.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Budget, scheduler, backpressure, fairness, locks (PARTIAL)` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + PROVIDER_WRITE** · 可省代码 15% · 社区插件最高覆盖 allinluna 51★ 覆盖≈35% · 状态 **NOT_RUN (W9)** · ◐ 上游已部分实现
...
**备注 · 还得自写(residual)**:Multi-dim BudgetSpec + parent/child accumulation, tenant WFQ/priority aging (~100 LOC deficit round-robin), cross-process sqlite lock rows, exclusive tools, Backpressure message into the worker protocol (closed enum — B-edit), lock release on cancel (~600 LOC).
**备注 · 风险(risk)**:No npm lib does tenant-fair hierarchical scheduling; locks must be sqlite rows if workers are separate processes.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Retry classifier, circuit breaker, retry budget (PARTIAL)` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + PROVIDER_ADAPT** · 可省代码 30-35% · 社区插件最高覆盖 dsh-llm-fallbacks 20★ · 状态 **NOT_RUN (W8)** · ◐ 上游已部分实现
...
**备注 · 还得自写(residual)**:Cross-layer error taxonomy extending retry-policy codes, RunRetryBudget (shared CAS counter in run store), Retry-After parsing (~20 LOC), hedge exclusion, idempotency-gated retry (P4-12 check); keep llm-retry as the durable-before-wait executor.
**备注 · 风险(risk)**:cockatiel has no shared cross-policy budget and no Retry-After parsing; don't replace llm-retry's durable events with cockatiel's retry policy.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`External side-effect idempotency ledger` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE** · 可省代码 0-5% · 社区插件最高覆盖 allinluna 51★ 覆盖≈25% · 状态 **NOT_RUN (W7)**
...
`Reconciliation engine + saga compensation` · 层 L3_CONSUMER · 判决 **CONSUMER_WRITE** · 可省代码 5-10% · 社区插件最高覆盖 allinluna 51★ 覆盖≈15% · 状态 **NOT_RUN (W13)**
...
**用什么 · 标准(绑定词汇)**:JSON Patch RFC 6902 for StateDiff/repair-option representation (**本 epic 是形状所有者**)
**备注 · 还得自写(residual)**:observeState/compareExpected/compensate tool-declared hooks; engine as a workflow over the Run service emitting compensation ActionManifests (policy/approval/ledger/evidence); compensation runs as a nested run (P4-09) (~350 LOC).
**备注 · 风险(risk)**:Never fabricate rollback: irreversible ⇒ manual intervention; verify external state, not agent self-report.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Partial-turn resume, durable schedule/goal triggers (PARTIAL)` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + CONSUMER_WRITE** · 可省代码 0% · 社区插件最高覆盖 dsh-harness-one 17★ 覆盖≈35% · 状态 **NOT_RUN (W14)** · ◐ 上游已部分实现
...
**备注 · 看过但不用(reject)**:
- breejs/bree — No
**备注 · 还得自写(residual)**:Resume classifier joining repair codes (TOOL_NOT_STARTED / TOOL_OUTCOME_UNKNOWN) with ActionLedger/WorkflowJournal (continue / replay pure / reconcile), trigger events as outbox rows (P4-06) claimed by the scheduler with explicit catch-up enum skip\|run-once\|run-each (~200 LOC).
**备注 · 风险(risk)**:Don't add a second checkpoint store; the session log already is one (step/start, tool/call, tool/result, assistant/message + repair.ts).
**备注 · 计划错误(planError)**:P4-14's turn-checkpoint package is mostly redundant with upstream session events + repair.ts — collapse to a resume classifier.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Strategy router (answer / ReAct / plan-execute / durable-workflow / multi-agent)` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + QUALIFICATION_REUSE** · 可省代码 0-10% · 社区插件最高覆盖 ruflo 70,209★ 覆盖≈30% · 状态 **NOT_RUN (W9)**
...
**备注 · 还得自写(residual)**:All of rules.ts/types.ts/index.ts (~400 lines) + 100 fixtures as a waterfall listener like model-selection.ts emitting a strategy/decision session event.
**备注 · 风险(risk)**:B-files core/agent/dispatch.ts and agent-loop/agent.ts are upstream hot zone — implement as a waterfall listener, do not edit the loop; ship rule-based first.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Model router (success / cost / latency / residency / tool capability)` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + REUSE_UPSTREAM** · 可省代码 20-30% · 社区插件最高覆盖 dsh-plugin-subagent-director 14★ 覆盖≈25% · 状态 **NOT_RUN (W10)**
...
**备注 · 还得自写(residual)**:score.ts (data residency / observed latency / success rate exist in no catalog), router as waterfall over agent/request, regret ledger hooks fed from session-log outcomes.
**备注 · 风险(risk)**:Learned routers are Python + dead; every hosted router breaks keyless CI; vendored catalog snapshots drift → pin + refresh script; agent/request is a hot zone.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Capability negotiation + provider-specific prompt compiler` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + CONTRACT_WRITE** · 可省代码 40-50% · 社区插件最高覆盖 dsh-vision-router 1,051★ 覆盖≈10% · 状态 **NOT_RUN (W11)**
...
**用什么 · 标准(绑定词汇)**:JSON Schema for the output contract (所有者 P0-06,import 其定义) · OTel GenAI semconv gen_ai.usage.* for token-estimate telemetry (所有者 SLICE-3.3:§3.3:名字来自 @opentelemetry/semantic-conventions 常量包,slice 接 pipeline;时点改为 W11 前(P5-03/P5-06 首发 gen_ai.usage.*))
**备注 · 还得自写(residual)**:PromptIR types (instructions / context slices / tool surface / output contract / policy notices), IR→dsh Message[] lowering (one function), clause-preservation golden tests, negotiation + IR hash written into EpochHeader via request-header.ts foldRequestHeader.
**备注 · 风险(risk)**:Do not create a second provider-translation layer; token estimate uses CHARS_PER_TOKEN=4 until the tokenizer epic lands.
**备注 · 计划错误(planError)**:Planned packages/llm/prompt-compiler/src/compile.ts per provider already exists as pi-ai compat + llm-pi-ai/src/context.ts toPiContext + llm-deepseek/src/translate.ts.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Provider fallback, hedging, rate limit, budget` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + REUSE_UPSTREAM** · 可省代码 30-40% · 社区插件最高覆盖 dsh-llm-fallbacks 20★ 覆盖≈38% · 状态 **NOT_RUN (W11)**
...
**备注 · 还得自写(residual)**:hedge.ts (pre-action-only, single-winner fencing keyed to the action ledger), budget reservation against RunPlan, regional failover policy, core-registered llm/fallback event; keep llm-retry/retry-policy as backoff.
**备注 · 风险(risk)**:Hedging exists in no JS lib (Polly .NET has it); cockatiel/rate-limiter state is in-memory (not durable across restarts — acceptable).
**备注 · 计划错误(planError)**:The durable fallback/switch event must be a core event type (dsh-llm-fallbacks #52 shows plugin-declared event types are not loadable).
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Structured SubagentRequest contract` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE** · 可省代码 0% · 社区插件最高覆盖 dsh_workflow 113★ 覆盖≈40% · 状态 **NOT_RUN (W9)**
...
`Structured SubagentResult + evidence return` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE** · 可省代码 0% · 社区插件最高覆盖 allinluna 51★ 覆盖≈40% · 状态 **NOT_RUN (W11)**
...
**用什么 · 标准(绑定词汇)**:A2A Artifact + TaskState enum (incl. input-required/auth-required/rejected) as failure class (所有者 P5-05,import 其定义) · ACP session/update tool_call ids for tool-trace refs (所有者 P2-12,import 其定义) · OTel GenAI gen_ai.usage.* for usage/cost (所有者 SLICE-3.3:§3.3:名字来自 @opentelemetry/semantic-conventions 常量包,slice 接 pipeline;时点改为 W11 前(P5-03/P5-06 首发 gen_ai.usage.*))
**备注 · 还得自写(residual)**:result.ts (status, summary, artifact refs, state diffs, action receipts, tool trace refs, usage/cost, verification hints, continuation token), Artifact Store spill for large results, per-provider capability flags; keep structured validation provider-owned.
**备注 · 风险(risk)**:Same hot-zone caveat; cost reconciliation needs token-meter route pricing (route-pricing.ts exists).
**备注 · 计划错误(planError)**:P5-06 patches the upstream subagent hot zone — land upstream first.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Codex adapter: structured stream, continuation, approval, evidence` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + PROVIDER_ADAPT** · 可省代码 60-70% · 社区插件最高覆盖 dsh-cli-bridge 2★ 覆盖≈40% · 状态 **NOT_RUN (W12)** · ◐ 上游已部分实现
...
**备注 · 还得自写(residual)**:Route requestApproval to parent policy/user-approval seam instead of unattendedDecision, persist threadId + thread/resume\|fork, turn/steer for steer, collect diff/test/usage items into SubagentResult evidence; wire.ts (703 lines) already maps items.
**备注 · 风险(risk)**:App-server v2 protocol churns (v1.rs + v2 mappers in flux) — wire tests must use recorded fixtures; version pin drift needs a renovate rule.
**备注 · 计划错误(planError)**:Lists 'new' continuation/map-events files for features the pinned SDK already ships.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Claude Code adapter: structured stream, session resume, tool/artifact mapping` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + PROVIDER_ADAPT** · 可省代码 60-70% · 社区插件最高覆盖 dsh-cli-bridge 2★ 覆盖≈45% · 状态 **NOT_RUN (W12)** · ◐ 上游已部分实现
...
**备注 · 还得自写(residual)**:canUseTool → parent policy (today run.ts:326-357 pre-computes disallowedTools), persist session_id + resume/forkSession, query.interrupt() → cancel, SDK subagent messages + usage → SubagentResult, cwd = worktree from P5-12, pin settingSources: [] with a test that permissive local settings are ignored.
**备注 · 风险(risk)**:SDK is 0.3.x and moves daily; 'Claude's own settings cannot bypass parent policy' must be asserted by a test, not assumed.
**备注 · 计划错误(planError)**:Lists 'new' files for features the pinned SDK already ships.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`ACP provider: remote resumable session, trace enumeration, secure identity` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + CONTRACT_WRITE** · 可省代码 50% · 社区插件最高覆盖 dsh-plugin-product-subagents 19★ 覆盖≈20% · 状态 **NOT_RUN (W18)** · ◐ 上游已部分实现
...
**用什么 · 标准(绑定词汇)**:ACP 1.4.0 session/load, session/resume, session/fork, session/list, session/request_permission, authenticate, sessionCapabilities (所有者 P2-12,import 其定义)
**备注 · 还得自写(residual)**:Negotiate sessionCapabilities at initialize, session/load\|resume on reconnect, session/request_permission → parent policy, session/fork; dsh-side monotonic cursor on the child's session log (ACP has none); Trust-Kernel-signed continuation token for identity anti-forgery.
**备注 · 风险(risk)**:Protocol version pinning (PROTOCOL_VERSION 2; fork pins 1.4.0 while npm latest is 1.3.0 — confirm channel); authenticated steer beyond prompt/cancel is not an ACP method.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Continuation / steer / human-input / cancellation convergence` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + QUALIFICATION_REUSE** · 可省代码 0% · 社区插件最高覆盖 dsh-Visual-Workflow 14★ 覆盖≈25% · 状态 **NOT_RUN (W7)** · ◐ 上游已部分实现
...
**备注 · 还得自写(residual)**:Unified 5-way vocabulary (continue/steer/inject/cancel/human-answer) with priority + state preconditions, durable epoch-tagged control log, convergence barrier (child + world + actions quiescent before terminal); continuation.ts (1,569 lines) already has steer/inject/cancel/epoch/cold-resume/settlement.
**备注 · 风险(risk)**:Highest hot-zone risk in the chapter: continuation.ts is upstream-owned and refactored monthly — land vocabulary + barrier upstream; fork keeps the property test and the parent-crash fixture.
**备注 · 计划错误(planError)**:P5-10 patches the upstream subagent hot zone — land upstream first.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Generic taskboard / mailbox / blackboard primitives` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + PROVIDER_WRITE** · 可省代码 40-50% · 社区插件最高覆盖 dsh-agent-teams 1,285★ 覆盖≈48% · 状态 **ACCEPTED (W7)** · ◐ 上游已部分实现
...
**用什么 · 标准(绑定词汇)**:W3C PROV-DM field names for blackboard provenance (wasGeneratedBy, wasAttributedTo, wasDerivedFrom) (本 epic 未采用/不采用;所有者 P6-09,见裁决叠加)
**备注 · 还得自写(residual)**:Attempt/lease/owner fields, multi-process single-winner claim (sqlite BEGIN IMMEDIATE + UPDATE … WHERE owner IS NULL), receipt-driven status advance, blackboard (structured facts + artifact refs + provenance), moving roster/captain out to plugin layer.
**备注 · 风险(risk)**:Planned packages/collaboration/* would be a second implementation beside agent-team's 700+ lines of DAG/mailbox.
**备注 · 计划错误(planError)**:P5-11 plans packages/collaboration/* from scratch next to agent-team — promote agent-team's DAG/mailbox into the Service Definition instead.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Multi-agent coordination safety, worktree isolation, router-regret eval` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + CONSUMER_WRITE** · 可省代码 10% · 社区插件最高覆盖 dsh-cli-bridge 2★ 覆盖≈35% · 状态 **NOT_RUN (W10)**
...
**备注 · 还得自写(residual)**:worktree-provider (add/remove/prune/merge queue + verifier gate, ~300 lines), coordination-guard (wait-for graph, message-storm/no-progress/duplicate-task detectors, agent budget), router-regret (paired single-vs-multi runs → regret).
**备注 · 风险(risk)**:Merge queue semantics (verifier-gated, explicit) are dsh-specific; 50-agent scale test needs the P5-11 sqlite claim first; regret needs P5-01 decisions persisted + a P8/P9 benchmark corpus.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Provider-neutral Memory Service Definition` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE + CATALOG_ADOPT** · 可省代码 0% · 社区插件最高覆盖 dsh-memento 80★ 覆盖≈35% · 状态 **NOT_RUN (W4)**
...
`MemoryRecord: provenance, confidence, TTL, scope, purpose, conflict` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE + QUALIFICATION_REUSE** · 可省代码 0-5% · 社区插件最高覆盖 dsh-memento 80★ 覆盖≈25% · 状态 **ACCEPTED (W5)**
...
`Memory proposal / verify / merge / forget / export` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + PROVIDER_ADAPT** · 可省代码 25-35% · 社区插件最高覆盖 dsh-memento 80★ 覆盖≈40% · 状态 **NOT_RUN (W8)**
...
**备注 · 还得自写(residual)**:Policy, proposal evidence check, tombstone + erase SLA propagation (FTS/vec tables, projection caches, P6-09 refs), export with provenance, hallucinated-memory corpus (must be synthesized); EmbeddingProvider is a small L1 gap.
**备注 · 风险(risk)**:sqlite-vec is pre-1.0 (0.1.9, last push 2026-05) — pin + conformance-test the extension load path; Windows partial; do NOT pick a second sqlite driver.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Context graph + retrieval planner` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + PROVIDER_ADAPT** · 可省代码 15-20% · 社区插件最高覆盖 dsh-context 1,238★ · 状态 **NOT_RUN (W11)**
...
**备注 · 还得自写(residual)**:RetrievalPlan/budget/stop/trace/dedup-by-source/untrusted-data marking, policy filter, injection fixtures as JSON; tokenizer upgrade belongs behind token-meter seam.
**备注 · 风险(risk)**:Local graph DBs are dead or stale on npm — keep the graph in-memory (graphology) + sqlite-backed edges; Python corpora converted once (keyless).
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Per-agent context topology + telemetry contract` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + CONTRACT_WRITE** · 可省代码 0% · 社区插件最高覆盖 dsh-capability-menu 84★ 覆盖≈15% · 状态 **NOT_RUN (W12)** · ◐ 上游已部分实现
...
**用什么 · 标准(绑定词汇)**:OTel semconv gen_ai.* attribute names (+ dsh.context.source_id / selection_reason) (所有者 SLICE-3.3:§3.3:名字来自 @opentelemetry/semantic-conventions 常量包,slice 接 pipeline;时点改为 W11 前(P5-03/P5-06 首发 gen_ai.usage.*))
**备注 · 还得自写(residual)**:Declarative shared/private/retrievable zones in RunPlan + stable telemetry record (source ids, token counts, reasons, redacted previews); upstream child-agent.ts seed boundary + telemetry redaction waterfall cover 'child does not inherit full history'.
**备注 · 风险(risk)**:Telemetry token counts come from the CHARS/4 heuristic — label them estimated or adopt a real tokenizer.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Compaction fidelity, provenance, tool pairing` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + PROVIDER_WRITE** · 可省代码 5% · 社区插件最高覆盖 dsh-compaction-instant 14★ 覆盖≈40% · 状态 **NOT_RUN (W11)** · ◐ 上游已部分实现
...
**备注 · 还得自写(residual)**:coverage.ts / provenance.ts marks (covered ranges, preserved constraints, open actions, evidence refs, dropped categories) on CompactionResult + fidelity e2e; upstream has the balance invariant, checkpoint source and sectioned summarizer.
**备注 · 风险(risk)**:Any change to compaction/* event shapes touches the hot zone (compaction-basic upstream-owned) — keep marks additive on the payload.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Session lifecycle: paging, filters, delete, retention, repair` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + PROVIDER_WRITE** · 可省代码 0-5% · 社区插件最高覆盖 dsh-session-manager 58★ · 状态 **ACCEPTED (W4)** · ◐ 上游已部分实现
...
**备注 · 还得自写(residual)**:tenant/workspace/status filter kinds, lifecycle state machine (soft-delete / legal-hold / hard-erase / archive), propagation contract to attachments/memory/artifacts, damage-report surfacing of existing repair codes.
**备注 · 风险(risk)**:'Million-session fixture' must be generated, not a corpus; keep cursor opaque (already branded).
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Encryption at rest, tenant keys, tamper-evident audit, residency` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + PROVIDER_WRITE** · 可省代码 30-40% · 社区插件最高覆盖 Cambium 294★ 覆盖≈20% · 状态 **NOT_RUN (W9)**
...
`First-class artifact store: versions, CAS, lineage` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE + PROVIDER_ADAPT** · 可省代码 35-45% · 社区插件最高覆盖 dsh-research-report 41★ · 状态 **NOT_RUN (W10)**
...
`Privacy classification, redaction, fork lineage, export/erase` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + CONTRACT_WRITE** · 可省代码 15-25% · 社区插件最高覆盖 dsh-secure-audit 84★ · 状态 **NOT_RUN (W11)** · ◐ 上游已部分实现
...
`VerificationContract frozen before execution` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE** · 可省代码 15% · 社区插件最高覆盖 deepseek-harness-reliability-governor 2★ 覆盖≈40% · 状态 **NOT_RUN (W10)**
...
**用什么 · 标准(绑定词汇)**:in-toto Statement v1 + in-toto layout semantics (steps/inspections/functionaries/threshold) (所有者 SLICE-3.4:R1:envelope slice 在 P4-04(W9)前落地) · JSON Schema 2020-12 via zod4 toJSONSchema (所有者 P0-06,import 其定义) · RFC 8785 JCS canonical hash (所有者 P2-03,import 其定义)
**备注 · 还得自写(residual)**:Claim/CheckSpec/EvidenceRequirement/AcceptanceRule/confidence policy types, freeze semantics inside RunPlan, PlanAmendment re-approval path, ledger events with schemaVersion.
**备注 · 风险(risk)**:Predecessors P4-03/P4-04 (RunPlan) do not exist in the tree; freeze semantics have nowhere to attach yet.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`EvidenceCollector: content-addressed, unforgeable` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + CONTRACT_WRITE** · 可省代码 35% · 社区插件最高覆盖 dsh-research-report 42★ 覆盖≈25% · 状态 **NOT_RUN (W12)**
...
**用什么 · 标准(绑定词汇)**:in-toto ResourceDescriptor as EvidenceRef (所有者 SLICE-3.4:R1:envelope slice 在 P4-04(W9)前落地) · DSSE envelope for sealed EvidenceEnvelope (所有者 SLICE-3.4:R1:envelope slice 在 P4-04(W9)前落地)
**备注 · 还得自写(residual)**:Collection hooks at tool-completion / subagent-result / artifact-commit / approval boundaries, raw-vs-projection split with permission+token-budget filter, dedupe keyed by (hash, principal, time-context), retention policy, HTTP request/response summary capture, crash ordering (CAS commit before ledger ref).
**备注 · 风险(risk)**:cacache stores in its own dir layout; if the storage chapter mandates storage-sqlite, cacache is a second store → fall back to a ~150-LOC CAS over node:sqlite blobs.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Independent Verifier provider seam` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE + REUSE_UPSTREAM** · 可省代码 10% · 社区插件最高覆盖 deepseek-harness-reliability-governor 2★ 覆盖≈35% · 状态 **NOT_RUN (W13)**
...
**备注 · 还得自写(residual)**:VerifierProvider/VerificationRequest/VerificationReport/CheckResult types, kernel-enforced capability non-inheritance, executorId ≠ verifierId policy, quorum coordinator, timeout/abstain → unverified, deterministic verifier reading EvidenceRef + real world.
**备注 · 风险(risk)**:Verifier isolation is a kernel/policy property; wrapping any OSS judge in a plugin does not deliver it.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`ClaimGraph (claim–evidence–contradiction–uncertainty)` · 层 L2_PROVIDER · 判决 **CONTRACT_WRITE + PROVIDER_WRITE** · 可省代码 15% · 社区插件最高覆盖 grove 22★ 覆盖≈25% · 状态 **NOT_RUN (W14)**
...
**用什么 · 标准(绑定词汇)**:W3C PROV-DM relations (wasDerivedFrom, wasGeneratedBy, wasInvalidatedBy, wasAttributedTo) (所有者 P6-09,import 其定义) · in-toto SCAI predicate (attributes + evidence + conditions) (所有者 SLICE-3.4:R1:envelope slice 在 P4-04(W9)前落地)
**备注 · 还得自写(residual)**:Status lattice (proposed/verified/stale/conflicted/unverified), propagation on expiry/revocation/contradiction, verifier-only verified transition, projector from ledger, OutcomePackage rendering that never drops contradictions.
**备注 · 风险(risk)**:graphology is optional; vertical scoring tables are out of scope (nonGoal).
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`AcceptanceGate + OutcomePackage (signed, content-addressed)` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE** · 可省代码 20% · 社区插件最高覆盖 deepseek-harness-reliability-governor 2★ 覆盖≈35% · 状态 **NOT_RUN (W15)**
...
`Bounded repair / replan loop` · 层 L3_CONSUMER · 判决 **CONSUMER_WRITE + QUALIFICATION_REUSE** · 可省代码 5% · 社区插件最高覆盖 deepseek-harness-reliability-governor 2★ 覆盖≈40% · 状态 **NOT_RUN (W16)**
...
**备注 · 还得自写(residual)**:RepairPlan from failed checks, PlanAmendment, idempotency via Action Ledger/Reconciliation, budget ceilings, needs-human escalation — dsh's own state machine over P4-11/12/13.
**备注 · 风险(risk)**:Keep provider HTTP retry (llm-retry) and task-level repair separate (nonGoal).
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Causal trace + policy/evidence/cost trace + durable telemetry outbox` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + PROVIDER_WRITE** · 可省代码 40% · 社区插件最高覆盖 dsh-observe 4★ 覆盖≈45% · 状态 **NOT_RUN (W13)**
...
`Deterministic replay, simulation, decision diff` · 层 L3_CONSUMER · 判决 **REUSE_UPSTREAM + PROVIDER_ADAPT** · 可省代码 50% · 社区插件最高覆盖 dsh-repro 2★ 覆盖≈15% · 状态 **NOT_RUN (W14)**
...
**用什么 · 标准(绑定词汇)**:RFC 6902 JSON Patch for DecisionDiff (所有者 P4-13,import 其定义)
**备注 · 还得自写(residual)**:ReplayBundle schema (schema fingerprints, RunPlan, policy inputs, seeds, artifact refs), replay-mode enforcement 'no real network/write' through sandbox/tool seams, non-HTTP world observations (fs/process) recording, shadow replay of Router/Policy/Prompt Compiler, version-mismatch refusal/migration.
**备注 · 风险(risk)**:SnapshotAgent is flagged experimental by undici — pin the version and keep nock as fallback; policy replay needs the policy engine to be pure over recorded inputs.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`General-purpose capability scenario suite (15 scenarios, two lanes)` · 层 L6_QUALIFICATION · 判决 **QUALIFICATION_REUSE** · 可省代码 30% · 社区插件最高覆盖 dsh-eval-harness 14★ 覆盖≈30% · 状态 **NOT_RUN (W16)**
...
**备注 · 还得自写(residual)**:The 8 worlds (code/research/external-write/high-risk/long-run/malicious-plugin/multi-tenant/sdk-reconnect) as fixture Skill/Provider bundles + manifest runner; architecture linter 'fixtures can't be prod deps' extends tests/architecture/capability-seams.spec.ts.
**备注 · 风险(risk)**:Per-PR gate must stay keyless (scripted lane + property/fault runs); real-model statistical lane is nightly with keys; Python benchmarks need docker + python env in nightly.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Evaluation plane, chaos/security/scale gates, champion–challenger` · 层 L6_QUALIFICATION · 判决 **QUALIFICATION_REUSE + PROVIDER_WRITE** · 可省代码 35% · 社区插件最高覆盖 oh-my-knowledge 18★ 覆盖≈40% · 状态 **NOT_RUN (W17)**
...
**用什么 · 标准(绑定词汇)**:OpenFeature as the champion/challenger flag standard (only if the routing chapter also adopts it) (所有者 P0-05,import 其定义) · DSSE-signed EvolutionProposal (所有者 SLICE-3.4:R1:envelope slice 在 P4-04(W9)前落地)
...
**备注 · 风险(risk)**:promptfoo pulls a large dependency tree — dev/CI-only or npx, never runtime; OpenFeature worth it only if the routing chapter also uses it — cross-chapter decision.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Protocol version negotiation + capability discovery` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE + REUSE_UPSTREAM** · 可省代码 10% · 社区插件最高覆盖 dsh-multica-runtime 60★ 覆盖≈20% · 状态 **ACCEPTED (W5)**
...
`First-class remote resources (Run/Agent/Action/Approval/Artifact/Verification/World)` · 层 L5_SURFACE · 判决 **REUSE_UPSTREAM + CONTRACT_WRITE** · 可省代码 35% · 社区插件最高覆盖 sandbase-harness 641★ 覆盖≈10% · 状态 **NOT_RUN (W16)** · ◐ 上游已部分实现
...
`Remote lifecycle control (pause/resume/cancel/fork/retry/reconcile/close) w/ idempotency + version check` · 层 L5_SURFACE · 判决 **REUSE_UPSTREAM + CONTRACT_WRITE** · 可省代码 0% · 社区插件最高覆盖 dsh-multica-runtime 60★ 覆盖≈15% · 状态 **NOT_RUN (W17)** · ◐ 上游已部分实现
...
`Bidirectional server→client requests (durable approval / clarification / takeover / quorum)` · 层 L5_SURFACE · 判决 **CONTRACT_WRITE + REUSE_UPSTREAM** · 可省代码 0% · 社区插件最高覆盖 dsh-im-gateway 45★ 覆盖≈25% · 状态 **NOT_RUN (W10)**
...
**用什么 · 标准(绑定词汇)**:MCP elicitation/create server→client request shape (所有者 P2-12,import 其定义) · ACP session/request_permission payload shape (所有者 P2-06,import 其定义) · CloudEvents-style id/source/subject on the request envelope (所有者 P4-06,import 其定义)
...
**备注 · 风险(risk)**:Do not model this as MCP itself (dsh is not an MCP server to its own clients); borrow shapes so third-party MCP/ACP clients map 1:1.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Resumable event streaming (cursor/ack/replay/dedupe/backpressure)` · 层 L5_SURFACE · 判决 **REUSE_UPSTREAM + CONTRACT_WRITE** · 可省代码 5-10% · 社区插件最高覆盖 dsh-remote-web-ui 6,717★ 覆盖≈0% · 状态 **NOT_RUN (W17)** · ◐ 上游已部分实现
...
`Authenticated principal, tenant boundary, RBAC/ABAC, API scope` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT + REUSE_UPSTREAM** · 可省代码 30-40% · 社区插件最高覆盖 dsh-passwords 40★ 覆盖≈25% · 状态 **NOT_RUN (W17)**
...
`Schema-generated TS/Python SDK parity + contract test matrix` · 层 L5_SURFACE · 判决 **PROVIDER_ADAPT + QUALIFICATION_REUSE** · 可省代码 40% · 社区插件最高覆盖 — · 状态 **NOT_RUN (W18)** · ◐ 上游已部分实现
...
`Operator control-plane API + Run/Agent/Workflow visualization` · 层 L5_SURFACE · 判决 **CONSUMER_WRITE + PROVIDER_ADAPT** · 可省代码 25% · 社区插件最高覆盖 iPolloWork 5,273★ 覆盖≈35% · 状态 **NOT_RUN (W18)**
...
`Org governance: policy hierarchy, quota, retention, legal hold, audit export` · 层 L2_PROVIDER · 判决 **CONTRACT_WRITE + PROVIDER_ADAPT** · 可省代码 15-20% · 社区插件最高覆盖 tingly-box 345★ 覆盖≈15% · 状态 **NOT_RUN (W18)**
...
`Config provenance, typed dry-run, migration/rollback, ABI compat, DR release gate` · 层 L6_QUALIFICATION · 判决 **REUSE_UPSTREAM + QUALIFICATION_REUSE** · 可省代码 25% · 社区插件最高覆盖 dsh-undo-savepoint 142★ 覆盖≈15% · 状态 **NOT_RUN (W19)** · ◐ 上游已部分实现
...
`LLM provider conformance kit` · 层 L6_QUALIFICATION · 判决 **PROVIDER_WRITE + REUSE_UPSTREAM** · 可省代码 25-35% · 社区插件最高覆盖 dsh-agy 28★ 覆盖≈15% · 状态 **P9 C:VERIFIED,P:VERIFIED,U:VERIFIED,F:VERIFIED**
...
**备注 · 还得自写(residual)**:Six behaviour cases, retry-classification map to llm-retry, usage/cacheRead/cacheWrite accounting, over-long-input semantics, per-route fixtures (≥40 cases); extend llm-mock-server beyond /chat/completions.
**备注 · 风险(risk)**:Do not re-test pi-ai wire parsing (upstream twin is a settled seam); test dsh's observable LlmAdapter seam behaviour.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Activate pi-ai multi-protocol routes ("any API")` · 层 L2_PROVIDER · 判决 **REUSE_UPSTREAM + CONSUMER_WRITE** · 可省代码 50% · 社区插件最高覆盖 dsh-plugin-subscriptions 312★ 覆盖≈25% · 状态 **P9 C:VERIFIED,P:VERIFIED,U:VERIFIED,F:VERIFIED**
...
**备注 · 还得自写(residual)**:Route schema fail-closed validation (schemastery), bundle templates, route-activation.spec.ts, docs, 5 negative configs, mock-service headless E2E; dormancy is pure config in cordis.patch.yml.
**备注 · 风险(risk)**:models.dev is community-maintained data — pin a snapshot and verify baseURLs in the conformance kit; per-vendor quirks are P5-03's job.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Provider/model selection surface (--model)` · 层 L3_CONSUMER · 判决 **CONSUMER_WRITE** · 可省代码 0% · 社区插件最高覆盖 dsh-multica-runtime 60★ 覆盖≈25% · 状态 **P9 C:VERIFIED,P:VERIFIED,U:VERIFIED,F:VERIFIED**
...
`Edit tool tolerant fallback (exact → whitespace → indent)` · 层 L2_PROVIDER · 判决 **PROVIDER_WRITE** · 可省代码 0% · 社区插件最高覆盖 dsh-better-edit 21★ 覆盖≈0% · 状态 **P9 C:VERIFIED,P:VERIFIED,U:VERIFIED,F:VERIFIED**
...
**备注 · 还得自写(residual)**:Port only 3 levels (exact → line-trim → indent-shift) into fs-local/src/fsio.ts applyLiteralEdit, keep FS_AMBIGUOUS_EDIT at every level, annotate diff with level, add line+similarity hint.
**备注 · 风险(risk)**:Do not import opencode (an app, not a library); do not add AST/fuzzy semantic matching.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`token-meter real tokenizer (replace 4-chars/token)` · 层 L2_PROVIDER · 判决 **PROVIDER_ADAPT** · 可省代码 60-70% · 社区插件最高覆盖 dsh-context 1,238★ 覆盖≈0% · 状态 **P9 C:VERIFIED,P:VERIFIED,U:SCHEDULED_BLOCKED,F:SCHEDULED_BLOCKED**
...
**备注 · 还得自写(residual)**:usage-calibration.ts closed loop from real usage, lazy preload so the sync pure-function contract holds, 4-corpus accuracy fixtures, compaction non-regression.
**备注 · 风险(risk)**:tokenizers.js is 0.1.x/55★ (young, official HF port) — keep behind the token-meter seam; bundle defaults are deepseek-v4-* while the public tokenizer is V3's — BLOCKED if vocabularies differ.
**备注 · 计划错误(planError)**:Must confirm V4 tokenizer parity at implementation (public asset is V3).
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`Headless scripting: --resume / --output-format json|stream-json / stdin` · 层 L3_CONSUMER · 判决 **CONSUMER_WRITE** · 可省代码 0% · 社区插件最高覆盖 dsh-multica-runtime 60★ 覆盖≈40% · 状态 **P9 C:VERIFIED,P:VERIFIED,U:VERIFIED,F:VERIFIED**
...
`Agent-loop hard budgets (maxTurns / cost cap)` · 层 L3_CONSUMER · 判决 **CONSUMER_WRITE + CONTRACT_WRITE** · 可省代码 0% · 社区插件最高覆盖 dsh-quota-meter 16★ 覆盖≈30% · 状态 **P9 C:VERIFIED,P:VERIFIED,U:VERIFIED,F:VERIFIED**
...
`Task success-rate benchmark v1 (20–50 auto-judged tasks)` · 层 L6_QUALIFICATION · 判决 **QUALIFICATION_REUSE + PROVIDER_WRITE** · 可省代码 70-80% · 社区插件最高覆盖 dsh-eval-harness 14★ 覆盖≈30% · 状态 **P9 C:PREMATURE,P:PREMATURE,U:PREMATURE,F:PREMATURE**
...
**备注 · 还得自写(residual)**:dsh.py installed agent (~100 lines Python), anti-cheat self-built tasks (judge invisible/immutable), run.mjs/report.mjs aggregating success/cost/variance (Wilson), nightly workflow with explicit BLOCKED when keyless.
**备注 · 风险(risk)**:Python + Docker sidecar (macOS+Linux OK); keyless CI → BLOCKED by design.
**备注 · 计划错误(planError)**:计划引用的 examples/headless-agent/tests/coding-task.e2e.ts 路径已失效——文件实际在 apps/cli/tests/profiles/headless/tests/coding-task.e2e.ts（本会话核实）；registry 的 files[] 要改路径而不是当作缺失重建。
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
`System-prompt guidelines v1 + champion–challenger A/B` · 层 L1_CONTRACT · 判决 **CONTRACT_WRITE + QUALIFICATION_REUSE** · 可省代码 30-40% · 社区插件最高覆盖 oh-my-knowledge 18★ 覆盖≈44% · 状态 **P9 C:PREMATURE,P:PREMATURE,U:PREMATURE,F:PREMATURE**
...
**备注 · 还得自写(residual)**:Guidelines v1 text in packages/preset/persona (currently persona: ''), variant declaration format, ab-compare.mjs significance + promotion record into the Evidence Package, ab-gate.spec.ts negatives; 0% of content deleted.
**备注 · 风险(risk)**:promptfoo's runner overlaps P9-08's runner — if P9-08 does not use promptfoo, write ab-compare over P9-08 reports instead.
**备注 · 计划错误(planError)**:promptfoo has no significance testing — the Wilson gate is dsh code.
**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:
...
for r in rows:
    ...
    topc = max(r['community'], key=lambda c: c.get('coverage') or 0) if r['community'] else None
    topc_s = (f'{topc["name"]}' + (f' {topc["stars"]:,}★' if topc.get('stars') else '') + (f' 覆盖≈{topc["coverage"]}%' if topc.get('coverage') is not None else '')) if topc else '—'
    w(f'`{r["titleEn"]}` · 层 {r["layer"]} · 判决 **{v}** · 可省代码 {pct} · 社区插件最高覆盖 {topc_s} · 状态 **{status_of(eid)}**' + (' · ◐ 上游已部分实现' if r.get('upstreamPartial') else ''))
    w('')
    e = REG.get(eid, {})
    ...
    if adapt:
        ...
        w('**用什么(OSS / 标准)**:— 不在账本内,开工时补判' if r.get('_stub') else '**用什么(OSS / 标准)**:— 无合适 OSS(账本找过;主体自写)')
    if opt:
        w('**用什么 · 可选(optional,不进依赖不进 CI)**:')
        for o in opt: w(f'- {o["name"]}' + (f' — {esc(o["note"])}' if o.get('note') else ''))
    if rej:
        w('**备注 · 看过但不用(reject)**:')
        for o in rej: w(f'- {o["name"]}' + (f' — {esc(o["note"])}' if o.get('note') else ''))
    if ref:
        w('**备注 · 看过但不用(reference,只读设计)**:')
    ...
    if r['standards']:
        ...
        w('**用什么 · 标准(绑定词汇)**:' + ' · '.join(owned))
    w(f'**备注 · 还得自写(residual)**:{esc(r["residual"])}')
    w(f'**备注 · 风险(risk)**:{esc(r["risk"])}')
    if r['planError']:
        w(f'**备注 · 计划错误(planError)**:{esc(r["planError"])}')
    if r['community']:
        cs = sorted(r['community'], key=lambda c: -(c.get('coverage') or 0))
        w('**备注 · 社区插件(全列,不采用;缺口 = 我们的必备项,形态 = 要接住的 hook,§9.2)**:')
        for c in cs:
