...
| Any future re-anchor's `kind=B` file references are re-verified to exist on the new baseline | `pnpm run first100:verify-baseline-file-references -- --baseline-sha <sha>` (`git cat-file -e` per declared `kind=B` path against the given baseline; a `kind=N`-already-exists companion check reports the PARTIAL-rescope signal) | Manually, at any future baseline re-anchor (not wired into the unconditional per-slice gate — the CURRENT frozen baseline `0a53fb55` already has 9 known-stale `kind=B` references per BLOCKED-033, so an unconditional wire-in would fail every slice today; BASE-ALIGN-v2 runs it explicitly against the candidate baseline before adoption) | Mechanized (BLOCKED-033/037, 2026-09-02) — bidirectionally verified: run against `0a53fb55` independently reproduces BLOCKED-033's exact manual finding (8 epics, 9 files) byte-for-byte, plus surfaces one previously-undocumented `kind=N`-already-exists signal (P5-10) as a bonus of the companion check; a scratch-registry mutation (fake missing `kind=B` path) is caught and the revert confirmed clean. Run against the candidate baseline `4e84901e`: 9 epics/11 files missing `kind=B` -- exactly BLOCKED-033's 8-epic/9-file pre-existing set plus P3-07's 2 genuinely-new files, confirming BLOCKED-033's own P3-07-vs-the-rest classification. |
