...
**One further narrowing of what `U.1` proves, closed by `U.2`.** `U.1`'s Loader config (`tests/first100/fixtures/loader/p4-02-task-profile/cordis.yml`) mounts `@deepseek-ai/dsh-run` directly, in a nine-row test-only composition. So it closes "a real `app-boot` Loader boot reaches the compile", which is the gap `§4.4` named, and it does **not** close "the row at `bundle/base/cordis.patch.yml:638` is what reaches it".

**`U.2` closes that one** (2026-09-12, BLOCKED-232 (e)). It boots the shipped `headless` profile through `bootProductionProfile`, whose bundle list comes from the real `PROFILE_TEMPLATES` and carries `@deepseek-ai/dsh-base`, under an overlay that mounts no Run Service of its own. The mount row is now evidence by observation: `disabled: true` on it reddens both `U.2` cases and leaves `U.1`'s four green.
...
- **(e) No frozen case observes the shipped `bundle/base` row reaching the compile. CLOSED 2026-09-12 by `P4-02.U.2`**, two cases over the shipped `headless` profile's own bundle layers. The mutation that proves it is `disabled: true` on that row, which `U.1` cannot see.
...
- **(g) This page's row count for `sdk-minimal` is wrong; what it was cited for is not — corrected 2026-09-19.** The §4.4 production-reach paragraph says `packages/bundle/sdk-minimal/cordis.patch.yml` "declares 33 plugin rows". It declares **31**, counted three ways that agree (`^    - id: `, `- id:`, `^      name: `), both at `f3c46115daa0b804137b5740c5a2ba05bc3c117c` and at this page's own baseline `3672337016dcc7e4223c1b85591c281415814c22` — the file never held 33 on either tree, so this is a miscount and not a change underneath the page. The claim the count was cited for re-measures unchanged: `@deepseek-ai/dsh-run` appears **0** times in that file, against one mounted `@deepseek-ai/dsh-tools` row as the positive control, so the paragraph's conclusion — this epic's code does not run on `sdk-minimal` — stands. The original sentence is left as written; this page records corrections rather than editing its measurements.

## DEFERRED-TO records (BLOCKED-232 closing condition 1, written 2026-09-19)

Three of this epic's clauses have zero production readers, measured and recorded in §4.4c and in [BLOCKED-232](BLOCKED-QUEUE.md#blocked-232)'s ownership table. This section is the form BLOCKED-215 established, and it exists so a later reader of THIS page finds the disposition here rather than inferring a reader that does not exist: each clause is **discharged on the producing side**, and its consuming side is **named with the epic that owns it**.

A deferral is not an exemption. It says the clause's producing half is built and observed, its consuming half belongs to a named epic, and P4-02 is signable on the producing half alone — not that the clause is satisfied end to end.

| P4-02 clause | discharged here (producing side) | deferred to | why that epic and not this one |
| --- | --- | --- | --- |
| **must[2]** — 「高风险或歧义字段缺失时产生 question,不擅自猜授权。」 | `questions[]` is compiled and appended to the session log, durably, on the one production path (`recordTaskProfile`). The clause's own verb is 产生 — *produce* — and a question is produced and survives the process. | **P2-12** (Human Interaction Channel) | `ask question` is one of P2-12 must[0]'s five channel verbs, and its must[3] (「Question 与 approval 分离,回答只作为输入,不自动授予权限」) is the same non-granting property this clause states from the producing end. Asking a question is a channel, and P4-02 owns no channel. P2-12's ledger row is `NOT_RUN`. |
| **acceptance[1]** — 「所有 hard constraint 可从 profile 追溯原始来源。」 | Every hard constraint carries `origin` and `confidence` in the appended body, so the traceability exists in the durable record. | **P4-03** (second reader **P5-01**) | P4-03's acceptance[1] is 「Plan 中每个 node 能追溯 TaskProfile requirement」 — literally this traceability consumed. P5-01's must[0] takes a TaskProfile as input. Both are `NOT_RUN`; P4-03 is wave 8 with P4-02 as a declared predecessor. |
| **acceptance[2]** — 「未知 side effect 不会被标为 none。」 | An unclassifiable side effect is recorded as `unknown-default` at confidence 0, never as `none`, in the appended body. | **P4-03** (second reader **P5-01**) | P4-03's must[1] is compile-time capability/policy/budget satisfiability, which is exactly what a wrongly-`none` class would corrupt. Nothing else in the registry reads a side-effect class. |

**Two candidate owners the delegate named do not hold up, recorded so the question is not reopened by inference.** Neither **P4-07** (lease, heartbeat, fencing) nor **P4-08** (workflow journal) names a TaskProfile in any must, acceptance or validation clause. P4-08's must[0] records 「input/output artifact refs」, which a `TaskProfileRef` could be — but 「could be」 is the inference §12.78 forbids as a basis for reopening ownership.

**What this does NOT claim.** Not that the three clauses are satisfied: each is satisfied on its producing half only, and the register carries the consuming half against the named epic. Not that the deferral shortens anyone's work — P4-03 and P5-01 inherit these clauses' reading halves as part of their own acceptance. Not that `validateTaskProfile`'s own gap is deferred: it was WIRED (`packages/run/run/src/index.ts:158`, with `taskProfileRef` at `:163`), which is closing condition 2 and not a deferral.
