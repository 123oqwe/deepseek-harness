...
## Forward addendum (2026-09-19): the stop now reaches a dispatch, and what it still does not reach

[BLOCKED-265](BLOCKED-QUEUE.md#blocked-265) opened on a PTC-shaped question and the measurement found something wider, which belongs on this page because it is about this epic's own clause.

**`stopGateFor` produced `'stopped'` and nothing read it.** Measured across the tree on 2026-09-19: `advanceLeasedAgent` returns it (`packages/core/agent/src/dispatch.ts:312`, produced at `:320`), and every caller dropped it — `agent-loop`'s dispatch short-circuits only on `'fenced'` and `'lease-refused'`, `core/tools`' approval transitions discard the return outright, and `dsh-run`'s own wrapper hands it to four call sites that ignore it. So the only place a stop refused anything was the lease acquisition for a NEW Run, which is the half `acceptance[0]`'s two P cases already pin. A stop raised while a run was working stopped nothing it was already doing, on either dispatch path.

**What landed.** One gate, `refuseNewAction`, asked per dispatch by both the native loop and the PTC sub-dispatcher, before any ledger reservation, reusing `stopGateFor` and the lease package's own `checkFencing` rather than adding a second table. A refused call gets the same model-visible sentence on both paths; a refused PTC sub-call additionally ends the program through the run controller it already owns.

**What it still does not reach, and neither does anything else.** An action ALREADY EXECUTING when the stop arrives is not stopped: this gate refuses what has not started. Closing that needs the executor's response to its own abort signal, which is a separate measurement and is not claimed here. `acceptance[0]`'s second half — that nothing in flight produces a further external effect — is therefore still open, and the cases added with this gate do not close it.

**Reach.** PTC is not the default presentation mode (`packages/core/tools/src/index.ts:1047` defaults `mode` to `'native'`, and no shipped bundle sets `ptc`), so the PTC half of this was not reachable on a shipped profile. It is one validated `Config` field away, which is why it was fixed rather than recorded as harmless.
