...
| 2.12 | **用例的两个读数若必然相同,它什么都没断言**;揭穿它的只有一个本该变红却没变红的变异——所以每条冻结用例的 `sensitivityProof` 里至少一个变异必须精确红它,变异存活先查用例再查代码(2026-09-10 P4-11:父子预算用例"记谁都 3 次"、M46 默认 `status=503` 把 503 测两遍,两次都是变异不红才暴露) | 整改令 §12.76;执行者冻结 note |
