...
**Item 3 — CLOSED at model-step admission, not at tool dispatch.** I first reported it as not fitting, which was correct about tool dispatch and too quick about the rest: `tool-calls.ts:95` advances the run to `waiting_tool` — itself in `NON_CONSUMING_STATES` — immediately BEFORE dispatching, so a guard there would refuse every tool call in the harness. §12.60 named the right seam instead: the `waiting → running` transition point. `RunPlugin`'s `agent/pre-step` listener now returns `{ kind: 'reject' }` when the run holds no dispatch slot, which is the deliberate short-circuit the waterfall contract describes — refusing IS an opinion about the step, so `next()` is not called.

`queued` is excluded, and the exclusion is load-bearing rather than cosmetic. It is in `NON_CONSUMING_STATES` because a queued run has NOT STARTED, not because it released a slot, and it is also where a fenced host remains when `ensureRunning`'s `queued → starting` is refused. The first implementation without this exclusion broke P4-07's frozen behavior — its own case caught it — by rejecting the step for a fenced host, replacing the model-visible "your calls did not run" refusals with silence. P4-07 owns that case and answers it downstream.

`holdsDispatchSlot` now has its production caller. The failure it prevents is concrete: an orphaned run whose host kept stepping would spend LLM calls on work another host owns, and write results against a lifecycle it no longer has authority over. The case counts adapter requests through a proxy and asserts zero; the mutation that removes the check spends one.
