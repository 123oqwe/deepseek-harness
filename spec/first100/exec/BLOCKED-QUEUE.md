...
**What the corpus shows that gate doing:** 145 `action/risk-gated` events, **all `allowed-by-preset`**, across risk classes `internal-write` (83), `read` (28), `security-sensitive` (25), `local-reversible` (7) and `external-communication` (2). No `safety-critical` is ever produced, which is why `hardDenied` has never fired — the band exists and nothing observed lands in it. The `security-sensitive` events all occur under `danger-full-access`, whose threshold is `safety-critical`; under `workspace-write` and `read-only` only classes below their `destructive` threshold appear, so no approval was skipped.
