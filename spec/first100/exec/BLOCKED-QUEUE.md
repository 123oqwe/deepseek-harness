...
**RULED: (B), plus a layer I had not proposed.** The delegate accepted keeping the iterative implementation and added `canonicalize@2.1.0` as a **devDependency used as a differential oracle** — a property case asserting byte-identical agreement with the reference over generated JSON. The reasoning, which is better than my own: "four properties hold" proves four properties, not RFC 8785 conformance. JCS also fixes UTF-16 code unit key ordering, ES6 number formatting, `-0` rendering and JSON escape spelling, and a hand-written implementation claiming conformance has to be tested against the standard rather than against a list someone thought to name.

**The oracle earned its place on the first run.** Mutating the key sort to `localeCompare` — which still sorts — passes every one of the four named properties and fails ONLY the differential case. Without the oracle that mutation would have survived, and the implementation would have been non-conformant while every named property was green.

**(A) was declined for a reason worth keeping:** a depth ceiling decides what input the product accepts, and that subject belongs to P3-12 (malicious-input boundaries). Deciding it inside P2-03 would have been one epic settling another's clause. A pointer is recorded on P3-12 instead.
...
- **Resolution (delegate ruling, narrow):** the delegate executes exactly one command family — `generate-ledger.mjs --record-signoff` plus the commit/push that lands `delegate-signoff.json`. Nothing else changes: `--accept`, tests, scripts, gates, and all product commits stay Supervisor-run. The delegate ran this for P1-08/P1-09/P4-01 today (commit `9e4f05a00b`) before recording this entry, and states that plainly rather than back-dating the authorization.
- **Open to the user, not blocking:** this narrows a rule the user set. The user may prefer the opposite trade (keep §1 absolute, and accept that predicate (iv) is an honor-system record). Until they say otherwise the carve-out stands, because the alternative is a gate that reads as mechanical and is not.
- **Follow-up for the Supervisor:** P2-01's existing entry was Supervisor-written under the old arrangement. It is not being revoked — the underlying verification did happen — but it should be re-signed by the delegate at the next occasion its row digest is recomputed, so every entry in the file has one provenance.
...
- **Residual on P6-01 itself (delegate condition):** the Supervisor correctly chose Option A (fix the two in-memory providers to store and filter by scope like `durable-file`) and correctly refused Option B (seam-level filtering), because `MemoryRecordView` is `{id, principal, content, updatedAt}` — it carries no tenant, so the seam cannot tell which record is out of scope, and adding the field is a Contract-face change requiring supersession and belonging to P6-02. Changing the C face during F is exactly what the F ruling excludes. **But the residual — a hostile provider can still return out-of-scope records — must not live only as a `CHARACTERIZATION:` case in the test file.** It gets a register entry with a named owner (C supersession or P6-02) and an unlock signal, because a residual recorded only in code is a residual that disappears at the next refactor.
- **Also noted, not fixed:** `contextBudget.maxTokens` is declared and read by nothing repo-wide (3 × 5KB records pass through `maxTokens: 1` untouched, reporting `truncated: false`). Pinned by a case, deliberately unfixed — enforcing it needs a token-estimation strategy beyond what its own fault case proves. **This is the sixth declared-but-unproduced field this program has hit**, which makes the frequency itself the finding: it is a fact about how these type declarations were written, not six unrelated field bugs. Tracked here so the seventh lands on a count, not on a shrug.
...
**First whole-set observation of this rule's chains:** GitHub gate ③ run `34563699840` at `17bdee8fdf`, which carries `c1b872722f`. Before it, every `--check` in the table above had only ever been run one at a time by a lane, which is how instance 3 below reached a pinned commit.

**Instance 3, 2026-09-11, same rule and same author as instance 2.** `cd521f6421` added `@deepseek-ai/dsh-workspace-trust` to `packages/run/task-profile`'s manifest for a `TrustState` import — a new workspace import edge — without running `gen-module-graph`. `verify-module-graph` was therefore red at `9a820c4016`, the commit the delegate had pinned. **It was invisible from the lane's working tree**, which had already regenerated the graph for later work; it surfaced only when the §5.1.8 self-check was run in a detached worktree AT the reported SHA. That is now a standing rule of its own: the self-check runs in a tree checked out at the SHA being reported, and the report says which tree it ran in. Fixed in `c1b872722f` with generated files taken from that detached worktree, so the graph describes the pinned tree exactly.
...
- **When:** 2026-09-10, lane B, while writing P4-02's Provider-stage preFlight. P4-02's ruling put its compiler on the Run's `accepted → planning` transition; measuring where that transition happens is what found this.
- **Measured, not inferred** (all greps over `packages` and `apps`, `--include='*.ts'`, excluding `/tests/`, `*.spec.ts` and `lib/`):
  - **The service has no consumer at all.** `RunPlugin` registers itself as `'runs'` (`index.ts:567`). Outside `packages/run/run`, there is no `ctx.runs`, no `ctx.get('runs')`, and no plugin whose `inject` list names it. The single hit in the whole tree is `packages/extensions/tool-cordis/src/api-catalog.ts:1561`, a generated documentation artifact that lists the service key; it is not a call. Everything below follows from that one fact: `advance`, `listNonTerminal`, `resume`, `runsForSession`, `openForSession`, `reclaim` and `runFor` are unreachable from outside the package, so the only production behaviour is what `RunPlugin`'s own listeners do.
  - **The method-name grep is the wrong instrument here, and says so loudly.** `grep -rn "\.resume(" ` outside this package returns 22 production hits and `.accept(` returns 8 — every one of them a same-named method on something else (`agents.resume`, a Node stream's `resume`). Service reachability is the measurement that decides this; a name count would have reported the opposite answer.
  - What writes the Run event log in production is `createRun` alone (`packages/run/run/src/index.ts:332`, `:359`), which mints the genesis entry at `accepted`. **Nothing ever appends a second entry.**
  - What actually advances in production is `Agent.lifecycle`, which is **P4-05's `AgentLifecycle` vocabulary** — `queued`, `starting`, `running`, `waiting_tool`, `cancelling`, `completed`, `failed` — driven from `RunPlugin.ensureRunning` on `agent/pre-step` (`index.ts:795-814`). `RunState`'s `planning`, `waiting`, `verifying` and `reconciling` are a second, parallel vocabulary that no production path enters.
- **Clause by clause, which half of P4-01 arrived:**
  - **must[1] (append-only event log) — arrived, partially.** The log exists, is persisted (`store.loadAll` / `store.put`), and every Run carries its genesis entry. What has never happened in production is an append after genesis.
  - **must[2] (service-owned Run identity) — arrived.** `RunPlugin` holds the Run and is its sole owner; `Agent.runId`, `Agent.lifecycle` and `Agent.runLease` are written from that one place.
  - **acceptance[0] ("list non-terminal Runs after a restart and resume them") — library only.** `listNonTerminal()` (`index.ts:429`) and `resume()` (`index.ts:440`) are exercised by `packages/run/run/tests/state-machine.spec.ts` and by nothing else. No startup path enumerates non-terminal Runs.
  - **acceptance[1] ("an illegal transition is refused") — library only.** `LEGAL_RUN_TRANSITIONS` and `transition`'s refusal are real and tested, but the only caller is the test suite: a refusal nothing can trigger in production refuses nothing.
- **Why this is filed as its own item rather than absorbed by P4-02.** P4-02 noticed it because its own mount point depended on the answer, but the defect is P4-01's, and P4-01 is **ACCEPTED**. Letting the epic that happens to trip over a predecessor's gap quietly repair it inside its own stage is how a clause ends up owned by nobody — and it would also hide the fourth instance of the pattern this program has now seen in P2-02 (§12.69), P6-07 (§12.75) and P1-10 (§12.78). This one is the first caught before any code was written for it.
- **Resolution (delegate, gq-92, 2026-09-10):** P4-01 is **de-signed at the next push**, on the same standard applied to P6-07 — four green cells over clauses whose subjects have no production caller. The remediation slice **P4-01.U2** is assigned to lane B, immediately after P4-02, in the same package: the agent loop drives `planning → running → succeeded | failed | cancelled`; startup calls `list()` for non-terminal Runs and takes `resume()`'s decision; frozen and observed on a shipped profile, then re-signed. P4-02's own `accepted → planning` transition stays with P4-02 — compiling the profile IS the beginning of planning — and it gives the Run event log its first real appender; `running`, the terminal states and restart recovery are P4-01.U2's, not P4-02's.
- **Status:** ANSWERED-BY-DELEGATE (gq-92) — de-sign and remediation slice both decided; the slice is queued, not yet started.

### BLOCKED-184 — STANDING RULE: incremental compile state outlives a reverted type; when typecheck reports a symbol that is not on disk, delete `tsconfig.*.tsbuildinfo` before believing it

**2026-09-10, P4-02.C.** A `declare module '@deepseek-ai/dsh-llm/message'` written inside a test file — to construct a message kind this build has never seen — changed the union for the whole repository. `pnpm run typecheck` reddened **eight unrelated `apps/cli` fixtures**, each error naming the invented kind. **After the source was reverted, typecheck still reported 174 errors while `grep -rl` found the symbol nowhere on disk**: the union survived in `tsconfig.host.tsbuildinfo`. Deleting that file took typecheck back to zero.

**The rule:** when an error names a symbol you cannot grep in the working tree, **suspect the incremental state before you suspect your own reading**. This is the same rule as "evidence must come from the thing actually examined" — a gate run from `/private/tmp` reporting `exit=1` was read as "the base is red" when it meant "no package.json here" — and a stale `tsbuildinfo` is that failure in its most convincing form: `tsc` reports faithfully about **a tree that no longer exists**.

**The companion instrument rule, filed here rather than under its own number (delegate, gq-92, 2026-09-10), and binding on §4.4a's "name the call sites and count them": measure SERVICE REACHABILITY before you measure method names.** Ask first whether anything outside the owning package can reach the service at all — `ctx.<key>`, `ctx.get('<key>')`, an `inject` list naming it, a direct property access — and only then count calls. Same-named methods make a name count lie, and it lies in the dangerous direction: BLOCKED-183 was measured both ways, and `.resume(` returned **22 production hits outside the owning package, every one of them something else** (`agents.resume`, a Node stream's `resume`), while the service itself had **zero** consumers. A reviewer who stopped at the method count would have signed off on "it has callers."

**The design conclusion that comes with it: do not augment a merge-extensible union from a test file.** Declaration merging is global, so a fixture's "local" extension is a repository-wide one. To exercise a `default` branch, use a member the union really has and the function under test deliberately does not name — P4-02's origin classifier uses `tool` — rather than inventing a kind. A cast is not the alternative either: `as unknown as X` is what BLOCKED-126 forbids, and a boundary that takes `unknown` (as `validateTaskProfile` does) needs no cast at all.
...
**ANSWERED-BY-USER via delegate (gq-92), 2026-09-10 12:55 EDT — reading 1: memory ON by default.** The user was asked in product terms (「跨会话记忆默认开还是留 opt-in」, with the recommendation that every comparable product — Codex, Claude Code, OpenClaw 2.0 — defaults to memory and the user's own product direction, AI employees, needs it) and answered 「我同意你去开了」. Resolution: the base bundle's `memory` row is enabled with `durableFileDirectory: !!js dshHomePath('memory')` (the runtime self-registers `durable-file` when that key is set, so `MEMORY_PROVIDER_UNAVAILABLE` no longer applies) and the `memory-context` row is enabled with it; must[3]'s four-dimension scoping is what bounds every read a user performs, and the P6-01.U observation must now be made on the SHIPPED profile (Loader boot of the base bundle as shipped, not a fixture that enables the rows itself). Two invariants travel with the switch: (a) an EMPTY memory produces a byte-identical model input, so existing keyless snapshots do not churn and the change is provably additive; (b) an untrusted workspace (P1-07) recalls nothing from any other workspace. BLOCKED-155's reading 1 (grow the seam's stored record to P6-02's `MemoryRecord`) becomes reached by the same switch and is scheduled with it. Journal §12.79.
...
- **Update — v2.2 stage-merge activation for W4's remaining stages (2026-09-03, ANSWERED-BY-DELEGATE(gq-92), with two corrections the delegate made to the Supervisor's own proposal):** v2's Tier-B stage-merge authorization had sat unused — all 11 W4 epics ran C-stage as its own slice. Activated for W4's remaining stages, with these rules:
  1. **Split criteria are stage-scoped, not universal (delegate's own correction to their stated criterion).** "Crosses ≥2 subsystems → must split" applies to **C and P stages only**. It is *inherently* true at U-stage — Consumer/Composition work is by definition wiring a capability into several existing packages — so applying it there mechanically rejects every possible merge (verified: all 11 W4 epics' U stages span ≥2 subsystems). **U-stage uses the file-count threshold instead** (>6 files → split).
  2. **Approved merge set: U+F for 6 epics** — P0-04, P1-02, P1-07, P1-08, P1-09, P6-01. **The correct justification (replacing the Supervisor's own invalid one):** the Supervisor argued these were safe because `F ⊆ C` files meant "zero added surface." That is the wrong set relation — the merge is U+F, so the load-bearing question is `F ⊆ U`, which the delegate measured as **false for all 6** (each merge genuinely adds +1 file). The valid argument: **merged surface is 3–5 files, all under the >6 split threshold, and the added file is the epic's own C-stage test file — returning to already-owned ground to add negative cases, not claiming new territory.** Weaker premise, stronger conclusion: verifiable rather than merely asserted.
  3. **Merging delivery never merges scrutiny.** For the two Tier-S epics in that set (P1-02, P1-07), F-stage's negative cases must be **frozen separately and reviewed separately at full Tier-S spec** — not folded into the U-stage review round. What merges is the number of CI round-trips, not the number of independent eyes.
  4. Every other quality gate is unchanged: independent review, the three greening predicates, real CI observation, and the delegate's own per-case audit.
...
**Its visibility was zero, and that is the part worth keeping.** Under the old rule `check-ready` printed `0 epic(s) ready to start` — **a legal output, identical to the one a genuinely saturated program produces.** "Nothing can start because a bug froze the gate" and "nothing can start because everything is already being worked on" are the same string. The program had entered a state where no epic could begin, and nothing would have said so.

By the three visibility tiers recorded above, this is **tier 3**: no symptom, nothing to count, exposed only when someone happens to want a new epic. What exposed it was the pre-flight subject-existence check adopted after P0-08.U — **a check added for an unrelated purpose**, which is the only reason the state surfaced when it did rather than at "why has there been no progress?".

**The symptom itself was made self-describing (2026-09-05, delegate-authorized, scope fixed by the delegate to facts only).** `0 epic(s) ready to start` now prints the counts that separate its two meanings — in flight, blocked by predecessors, blocked by file overlap, awaiting acceptance — and never an interpretation. Whether a given state is expected is the reader's judgment, and the tool stating it would be the tool guessing.

This is not a new capability but an existing output explaining its own basis. The distinction is load-bearing: **the defect's only symptom was this line, and the line could not distinguish its own two meanings.** It surfaced solely because someone happened to want a new epic at that moment.

**Building it surfaced a fifth state nobody had listed.** The delegate specified three counts; the buckets then summed to 83 against 85 non-ACCEPTED epics. The missing two were P4-05 and P6-01 — finished, unaccepted, neither startable nor in flight. **A listing whose numbers do not reconcile cannot be trusted to be complete**, so the fifth bucket and a reconciliation line were added, and the line fails closed when the totals disagree.
...
**Originally deferred (superseded by the resolution above).** The gate was believed to be `scripts/first100/check-ready.mjs`, P0-04's Usage-stage deliverable, with **P0-04 ACCEPTED** — so changing it edits an accepted epic's deliverable, exactly the situation [BLOCKED-076](#blocked-076) had to rule on for `docs/architecture.md`. The fix is small and the criterion is mechanical (an epic whose every non-N/A cell is GREEN releases its file locks, since its writes have landed), but a Supervisor rewriting the readiness gate to unblock their own next epic is precisely the shape that needs a second party.
...
**The measurement refutes it.** Adding `orphaned → running` reddens **exactly one** case — the explicit "may NOT resume directly as running" prohibition. **Every stale-epoch assertion still passes.**

**Why, and why this is not the corollary the delegate offered.** `decideTransition` compares epochs on *every* proposal regardless of which edges exist, so the epoch check cannot depend on the edge set. The reasoning was **right about the system and wrong about the suite**: a run that resumed directly really would carry a dead worker's epoch forward, and that really would corrupt the staleness baseline — but nothing in either suite follows a run through orphan, reclaim, and back to assert the epoch advanced. **The prohibition is asserted; the property it exists to protect is not.**

So the epoch assertions are sound, and the coupling is real, and there is still a hole between them: the edge could be re-permitted by someone who reads the prohibition as stylistic, and only one case — the one that states the prohibition — would object. **A test that says "this edge is forbidden" carries none of the reasoning for why, and cannot fail for the reason that matters.**
...
**Two independent gaps, and they must not be collapsed.**

**(1) The log could not say why.** `first100-exact-sha.yml` ran `vitest --reporter=json` as the *only* reporter. vitest exits non-zero for causes that never enter `assertionResults` — an unhandled rejection, a teardown throw, a worker crash — and with json alone such a run leaves no trace in either artifact: the report has no field for it, and nothing human-readable was attached to print it. The job's own comment shows fail-closed was designed on purpose; **diagnosability was not designed at all.** Fixed by adding `--reporter=default` alongside (additive; the JSON report keeps its format and path, so predicate (iii)'s input is untouched).

**(2) The greening tool has the same blind spot, and this half is NOT fixed.** `generate-ledger` derives its verdict from the report. Asked to green P0-08.U from that observation it **accepted without complaint**, because the report it reads shows zero failures. The Supervisor greened the cell, then reverted it on realising the tool's acceptance was the gap rather than an authorisation. **The same red would green again today.** The remedy is for greening to consult the job conclusion as well as the report — `gh run view <id> --json conclusion` suffices, and the run URL is already stored in every cell, so no new artifact field is needed. **Deferred, not done:** it changes predicate (iii)'s input channel and belongs under the evidence-mechanism rules, sequenced after the epics.

**Scope of the doubt, bounded by measurement rather than left open.** A blind spot invites suspicion of everything that passed through it. The delegate audited all **34** observations behind every green cell: **5** had `failure` conclusions, covering 8 cells across 6 epics, and **every one of the five was the same registered flake** (`experimental Inspector real Worker forwards Client Console objects…`), admitted by the tool exactly as the flake registry provides. **No accepted row rests on an unregistered failure.** The distinction that matters: in those five the report said `success: false` and listed the failure, so the tool saw it and adjudicated — it was working. Here the report says `success: true`, so **the tool has nothing to adjudicate**. Not leniency; invisibility. **The blind spot has been triggered exactly once, and that once was caught and reverted.**
...
**Ruling: widen, at class level, per-stage on demand.** Option (B) — declaring loop mounting out of scope for Usage — was rejected because it overturns the registry, which names `tool-calls.ts` in both P2-03's U and F. The reasoning is recorded once here; each affected stage adds one `deliverablePathPatches` entry citing this entry when it is dispatched, never in bulk ([BLOCKED-070](#blocked-070)'s shape). **21 epics declare an `agent-loop/src` file** — `tool-calls.ts` (P2-03, P2-05, P2-06, P3-03, P4-12, P7-02), `agent.ts` (P0-03, P4-05, P4-11, P4-14, P5-01, P5-04, P7-05, P7-06), `runtime-context.ts` (P2-01, P2-02, P3-01, P6-01, P6-04, P6-05, P7-07) — so without a class-level record the queue would accumulate 21 entries reading as 21 independent concessions.

**`docs/architecture.md` belongs to P0-02.U, and P0-02 is ACCEPTED.** Every loop change therefore edits a closed epic's deliverable. Checked before ruling: **none of P0-02's frozen cases assert anything about that file's content**, so editing it cannot invalidate P0-02's green. Each patch's `reason` must say so, or a later reader will conclude someone is quietly editing an accepted row.

**The rule was already broken once, on an accepted epic.** During this program `docs/architecture.md` changed twice (P0-02.U, P1-08.U) while `agent-loop/src` changed four times — **P2-01 modified the loop and did not update the document, and P2-01 is ACCEPTED.** Handled as [BLOCKED-069](#blocked-069) handled its own: not reopened, not retroactively undone, and not pretended to have been followed. The documentation debt is recorded against P2-01 and **P2-03 must not discharge it** — folding another epic's omission into this one's commit merges two epics' accounts and hides which change actually owed what.

**Third member of [BLOCKED-046](#blocked-046)'s trigger-gap family, and the first with no gate at all.** A commit touching `packages/core/agent-loop/src/**` must also touch `docs/architecture.md` — mechanically decidable, one line of `lefthook` configuration. The other two members (the cordis catalog, twice, from two different epics) at least had a gate somewhere; this requirement is enforced by nothing but memory, and now has 21 chances to be forgotten. **Not built now**; recorded so the trigger work has its third concrete case.

**Correction to the reconciliation criterion, from the delegate, and it outranks the ruling above.** [BLOCKED-046](#blocked-046) offered: *"remove this edit — can the declared change still pass repo-wide gates? No → reconciliation; yes → scope creep."* Applied here it answers **"yes, it passes"**, because no gate checks this rule — which would license skipping a requirement the repository states in plain text. **The criterion substitutes "will a gate go red" for "is this edit required", and those are equivalent only where the requirement has a gate.** For a prose-only requirement it returns a stable false negative: it is not measuring necessity, it is measuring gate existence. **When the criterion answers "it passes", confirm that is because the edit is unnecessary and not because no gate exists.** The requirement's own text governs; the criterion does not.

This matters beyond one ruling: **every "run the gate and see if it goes red" verification in this program is only as good as the gate's existence.** This queue has now measured ten documentation gates entirely outside the program's evidence path ([BLOCKED-069](#blocked-069)), a catalog trigger that fired twice unnoticed, and now a rule with no gate whatsoever. The false-negative rate on that style of check is higher than its confident phrasing suggests.
...
**A warning does not prevent this class; only the act of evaluating does.** Instance #6 was named in advance — the delegate wrote *"`phaseOrder` says prep comes first, but that is ORDER, not DEPENDENCY; evaluate each one, do not infer"* — and the Supervisor made exactly that inference anyway, then overturned it by reading all 13 `predecessor` fields. **A trap that still lands after being pointed at is not defended by remembering to be careful; it is defended by performing the evaluation.** That is why the rule below is an action, not an admonition.

**Instances 8-10, and the recursion that matters.** Three more landed the same day, and two of them fell on the evidence machinery itself rather than on the thing under test:

8. **The probe.** Verifying a global-singleton fix, the "real write is still caught" half reported *nothing caught* — which read as the fix working. The probe was malformed: `(globalThis as any).X` puts a paren between `globalThis` and the dot, so the read pattern could never match. **The instrument, not the subject, was broken**, and the fix would have landed carrying a false proof. Instances 1-7 were "the thing under test was never evaluated"; this is "the tool used to evaluate it was never evaluated" — the same shape recursed one level, onto the very tool that catches the other seven.
9. **The frozen expectation.** A stage's `expectCases` was built by scanning the spec for literal `it('...')` lines: 38 found, **41** actually discovered, three generated by a loop and carrying no literal title. A freeze whose case list comes from *reading* the file rather than *running* it records an expectation describing a suite that was never executed — and that cell could never have greened, with no indication why. `dryRunProof` exists to attest exactly this; the list must come from the JSON reporter.
10. **Confirmation mistaken for verification.** A delegate finding ("this dependency is declared and never used") was reported as independently confirmed by the Supervisor. The delegate had searched for the package name followed by a closing quote; the Supervisor searched for the package name followed by a closing quote. Neither pattern can match `@deepseek-ai/dsh/package.json`, a subpath reference — the real, load-bearing usage. **A check that shares a blind spot with the method it checks carries zero information while reading, in the record, exactly like a genuine second source.** A bad commit landed and reddened CI.

**Instances 11-12, both caught before they cost anything.**

11. **Coverage held by a side effect, not by a declaration.** `tsconfig.host.json`'s `include` lists `scripts/**` and `tests/**` but not `benchmarks/**`. A U-stage runner under `benchmarks/` typechecked anyway — **only because a spec file under `tests/` imported it**. The scenarios' own `index.ts` had no project membership at all, and the day that spec stopped importing it, type coverage would have vanished with no error anywhere. "This file is typechecked" was true; the reason it was true had nothing to do with any declaration. Found while fixing an unrelated TS6307 and repaired by adding `benchmarks/**` to the include.

12. **A citation written from memory instead of from the ledger.** Building acceptance-coverage entries, the Supervisor cited a P0-04 case titled *"completes a 300-package scan within the 10-second budget"* — **a title that does not exist**, composed from an impression of what the suite probably contained. The pre-write exact-match check refused it. This is the second time that check has caught the same habit (the first was column-truncated titles and a curly apostrophe), and the more useful half is what the real cases turned out to be: the timing case runs a **dense, many-overlapping-cycle graph** — the shape where a cycle search degrades — and a second case requires the true shortest cycle to be found **even when a longer disjoint cycle is encountered first**. The invented citation would have left acceptance[2] "covered" by a case that measured a performance number instead of the property the clause promises: [BLOCKED-067](#blocked-067)'s shape, prevented by a mechanical check rather than by care.

**The same file, the same kind of function, hours apart, opposite outcomes (2026-09-05).** `checkDeliverablePathPatches` computed a complete verdict that nothing consumed — instance 4 above, and the reason a ruling that depended on it rested on a false premise. Later the same day `checkSensitivityProofs` was added to the same file, and the difference is one line:

```
:1931   const sensitivity = checkSensitivityProofs(freezeEntries)
:1932   if (!sensitivity.valid) throw new Error(...)      ← the verdict is consumed
```

**Computing a verdict and acting on one are separate acts, and only the second is a gate.** The pair is worth keeping together: the same author, the same file, the same shape of function, and the only distinguishing feature is whether anything reads the result. That is the whole of this class in two adjacent examples.

**A second pairing, on schema duplication.** [BLOCKED-083](#blocked-083) records two packages independently declaring `CorruptedLogEvidence` — cross-package, with "keep the packages independent" as a stated (if mispriced) reason. The same day, the Supervisor added a `sensitivityProof` field to a freeze entry using invented key names while `generate-specs.ts:338` already defined `{ mutationDescription, failureSummary }`, with three entries already using it. **In-package, in a file being edited at that moment, with no reason at all — simply not looking.** The cross-package case at least bought something; this one was pure omission, and it shows that "check what already exists" is not automatic when adding a structured field. **One `grep` for the concept in the same file or package is the whole remedy.**

**Third binding requirement, from instance 10.** *An "independent verification" must state where its method differs from the method it is checking. If no difference can be named, it is repetition, not verification.* The positive example from the same day: two `git show | shasum` measurements of one file disagreed, and the tiebreaker was `git rev-parse` — which qualified precisely because it asks git's object index rather than re-reading the file stream, a **different kind** of question, not the same question asked again.

**Also from instance 10, worth its own line:** the missed channel was a subpath `import.meta.resolve`, and `layering.md`'s Detection section already names that channel — *"a dynamic `require()`/`import()` call invisible to a static package-graph walk."* Three packaging gates missed it for the same structural reason. **A blind spot the documentation explicitly names was still walked into, by two parties actively using that documentation** — reading a warning and evaluating against it are different acts.
...
**Integrity of the source, checked before trusting anything in it.** Its `requirementSource` names `first100-requirements-matrix.md` at `401a3c63…`; the file on disk hashes to exactly that. Its `canonicalEpicIds` lists 100 ids, and **all 100 resolve against this repository's `registry.json`** — zero unknown. The registry is internally consistent and describes this program, not a divergent one.

**Shape.** 13 preparation contracts, 46 lifecycle contracts, 15 observation slices, 3 control slices (`R10-REVIEW`, `R10-GATE`, `R10-PROMOTE`). `blockingMissingCommandContracts` is empty, and all 13 preparation contracts carry `commandContract.status: READY` with real `red`/`green` argv, expected exits, and typed `intendedFailureId`s. **The specification is complete.**

**Build state: nothing exists.** Collecting every `implementationPaths` and `trackedPaths` entry across the 13 preparation contracts yields 23 distinct paths. **All 23 are absent from this repository** — `scripts/first100/generate-r10-qualification-registries.ts`, `spec/first100/safety-case-registry.json`, `spec/first100/disturbance-coverage-registry.json`, `spec/first100/gate-specific-qualification-registry.json`, `tests/first100/r10-registry-contract.spec.ts`, the qualification Agent Note triplet, and the rest. R10 preparation is **0% built**, not partially built.

**`registryStatus: PLANNING_CANDIDATE_NOT_EXECUTION_AUTHORIZATION`.** R10's own registry states it is not yet authorized to execute. Reaching 100/100 does not by itself start R10; that status has to change first, and changing it is the maintainer's act, not this program's.

**Ordering, from `phaseOrder`:** `13_PREPARATION_SLICES_IN_REGISTRY_ORDER` → `100_ROWS_IN_CANONICAL_EPIC_ORDER` → `15_OBSERVATIONS_IN_REGISTRY_ORDER` → `R10-REVIEW` → `R10-GATE` → `R10-PROMOTE`.

**CORRECTION (2026-09-04, same day): this entry first concluded from that list that "the 13 preparation slices are not gated behind 100/100 and could be built now." That is wrong.** `phaseOrder` is a *listing order*, not a dependency graph, and the dependency is recorded elsewhere: **`R10-PREP-R1-REGISTRY-CONTRACTS`'s `predecessor` is `P8-10-D-PROMOTE`** — P8-10 being a **wave-19** epic, the last in the program. R10 preparation cannot begin until the final registry epic is promoted. Every one of the 13 then chains strictly serially (each one's predecessor is the one before it, by `DIRECT_PARENT_COMMIT` or `SAME_FIXED_SHA`), so there is **no parallelism within preparation either**.

The error is instance #6 of [BLOCKED-072](#blocked-072), and the delegate had named this exact trap in advance — *"`phaseOrder` says prep comes first, but that is ORDER, not DEPENDENCY; evaluate each one, do not infer from `phaseOrder`."* The correct reading came from evaluating all 13 `predecessor` fields; the wrong one came from reading a sequence and treating it as a graph. **A listing that resembles a dependency order is a description, and this queue's standing rule is that a description is not an evaluation.**

**Actor roles, verified rather than skimmed.** Across the 13 preparation contracts: 10 `EXECUTOR`, 1 `MAINTAINER`, 1 `INDEPENDENT_READ_ONLY_FIXED_SHA`, 1 `DETERMINISTIC_GATE`. **`SIGNER` never appears as any slice's `actorRole`** — all 13 of its occurrences in the file are inside `mustDifferFromRoles`, i.e. a constraint that the executor must *not* be that role. So the hollow-signature-root lock ([BLOCKED-050](#blocked-050)) does not touch R10 preparation at all, which removes a coupling that would otherwise have been ugly.

**Exactly one user touchpoint, not "several".** `DECISION` appears once in the whole registry: `R10-PREP-D1-REGISTRY-APPROVAL` (`mode: MAINTAINER_DECISION`), sitting fifth in the chain. Four executor contracts complete, then everything halts for one human decision before the remaining eight — it is a mid-chain stop, not a final rubber stamp. The precision matters for what the user is told: their involvement in R10 preparation is one decision, once.

**What this changes about the terminal state.** "R10 passed" is not a gate that evaluates once the 100 rows are ACCEPTED. It is 13 preparation slices, 15 observation slices and 3 control slices, none of which has a single file on disk today, plus a status flag only the maintainer can flip, plus (per [BLOCKED-039](#blocked-039)) a `DEEPSEEK_API_KEY_EXTERNAL` and a separately-approved budget for the Q3 statistical portion with its three-night floor. **Any estimate of remaining work that counts only the 88 unaccepted registry rows is wrong by the whole of R10.**
...
**So the deviation is an archival error, not a convenience.** The affected stages test local behaviour — P6-07.F's are session-log paging, filtering, deletion, retention, and corrupt-log repair, with no model call anywhere — and the spec filed them into the suite that costs tokens and requires the user's key. A substitution moves a local test out of a paid channel it never belonged in. **Recording it as 99 separate path deviations would misrepresent the program as systematically rewriting paths to suit itself**; it is one archival correction with 99 application sites.

**Already-set precedent, found in this program's own freeze register.** P6-07's **C** stage hit exactly this and was corrected on 2026-09-03: the freeze entry naming `lifecycle.e2e.ts` under `-c vitest.e2e.config.ts` (27 cases) was **superseded** by one naming `lifecycle.spec.ts` under the default config (28 cases), and the cell greened on the latter. `tests/first100/adjudication.json` carries **no** patch for P6-07 — so C deviated from its registry-declared path through supersession alone.

**RULING (delegate, 2026-09-04).** Three facts settled it. First, **the registry cannot be repaired**: these paths come from the pinned v1.0 source (`.e2e.ts` appears 100 times in it), and under [BLOCKED-035](#blocked-035) a pinned source is byte-fixed and is never edited to suit the program — so the deviation can only ever be *recorded*, never removed. Second, of the two remaining mechanisms, **`deliverablePathPatches` is the one a machine checks** and freeze supersession is the one that relies on someone remembering; by [BLOCKED-034](#blocked-034) the checked path wins. Third, the two are not alternatives but halves: **BLOCKED-070 carries the reasoning once, in prose, at class level; each patch carries one site, mechanically validated.** A patch's `reason` cites this entry rather than re-arguing it.
...
**Finding.** `pnpm run doc-sync` fails **10 gates** on a clean `land-base-align-v2` tree: translation pairing, markdown wrap, export jsdoc, config catalog, persistence catalog, subsystem pages, package paths, package README model experience, doc budgets, package README limitations. None is caused by any single recent change; they are other lanes' files, accumulated.

**Why nothing caught them — measured, not inferred.** All ten live in `docSyncLeafGates()` (`scripts/run-gates.ts`), which **is** spread into both `ciPrimaryGates()` and `ciStaticGates()`, and `ci.yml` runs `check:ci:static` — but `ci.yml` triggers on `pull_request` only. The First-100 program never opens a pull request: it pushes to a fork branch and greens cells from `first100-exact-sha.yml`, whose entire job is install / `typecheck` / `vitest run` / build / pack. **It runs no documentation gate at all.** `docs-pages.yml`, the only workflow invoking `doc-sync` by name, is `workflow_dispatch`-only and publishes from a release tag.

So this is not "a red gate nobody manages" — it is **a set of gates this program's evidence path cannot see**. Every cell greened to date was greened by a workflow that never evaluates a documentation invariant, which is exactly why the count reached ten without anyone reporting one.

**This was a reasoned decision, not an oversight — which is why nobody questioned it.** `first100-exact-sha.yml`'s own header states the reasoning in full: *"`ci.yml`'s test jobs trigger on pull_request only — no channel exists to run the full gate against an arbitrary candidate SHA. This workflow is that channel, and per A4/B7① it is the ONLY place a ledger cell may be greened from."* The author knew `ci.yml` was PR-only and built this workflow precisely because of it. What went unexamined is that making one workflow the sole greening channel also **removes every gate that workflow does not run from the program's evidence path entirely**. A correct decision produced an unasked question, and the ten failures are its accumulated answer. Record this rather than the failure count: the count is a symptom, the unasked question is the mechanism, and the same mechanism applies to any other gate family `first100-exact-sha.yml` omits.

**Already-accepted rows carry this gap too.** Any of the 12 ACCEPTED epics that declared a documentation deliverable — P0-02's `docs/architecture/trust-kernel-boundary.md` and `spec/trust-kernel.md` are the clearest — had those files evaluated by no documentation gate at any point. Their behavioural evidence is unaffected and **the rows are not being reopened**: the cost of re-running them exceeds the value, and the behavioural half is real. But the documentation half of those rows is mechanically unverified, and this entry says so rather than letting silence imply otherwise. **Binding follow-up:** once the ten are cleared, run the repaired gates against the documentation files declared by all accepted rows, and record every failure as a finding attributed to its introducing epic. No retroactive un-accepting; no pretending it was checked.

**Consequence.** The debt is invisible and cumulative, and it does not stay invisible: `ci.yml` will evaluate all ten the moment this branch is proposed to master. A program that has greened 12 epics without a single doc-gate observation will meet all ten failures at once, at the point where they are most expensive to diagnose and least separable by author.
...
**Named, and three of them are on ACCEPTED rows.** The paragraph below wrote this as a hypothetical when it was filed; it is not one. Running the real gates today reddens documentation deliverables belonging to epics already marked ACCEPTED:

| File | Gate finding | Declaring epic | Ledger status |
|---|---|---|---|
| `packages/identity/principal/README.md` | no `## Known Limitations and Deferred Work` section | **P2-01** | ACCEPTED |
| `packages/schema/schema-registry/README.md` | two limitations-like headings where exactly one is allowed | **P0-06**, **P1-08** | ACCEPTED |

**Ruling: not reopened.** The behavioural evidence for those rows is real, and what is red here is README structure, not a falsified behavioural claim. **But it is not passed over silently either:** when these are repaired, the commit must name which ACCEPTED epic's declared deliverable each file belongs to. "ACCEPTED" must never quietly also mean "its documentation deliverable was never checked."

**One violation deserves separating from the other six.** `capability-token/src/attenuate.ts`'s `issueToken` carries `@param trustRoot` — a documented parameter **that does not exist**, beside a real parameter `_trustRoot` that is undocumented. This is the purest instance yet of the class this queue keeps recording: prose asserting the existence of something that is not there. It is the same defect as the catalog header naming a gate not in `doc-sync`, as the README describing landed code as an unimplemented scaffold, and as this session's own test count produced by a check that never examined what it claimed to. Count it once as a JSDoc violation and once as a member of that class.
...
**Triage status: done, durable, and already vendored.** `upstream-status-COMPLETE.md` (§二) carries the full P9-01..09 upstream-coverage verdict — P9-02 major shrink (pi-ai routing already ships, only a default template + conformance gate is needed), P9-01/P9-03 partial shrink (P9-01 narrows to "extract a reusable conformance kit + registration gate + fault injection"; P9-03 narrows to "headless `--model` flag + a CLI-selection session event"), P9-04 through P9-09 unchanged (genuinely full-scope, upstream has no coverage). This file is vendored into `spec/first100/sources/base-align-v2/` — the terminal-state requirement "P9 九项全部 VERIFIED 或 scheduled-BLOCKED 在案" is satisfied by its content as it stands; no further action was needed to reach that bar.

**Correction 2026-09-04 — the record is real, but it is NOT pinned, and the claim that it was is false.** This entry previously said the file is "byte-pinned by `.gitattributes`". That directory's `.gitattributes` contains `* -whitespace`, which is a whitespace-check **exemption** — it relaxes `git diff --cached --check`, the opposite of a pin — and nothing anywhere verifies this file's SHA. Its own sibling in the same directory *is* pinned (`NEWGAP_MATRIX_SHA` in `extract-registry.mjs:74`, enforced fail-closed at `:180`), so the mechanism exists and simply does not cover this path; the extractor references `upstream-status-COMPLETE.md` only as a free-text `rationaleDoc` string (`:509`).

**Proven by mutation, both directions.** Appending a fabricated verdict (`P9-07 = 已完成,无需实现。`) to `upstream-status-COMPLETE.md` and running `extract-registry.mjs` exits **0** — the tampered file is accepted silently. Control: the same append to the SHA-pinned `new-gap-matrix.md` exits **non-zero**. Both files restored byte-exact (`diff -q`) afterwards. So the single document carrying the P9 half of the program's terminal condition can be edited, by anyone or by any agent, with no gate anywhere reporting it.

**Consequence for the terminal state:** "P9 九项全部 VERIFIED 或 scheduled-BLOCKED 在案" is today evidenced by an unverified file. That is weaker than every other terminal-state input, all of which trace to SHA-pinned sources. **Add this path to `NEWGAP_SOURCES`'s pinning (or an equivalent fail-closed SHA assertion) as part of Stage-0's scope** — it is a one-line addition to machinery that already exists and already covers its sibling, not new infrastructure.

**Pin applied and ratified 2026-09-04.** `CITED_SOURCE_SHAS` in `extract-registry.mjs` now fail-closed SHA-verifies this path. Kept deliberately out of `NEWGAP_SOURCES`, whose entries are *parsed into epic rows* — this document has no matrix or wave-map structure, so it needs a verified-but-never-read list of its own; the reason is stated in the code, not only here. Proven three ways: on a clean tree extraction exits 0 and `registry.json` is **byte-identical** (the pin changes no output); a fabricated appended verdict exits non-zero naming the path; a deleted file exits non-zero. `registry-regenerate.spec.ts` 9/9.

**Ratified by the delegate as in-scope now rather than deferred to Stage-0**, on the grounds that this is not registry-ification — it adds no P9 registry row and does not unlock the 100-epic hard count — but closes a *proven* tamper hole using machinery that already exists, and a document that decides a terminal-state answer must not be silently editable. The delegate's own verification is worth recording: their first two measurements disagreed (`git show | shasum` on a large file through a truncating pipe), and rather than picking one they consulted the authoritative git object id — `491298bc…`, 509399 bytes, identical before and after. That is instance #3 in [BLOCKED-072](#blocked-072).

**Method note, recorded because it nearly produced a false finding.** Checking coverage by grepping each id individually reports `P9-05` through `P9-09` as **absent**: the file states their verdict on one line as a slash-joined range (`P9-04/05/06/07/08/09 = 全缺,照做`). All nine are genuinely covered. A per-id `grep -c` would have supported a confident report that the triage was incomplete — the same shape as [BLOCKED-046](#blocked-046)'s catalog-header prose and this session's deduped-Map test count: a mechanical check that never examined the thing it claimed to. **Read the source lines before concluding from a count.**
...
**UPDATE 2026-09-03, terminal-state bar raised — user ruling (relayed via delegate, quoted user approval "OK啊去吧"), supersedes the "no further action needed" line above.** The prior reading of "P9 九项全部 VERIFIED 或 scheduled-BLOCKED 在案" treated a single triage document (`upstream-status-COMPLETE.md`) as sufficient for the whole terminal-state clause — the user's actual bar is higher than that wording literally allowed: **the program's real target is "110 optimization items completed to real quality," and a triage document satisfying "scheduled-BLOCKED" with zero code is below that bar.** Revised standard, effective now (timing unaffected — see below):

> P9's 9 items' terminal state does NOT accept a triage document alone as satisfying "scheduled-BLOCKED 在案." Each item must get a real registry row and go through the full 4-stage (C/P/U/F) acceptance pipeline like every other epic. An item found genuinely infeasible must carry per-item, evidence-based justification (what was actually tried, exactly where it got stuck, why it's infeasible within this machine/program's scope) — never covered by one blanket planning document standing in for all 9.

**Timing is unchanged — do not act on this now.** Stage-0 remains deferred to "before W20 opens," per this entry's own existing binding requirement above. The user's ruling raises the bar, it does not move the trigger; acting on it now would compete with W4 for resources for no gain (nothing in W1-W19 depends on this).

**Numerator/denominator gap found while recording this, add to Stage-0's own scope**: `scripts/first100/generate-ledger.mjs`'s `P9_EXTENSION_ITEM_COUNT = 9` exists ONLY as a constant added to `registry.epics.length` to compute `totals.totalEpics` (currently 110) — no code anywhere checks the 9 P9 items' actual status, and `totals.acceptedEpics` is derived purely from real ledger ACCEPTED rows, which P9 items can never populate today (they have no registry rows). Consequence: even if all 101 current registry epics reach ACCEPTED, `acceptedEpics` tops out at 101 — the numerator can never reach the 110 denominator under the current mechanism. Stage-0 registry-ification MUST resolve this alongside vendoring/SHA-pinning/extractor-unlocking: either give the 9 P9 items real registry rows so they can genuinely accept (making the numerator reachable), or make `totalEpics`'s own definition explicit about why acceptedEpics is capped below it. Do not discover this gap for the first time at W20's own gate-opening.
...
### BLOCKED-045 — new First-100 GREEN packages accumulate real `pnpm run test:docs`/doc-sync gate debt (bilingual pairing, Model Experience short-form entries, doc word budget); not CI-blocking for ledger-greening, deliberately deferred (Supervisor finding, 2026-09-03)

**Finding**: landing P0-04(C)/P1-08(C) GREEN into `first100-exec` broke a real `first100-exact-sha.yml` CI case — `scripts/doc-standard.spec.ts` (a vitest spec, part of the workflow's "Full test suite" step) failed because `packages/plugin/plugin-compat/README.md` used `kind: "package-library"` without being in `scripts/doc-standard.spec.ts`'s `PACKAGE_LIBRARIES` allowlist. Fixed (see the commit landing this entry): added `plugin-compat`/`plugin-ownership` to the allowlist (both match the existing `plugin-manifest` precedent exactly — Contract-stage type declarations and pure functions only, no plugin entry/mount) and added `plugin-ownership`'s missing `### Dev Note` section (the same gate's structural-skeleton check).

**Separately, while diagnosing this, running the full `pnpm run test:docs` (broader than what CI actually gates — confirmed by reading `.github/workflows/first100-exact-sha.yml` directly: it runs only `pnpm run typecheck` + the vitest suite, never `doc-sync`/`test:docs`) surfaced real, growing debt these landings introduce, none of it CI-blocking**:
- `packages/plugin/plugin-compat/README.md` / `packages/plugin/plugin-ownership/README.md`: short-form Model Experience entries need an audited `SENTENCE_MODEL_EXPERIENCE` entry (`scripts/verify-package-readme-model-experience.ts`).
- Same two files, plus `docs/architecture/layering.md` (P0-04's new doc): not bilingual-paired (`scripts/verify-translation-pairing.ts` finds no path-based exemption for new First-100 packages — every package README and doc under `docs/` is in-scope by default, and CLAUDE.md's "bilingual work requires explicit invocation" governs who performs the translation, not whether the pairing gate itself is satisfied).
- `packages/README.md`: crossed its `verify-doc-budgets` word ceiling (1005 > 994) after P0-04's 2-line addition.

**Explicitly not fixed now.** None of this blocks the real ledger-greening gate (confirmed by reading the workflow file, not assumed), and every remaining W4/W5+ epic's GREEN landing will introduce the same class of gap (a new package README, no bilingual pair, possibly a new `SENTENCE_MODEL_EXPERIENCE` entry) — fixing it piecemeal per landing is real, repeated work each landing already carries enough of. Scheduled: a dedicated doc-sync pass across every First-100-created package, batched, before this program's final release gate (not blocking any individual wave). Until then, this entry is the tracking record — check it before assuming `pnpm run test:docs` clean is a landing requirement; it is not, for this program's own ledger-greening purposes.

### BLOCKED-046 — parallel GREEN lanes each adding a new package leave repo-wide single-point-of-truth files (pnpm-lock.yaml first observed) unreconciled after merge; must recompute once on the merged tree, never inside one lane (delegate + Supervisor, both independently caught, 2026-09-03)

**Finding**: landing P1-02(C)/P1-08(C)/P1-09(C) GREEN — three separate lanes, each adding one new package (`plugin-provenance`/`plugin-compat`/`plugin-ownership`) — produced three consecutive real `first100-exact-sha.yml` failures (`6d69e3b681`, `9a3e51305f`, `c68ad64261`), all stuck at the same step: `pnpm install --frozen-lockfile` (`ERR_PNPM_OUTDATED_LOCKFILE`, specifiers in `pnpm-lock.yaml` not matching `plugin-provenance/package.json`). Root cause: `pnpm-lock.yaml` is repo-wide single-point state — each lane's own local `pnpm install` regenerates it correctly for THAT lane's new package in isolation, but a later merge only combines `package.json` files textually; nothing recomputes the lockfile against the union of all newly-merged packages. The Supervisor's own proximate mistake compounded this: staged `pnpm-lock.yaml` before re-running `pnpm install` after a manual merge-conflict resolution, so the commit carried the pre-install (stale) file — caught only by CI's `--frozen-lockfile`, never locally, since local installs in this session didn't pass that flag.

**Fix applied**: `pnpm install` (no `--frozen-lockfile`) rerun on the fully-merged tree, verified via `pnpm install --frozen-lockfile` succeeding locally before pushing again.

**Widened 2026-09-03 (delegate) — this entry covers repo-wide check BLIND SPOTS, not only stale generated artifacts.** A file a gate cannot see is a file that gate does not exist for, and scanners do not report that: they skip silently and return success.

Members found so far, each of which produced a real CI failure or a silent exemption:

- **`pnpm-lock.yaml`** unreconciled after parallel merges (the original finding above).
- **`spec/*` generated artifacts and the cordis catalog** — stale after any lane changes a package surface; both fail their own byte-identical reproduction tests. Regenerate on the merged tree.
- **Missing `tsconfig.base.json` `paths` aliases for published subpath exports — now FOUR recurrences.** `@deepseek-ai/dsh-plugin-compat/solver` (added by P1-08.U, found by an unrelated lane whose composition fixture died on `ERR_MODULE_NOT_FOUND`) is the fourth. Its symptom is what makes it costly: the source plane resolves the subpath to unbuilt `lib/`, so **every real-composition boot from source fails**, and the failure looks like a broken test fixture rather than a missing alias. The lane that hit it had to diagnose past a wrong-reason RED before it could freeze. **Add the alias in the same change that adds a subpath import**, not after a downstream lane trips over it. `gen-tsconfig-paths.ts` emits only a bare and an `/invariant` alias per package, so a package publishing `./types` in its `exports` needs a hand-written entry outside the generated region or the source plane cannot resolve it. `@deepseek-ai/dsh-workspace-trust/types` was the **third** recurrence; the file's own comment already recorded that this "happened once already".
- **A `runLoaderSmoke` lane that dies with `ERR_MODULE_NOT_FOUND` is almost certainly hitting a missing `paths` alias, not a missing build.** Two lanes hit this within an hour and both first concluded a build was required. **That conclusion was wrong**, and the lane that reported it retracted it after testing properly: `pnpm run clean` (every `lib/` gone) followed by the accepted `P0-01.composition.spec.ts` passes 2/2 with no build output present. `runLoaderSmoke` defaults to `src` mode (tsx + tsconfig `paths`), so a subpath with no alias is the *only* thing that falls through to real `exports` and thus to a `lib/` that may not exist. A build "fixes" it by satisfying the fallback, which masks the real cause. **Diagnose with a control** — run an already-accepted composition spec on the same tree; if it fails identically the cause is environmental, and the first place to look is `tsconfig.base.json`.

- **A `json.dump` rewrite of `command-freeze.json` silently re-escapes every CJK character in all existing entries.** Append textually instead. The damage is invisible in review because the diff looks like formatting; catch it by checking the commit stat for deletions on a file you only appended to.
- **A green package-level `tsc -b` does not mean a new spec file typechecks.** Package `tsconfig.json` sets `rootDir: src` and does not include `tests/`, so a spec's type errors are invisible to `tsc -b <pkg>/tsconfig.json` and surface only under repo-wide `pnpm run typecheck` (which seeds a face config covering `packages/*/*/tests/**`). Found when P1-02's Provider stage passed its package build while its new spec carried three real `noUncheckedIndexedAccess` violations. **Every lane must run repo-wide `typecheck`, never only the package one, before reporting a stage green** — the package-level command is the faster check, not the sufficient one.
- **Files carrying a literal NUL byte — now TWICE, from two unrelated lanes.** `plugin-compat/src/index.ts` (three, as composite-key separators) and `agent-instructions/src/index.ts` (one, in a cache-identity key added by P1-07.U). A raw NUL makes `file(1)` report `data`, makes `grep` skip the file as binary, and makes **`git diff` render it as `Bin` rather than showing the change** — so the file is invisible to every text-scanning gate *and* its diffs are unreviewable, with no error anywhere. Write `\0` instead; the runtime value is identical. The idiom is attractive (a NUL cannot occur in an identifier, so it is a safe separator) and will keep recurring: **check `git diff --stat` for a `Bin` marker on a `.ts` file.**

  **Source traced (2026-09-04):** it is **not** a copied idiom. The repository's five pre-existing uses (`fs-local/win32.ts`, `sandbox-windows-acl/win32-abi.ts`, `subprocess-local/process-inspector.ts`, `win32-process/abi.ts`, `code-runtime-python/index.ts`) all spell it correctly as `\x00`, and no source file now carries a literal one. Both offending bytes were emitted directly at file-write time by the authoring agent, in a template literal where the escape was intended. **So the mitigation is detection, not convention** — telling a future lane "use `\0`" does not help when the lane believes it did. The `Bin`-marker check is what actually catches it. `packages/plugin/plugin-compat/src/index.ts` used three raw NULs as composite-key separators in template literals. The runtime value equals `\0`, but a raw NUL makes `file(1)` report `data` and makes `grep` treat the file as binary and skip it — **the file was invisible to every text-scanning gate in the repository, with no error anywhere**. Replaced with the escape. Four other files contain literal NULs; those are fixtures deliberately exercising NUL handling. This belongs to the same family as [BLOCKED-048](#blocked-048) (a frozen test never discovered by CI) and [BLOCKED-052](#blocked-052) (a test that cannot fail on the platform it runs on): in all three the check reports success while never examining the thing it is supposed to examine.

- **Widening a public union type stales the generated cordis API catalog — and the artifact's own header names the wrong gate (2026-09-04).** P1-07.F added `'not-an-upgrade'` to `TrustUpgradeDenialReason` so `requestTrustUpgrade` could refuse a lowering target. That is a public-face change, so `packages/extensions/tool-cordis/src/api-catalog.ts` went stale and `cordis-catalog.spec.ts`'s byte-for-byte reproduction case failed — **one failure in 19183** — and stayed red across two descendant commits, neither of which regenerated it. The behavioural cases of both F stages in that batch passed; only the derived artifact was stale.

  **The header lied about its own gate, and it disabled two readers.** It read "freshness-gated by `pnpm run verify-cordis-api` in doc-sync". Neither half holds: `verify-cordis-api` has **zero occurrences** in `scripts/run-gates.ts`, and the gate that does fire is `verify-cordis-catalog` at `run-gates.ts:724`, which names the exact path and the exact fix. Both the delegate and the Supervisor read that sentence and concluded the covering gate was absent from `doc-sync`, which inverted the remedy — the discussion moved to "add the missing gate" when nothing was missing. Measured instead of read: with the artifact reverted to stale bytes `verify-cordis-catalog` goes red and `verify-cordis-inspect-catalog` stays green; with the client artifact made stale, exactly the reverse. **Each artifact has its own gate and both already run in `doc-sync`.** Fixed at the generator source (`packages/typert/generator/src/cordis-catalog.ts`), never in the artifact — editing the artifact violates its own do-not-edit line and `verify-cordis-catalog` catches it immediately, as it did on the first attempt.

  This is the same shape [`verify-no-stale-red-claims`](#blocked-055) exists to catch, with one difference worth keeping: there, stale prose switched a **gate** off; here it switched two **readers** off. A gate that cannot be disabled by prose is still reachable by a reader who believes the prose.

  **The defect is the trigger, not the coverage.** The push that shipped it ran `typecheck` plus targeted vitest, on the reasoning that the change touched one package's `src` and one test. That reasoning is wrong: changing a public export necessarily spills into derived artifacts, and the public face *is* a surface under the repo's match-evidence-to-the-surface rule. No **rule** says "you changed a public export, therefore run `doc-sync`" — and by [BLOCKED-034](#blocked-034), correctness maintained by remembering which gate to run is itself the defect. **The mechanism to express that rule already exists; only the rule is missing.** Read the previous sentence as "so a hook mechanism must be built" and you will spend a day building what is already installed:

  - `lefthook.yml` is in the repository and live, with three stages: `pre-commit`, `pre-merge-commit`, `pre-push`.
  - `pre-push` currently holds exactly one job: `typecheck`.
  - `pre-commit` already carries four glob-triggered jobs, one of which — `third-party notices` — is precisely the shape needed here: **glob matches → run the generator that covers the artifact → regenerate → `git add`**, with a comment stating the principle as *regenerate rather than reject*.

  This entry is the first substantive record of any of that: `lefthook`, `glob`, and `pre-push` appear nowhere else in this queue, and the one adjacent mention (BLOCKED-036's passing reference to "the pre-push hook's own real `tsc -b`") proves the hook was running while nobody connected it to this gap. **The fix is a line of configuration, not a facility.**

  **Prerequisite, discovered while measuring:** `doc-sync` currently fails **10 of its gates** on a clean tree (translation pairing, markdown wrap, export jsdoc, config catalog, persistence catalog, subsystem pages, package paths, package README model experience, doc budgets, package README limitations) — all other lanes' files. Attaching a habitually-red suite to `pre-push` installs a gate everyone routes around with `--no-verify`, which is worse than no gate because it looks like coverage. **Clear those ten first, or attach only the specific green gates.**

  **A hook mechanism exists and this checklist is not yet attached to it.** `lefthook.yml` is live (`pre-commit` and a `pre-push` stage already running `typecheck`), and `pre-commit` already carries four glob-triggered jobs — including `third-party notices`, whose own comment states the principle as *regenerate rather than reject*: one glob over every input the generator reads, then regenerate and `git add`. That is precisely the missing shape, already implemented for a different repo-wide artifact. So this checklist is convention **because nobody attached it to the existing hook**, not because no hook exists. (Checked `.husky/` and `.git/hooks` first and found nothing — both are the wrong place: this repo uses lefthook, and a worktree's hooks do not live in `.git/hooks`. Another instance of concluding from the wrong location.)
...
2. `pnpm exec tsx scripts/first100/generate-specs.ts` — **required after ANY edit to `tests/first100/adjudication.json`**, including a single added path patch. `spec/deepseek-harness-optimization-manifest-v1.1.yaml` records the **byte size** of `adjudication.json` and its siblings, so every patch changes it and two byte-identical-render gates fail. This is the most literal member of the one-file-describes-the-whole-repo class, and [BLOCKED-053](#blocked-053) guarantees more patches are coming.
3. `pnpm run gen-cordis-catalog` — after any change to a public service surface; it **fails outright** on an unclassified type rather than warning. **"Public service surface" includes widening an exported union type**, which is the case that shipped a red CI three commits running; verify with `pnpm run verify-cordis-catalog`, the gate that actually names the stale path.
4. package README `kind` / Dev Note, and `docs/subsystems` index in both locales
5. repo-wide `pnpm run typecheck` — never only the package-level `tsc -b`
...
**Explicitly not built now**: no automated "recompute all single-point-of-truth files after every merge" gate — per the standing freeze, this stays a Supervisor discipline (recompute after merging ≥2 lanes that each touched such a file, verify with the tool's own strict/frozen mode before pushing) until a real mechanism is justified, likely alongside BLOCKED-042's audit.

**Complete member list, read from the repo's own gate assertions, not guessed (delegate method, 2026-09-03)**: searched `scripts/**`/`tests/**`/`packages/*/*/tests/**` for assertion titles matching "every package/every subsystem/one row per/byte for byte/must carry" — every such test is, by construction, a single-point-of-truth-file gate; its failure message names exactly what to recompute. Confirmed members, each hit at least once landing W4 GREEN work:
1. `pnpm-lock.yaml` — `pnpm install --frozen-lockfile` (CI step, not a vitest spec). Recompute: `pnpm install` (no flag) on the merged tree.
2. `packages/extensions/tool-cordis/src/api-catalog.ts` + every `docs/subsystems/*.md` `cordis-surface` region — `packages/typert/generator/tests/cordis-catalog.spec.ts`'s "reproduces every committed catalog artifact byte for byte". Recompute: `pnpm run gen-cordis-catalog`. A new `ctx.<name>` service (e.g. P6-01's `ctx.memory`) additionally needs a real `SERVICE_PAGE` entry pointing at a real subsystems page with an empty `<!-- BEGIN/END GENERATED cordis-surface -->` marker pair already present (the generator fills the region, never inserts the markers) — including a real bilingual `.zh.md` counterpart, a hard requirement of this generator itself, distinct from the deferred BLOCKED-045 doc-sync debt.
3. Every new package's `README.md` — `scripts/doc-standard.spec.ts`'s "keeps every package README on the summary, contents, and Dev Note skeleton" (needs a `### Dev Note` section, **in both locales** — `### 开发备注` for a `README.zh.md`) and its `kind` classification (`PACKAGE_LIBRARIES` allowlist entry required for `kind: "package-library"`, matching the existing `plugin-manifest` precedent when the package is Contract-stage types/pure-functions only, no plugin entry/mount).

4. **Every new `tests/fixtures/**/driver.ts` needs a `scripts/verify-application-entrypoints.ts` classification** (`'test-only subprocess driver'`). A real-composition test launches a driver subprocess, and that gate refuses an executable source it cannot classify. Two landed together and failed CI; the `time-context` entry is the precedent to copy.

5. **When a Usage stage turns a Contract-stage library into a plugin, REMOVE it from `PACKAGE_LIBRARIES` and change its README `kind` to `package-reference`.** The allowlist entry is a claim that the package has no plugin entry or mount configuration; once a `export default class … extends Service` lands, that claim is false and the gate fails on the next run. Expect this for every epic still holding a Contract-stage library — `capability-token`, `plugin-provenance`, `action-manifest`, `memory` and their siblings each replay it on reaching U.

**Empirical instance (2026-09-04), recorded because it settles an open architecture question.** It was previously only arguable *by design* that this program's Contract-stage libraries would become real plugins rather than staying inert type packages, and that the transition could not happen quietly. Both are now observed: `packages/run/run` became a plugin when P4-01's Usage stage landed (`export default class RunPlugin extends Service`), and `doc-standard` failed in that same CI round because the registry still declared it as having "no plugin entry or mount configuration". **The library-to-plugin transition does happen at the Usage stage, and it cannot be completed silently.**
4. `docs/subsystems/README.md` + `README.zh.md` — `scripts/project-doc-site.spec.ts`'s "indexes every subsystem page in both sides of the folder README" (one table row per page, both locales) — checked alongside this file's several other "every X" assertions (route publication, sidebar placement, navigation-item landing), all confirmed still passing after each fix this session, so no further member found there.
5. `tsconfig.base.json` (path aliases) — `scripts/gen-tsconfig-paths.spec.ts`'s "covers every workspace package in the committed config". Recompute: `pnpm run gen-tsconfig-paths` — already a standing step in every Writer's dispatch instructions this session, so this one has not actually broken CI (mentioned for completeness of the list, not as a new finding).

Not in this family but hit the same day, worth distinguishing: `packages/run/run`'s `package.json` declared a `"."` export (`./lib/index.js`) with no corresponding `src/index.ts` ever created — caught by `pnpm run typecheck`'s tsdown build step, not a generated-artifact drift. This is a single package's own internal packaging consistency, not repo-wide state; no member to add here, but a reminder that `pnpm run typecheck` (not just the test suite) needs a real local pass too when a new package's entry points are still being scaffolded across multiple Writer rounds.

### BLOCKED-047 — declaration-only stage greens (P0-08.C) prove a manifest states the right thing, never that it behaves correctly; acceptance[] verification must land on an executable stage, recorded explicitly so a later reader never mistakes "ACCEPTED" for "proven to run" (delegate ruling, 2026-09-03)

**Finding**: P0-08's C-stage has no `.ts` source — `tests/benchmark/runner.spec.ts`'s 3 frozen cases assert `benchmarks/harness-capability/manifest.yml` declares the right lanes/metrics/reporting-flags. For a pure-manifest epic, "the test passes" and "the manifest states the required words" are the same action — there is no independent logic that could be implemented wrong, unlike e.g. P4-01's 111 real-state-machine cases. The C-stage green is real and correctly scoped (must[0]-[2] map 3-for-3, no shortcut), but its evidentiary weight is real, not runtime — genuinely weaker than every other epic's green this session.

**The actual risk**: none of P0-08's 3 acceptance[] criteria (deterministic/security/fault lanes run without external API; the same seed reproduces the same failure location; real-model scoring stays separate from baseline invariants) are verifiable at C-stage — they all require the U-stage `runner.ts` to genuinely execute. If P/U/F stages for this epic also end up declaration-only (asserting more words in more files rather than running anything), the epic reaches "ACCEPTED" having never proven the benchmark framework can actually run — a shell verdict.

**Ruling, recorded on both the freeze entry and here for visibility**: P0-08.C's `command-freeze.json` entry is annotated declaration-only with this same finding. P/U/F stages for P0-08 MUST carry real-execution cases — at minimum, the 3 no-external-API lanes actually executing and producing a report, and the same seed run twice producing an identical failure location. A stage that only adds further declaration-only cases does not satisfy this epic's acceptance[] and will be rejected at that stage's review.
...
**Root cause, precisely (delegate's sharper framing, supersedes an earlier draft of this entry that blamed the `.e2e.ts` filename convention specifically)**: *a frozen entry's `argv` is documentation of what the Supervisor ran locally when freezing — it is never cross-checked against what the real CI observation step actually executes.* The two commands can silently diverge for any reason a local run tolerates but the CI step's own fixed invocation does not — a custom `-c` config flag (this case), a non-default working directory, a path an exclude pattern happens to catch, etc. The `.e2e.ts` extension was this occurrence's trigger, not the general failure mode.

**Not an isolated occurrence — traced back to source, not just the currently-frozen set**: `packages/session/session-lifecycle/tests/lifecycle.e2e.ts` is not a Writer's naming choice; the pinned `first100-requirements-matrix.md` (line 1134) itself declares this exact path as P6-07's own `kind: 'N'` deliverable, for both its C and F stages. The same `lifecycle.e2e.ts` naming pattern already appears in the same locked source for **two more, not-yet-built epics**: P6-03 (`packages/memory/memory-policy/tests/lifecycle.e2e.ts`) and P8-03 (`packages/sdk/protocol/tests/run-lifecycle.e2e.ts`). A scan of the 44 entries frozen at the time this was found showed P6-07 as the only current instance — but that scan cannot see un-frozen future epics' own declared paths, which is exactly where this will resurface.

**Fix applied**: recorded via the existing `deliverablePathPatches` channel (`tests/first100/adjudication.json`), not by editing the locked matrix — two entries, `P6-07-C-lifecycle-discovery` and `P6-07-F-lifecycle-discovery` (decided together now so F-stage does not rediscover the identical gap and invent a third path), `approvedPath` = `packages/session/session-lifecycle/tests/lifecycle.spec.ts`. The registry path stays byte-locked and unedited — the patch preserves the fact the source once declared `.e2e.ts`, which is precisely the evidence that made the P6-03/P8-03 recurrence foreseeable; editing the matrix in place would have erased that trail. Actual file `git mv`'d (zero content change, verified byte-for-byte: all 27 originally-frozen titles present unchanged in the renamed file). `command-freeze.json`'s stale C-stage entry (recording the never-executable command) was NOT overwritten in place — a new entry was inserted immediately before it in the array (so the mechanical `.find()` lookup every `generate-ledger.mjs` command uses picks the new one), and the old entry's own `note` field was appended with a `SUPERSEDED` explanation pointing at its replacement. Both entries remain present and queryable.

**Explicitly not done**: no rename/patch for P6-03 or P8-03 — they have not been scaffolded yet, and pre-patching a path for an epic with no real deliverable would be deciding an unverified question on their future Writer's behalf; by the time they're built, a genuine e2e-infrastructure need might justify a dedicated CI workflow instead (the sandbox/e2b/pi-ai-provider precedent) rather than a rename. **Mechanical trigger for whoever scaffolds P6-03/P8-03's C-stage**: before freezing, check whether the registry-declared test path is discoverable by `first100-exact-sha.yml`'s real observation command (bare `pnpm exec vitest run`, default config) — don't wait for CI to fail to find out, the way this entry had to.

**Not built as a mechanical gate now** (freeze still in effect) — but the concrete fix, not just the diagnosis, is already knowable, so recording it precisely rather than as a vague future audit item (delegate correction, 2026-09-03): every frozen entry already carries `dryRunProof.testsDiscovered` (a pre-existing field, predating this session — the P0-01.C entry already had it), the count of that entry's own case-titles found when the Supervisor dry-ran the frozen `argv` at freeze time. The missing check is one comparison between two numbers that already exist on both sides:
...
**How this entry gets unlocked, and what that will look like.** P1-02's Provider stage pins the current, hollow behavior in an executed assertion rather than in prose: a case that asserts an attacker who tampers with the artifact **and** rewrites the claim's digest to match still verifies as `trusted`. Its title marks it as a known gap citing this entry.

**When the Trust Kernel is given real key material, that test will fail. That failure is the unlock signal for P1-02, not a regression.** Whoever encounters it must not patch it back to green: they must delete it along with this blocker, because the gap it records will have closed. The same applies to any equivalent pinned case P2-02 grows. Pinning the gap this way is deliberate — prose describing a missing security control can be deleted silently by anyone; an assertion cannot go quiet without turning a suite red.
...
**A declared file can be evidence of the intended design — within a limit.** P6-01.U's Writer argued that `packages/context/README.md`, in that stage's declared `files[]`, is the context group map with one row per `packages/context/*` package, and therefore has **no legal edit at all** unless the consumer is itself such a package. That made the file list a fingerprint of the intended design, and the widening a recovery of it rather than a convenience.

**The limit (delegate, 2026-09-03): this reasoning holds only where the file's editability genuinely varies with the design.** The test is one question — *would this file also have legitimate content to edit under the other design?* If yes, it is not a fingerprint and proves nothing. If no, it is. It is evidence about authorial intent, never a general licence to widen scope whenever a declared file is inconvenient.
...
|---|---|---|---|---|
| **P1-02** *(LIFTED 2026-09-06 by the delegate (`guanjieqiao-92`), verified independently. Both halves are closed: the offline half on 2026-09-05, the Sigstore half on 2026-09-06 when the pinned `KNOWN GAP (Sigstore path)` case went RED and was superseded by five replacements — four refusals plus the positive control that the genuine bundle verifies. Its line in the machine-readable list above is removed in the same edit, per this register's own rule — the two must not drift apart, and the literal token is deliberately not repeated here so a scan of this file cannot read the annotation as a live entry.)* | 插件签名、来源证明. **The OFFLINE half is CLOSED**: signatures are verified with `node:crypto` against an admitted anchor's public key, all three acceptance[0] vectors die on `signature-invalid`, an unadmitted anchor is refused, revocation works in both directions, and the kernel holds a deployment's configured anchors. ~~**The SIGSTORE half is OPEN and is the whole of this lock now**~~ **CLOSED 2026-09-06.** `verifyPackageSignature` now verifies a Sigstore bundle with `@sigstore/verify` against the trusted root its admitted anchor declares, over the same `signedClaimBytes` the offline branch signs, with the certificate identity enforced by the verifier's own policy. The pinned `KNOWN GAP (Sigstore path)` case went RED and was deleted with four replacements: no bundle, a bundle issued to a different identity, a bundle signing a different claim's bytes, and an anchor carrying no trusted root — plus the positive control that the genuine bundle for the genuine claim verifies, without which the four refusals would prove nothing. Original clause list, for the record: (must[0], must[2], **must[4]**, acceptance[0] vectors 2–3) **(+ `validation[3]`, merged 2026-09-05)** the revoked-signing-identity test. | `createTrustKernel()` mints `signatureRoots` as `Object.freeze({})` — no key material, so the four-byte marker binds nothing. `observedSourceCommit`/`observedBuilderIdentity` are attestations, not recomputable local facts, so no non-cryptographic check closes swap-repo or forge-builder. **must[4] added 2026-09-04**: its `unsigned-dev` banner exists to distinguish "unsigned, untrusted" from "signed, trusted", but under a hollow root **there is no state in which the product can truthfully display "trusted"** — `verifyPackageSignature` trusts an unregistered issuer on first sight. A banner drawing that distinction would assert what the kernel cannot back, and a user would rely on it. Worse than no banner. **validation[3] has no subject, and the reason is the SAME hollow root rather than a separate gap** — which is why it is merged here instead of becoming its own lock. Measured: zero occurrences of `revok` anywhere in `plugin-provenance` or `trust-kernel`; `SignatureRejectionReason` has no `revoked` member; `registerTrustAnchor` returns an id rather than a disposer and nothing ever calls `delete`, so an anchor can never be removed. **The deeper reason is worse than a missing API: `verifyPackageSignature` trusts a first-seen issuer that was never registered at all.** Revocation would therefore be a no-op even if the registry could forget a key — **"no longer registered" cannot mean anything while "never registered" verifies.** One root cause, one lock, several clauses; a second lock here would inflate the count of how many distinct things remain unproven. | `packages/kernel/trust-kernel/src/index.ts`, outside every P1-02 stage's scope. | ~~The pinned case `KNOWN GAP (BLOCKED-050 hollow signature root): a tamper-and-rewrite attacker still verifies` starts FAILING.~~ **That criterion was MET on 2026-09-05 and was not sufficient**: it was written when the root was empty and both branches were equally hollow, so it fired as soon as either one closed. The new criterion is the case that replaced it: **`KNOWN GAP (Sigstore path): a claim naming an admitted Sigstore issuer verifies with no certificate and no inclusion proof`** starts FAILING, which happens when `@sigstore/verify` is wired. Delete it with the lock. The offline half's own replacement cases stay. — delete the old case with the lock. must[4] becomes claimable in the same moment, since a truthful "trusted" state then exists. **validation[3]'s own pinned case, `KNOWN GAP (P1-02 lock, validation[3]): an UNREGISTERED signing identity is trusted on sight`, starts failing at the same moment and for the same reason** — it proves not that a revoke API is missing but that one would be useless, which is precisely why its unlock signal coincides with this row's. Delete it with the lock and write the real revocation test then. |
...
| **P6-01** *(residual, added 2026-09-04 — the original lock is LIFTED; this is a new, narrower one)* | must[3]'s read scoping against a **hostile** provider | The Fault stage fixed the two shipped in-memory providers, which stored no scope and filtered by none: under `tenant-b` they returned, **destroyed, and overwrote** `tenant-a`'s records, and a read scoped to one `sessionId` saw a sibling session's. But the seam still cannot enforce scoping itself — `MemoryRecordView` is `{id, principal, content, updatedAt}` and carries **no tenant**, so `MemoryRuntime` has nothing to check a returned record against. A provider that ignores the access context is still believed. | **REWRITTEN 2026-09-06 as an OUTCOME, because the previous wording named a MECHANISM that another contract forbids.** It read "a `scope` field on `MemoryRecordView`" — and P6-01's own frozen cases assert that projection is *exactly* `{id, principal, content, updatedAt}` (one C-stage title, one P-stage title, both GREEN). **The lock demanded an action the epic it locks had already proven impossible.** **Step ①, restated: `MemoryRuntime` can determine which scope each returned record belongs to, and refuses out-of-scope records.** The means are the owner's to choose. Widening `MemoryRecordView` remains one option, but whoever picks it also takes on P6-01's C and P supersessions — that cost belongs to the choice, not to the lock. **Step ② `MemoryRuntime` actually post-filtering by scope.** **OWNER (both steps): P6-03, wave 8.** Its Usage stage already declares `packages/memory/memory/src/index.ts`, and its must[3] (`merge`/`supersede`/`forget`/`export`) plus acceptance[2] are read-path clauses that must post-filter anyway. Doing both steps there keeps this lock openable in one action instead of invalidating two of P6-01's greened cells now and leaving step ② unowned afterwards. **No longer 'owner unassigned'.** | The pinned `CHARACTERIZATION:` case asserting that a hostile provider's out-of-scope records still reach the caller **starts failing**, which happens on step ② only. **The previous wording implied P6-02 would unlock this row; it will not** — P6-02 delivers `MemoryRecord.scope`, a different type from `MemoryRecordView`, and the two must not be conflated. An F stage may make neither change. |
| **P2-03** *(added 2026-09-05; **LIFTED 2026-09-06** by the delegate (`guanjieqiao-92`), both halves closed by DIFFERENT mechanisms — one built, one re-anchored. Its line in the machine-readable list above is removed in the same edit, per this register's own rule.)* | must[2]'s `code-mode-embedded` and `plugin-rpc` halves — "code-mode embedded tools and plugin RPC cannot bypass" | Only `native-tool-call` is wired. **code-mode:** wiring `computeArgumentsHash` into `ptc.ts`'s dispatch makes `ptc.spec.ts`'s deep-arguments case throw `RangeError: Maximum call stack size exceeded`; `canonicalizeArguments` is excluded as the cause (iterative in both `src` and built `lib`, handling the same shapes at depth 5000 and 20000 standalone, including inside a worker), and **the cause is not known** — see [BLOCKED-077](#blocked-077). **plugin-rpc:** the repository contains **no plugin-RPC dispatch point at all** — four textual occurrences, none a call site — so an assertion that it cannot bypass the manifest would pass with the entire gate deleted ([BLOCKED-050](#blocked-050)'s shape). | P2-03's remaining Usage work. **Not P2-03's Fault stage** — F builds evidence for what U delivered, and U has delivered one of three paths. | **code-mode:** `ptc.spec.ts`'s deep-arguments case passes with the manifest wiring in place. **plugin-rpc:** a plugin-RPC dispatch point exists and is gated. Until then the clause is not falsifiable, which is why it is recorded rather than asserted — `assertManifestPrecedesExecution` never branches on `origin`, so a future call surface inherits the gate by construction, but that is a design property, not evidence.  **LIFT EVIDENCE, recorded because the two halves closed for unrelated reasons and a single "lifted" would hide that.** **code-mode half: BUILT.** The unlock signal named above — `ptc.spec.ts`'s deep-arguments case passing with the manifest wiring in place — is MET: the case passes in run 34051669730, and it passes because [BLOCKED-077](#blocked-077) was root-caused (a stale `canonicalize.js` artifact tracked beside its `.ts` source, which the module resolver preferred) rather than routed around. Three code-mode cases in supplement U.1 observe the manifest on the embedded path. **plugin-rpc half: NO LONGER THIS EPIC'S CLAUSE.** It is not built and is not claimed built — the delegation recorded as C12 split must[2] and re-anchored that half to **P1-06 must[4]**, which owns a real plugin-RPC dispatch point. Lifting on a split is legitimate only because the clause moved to an owner with a subject; had it merely been dropped this row would stay. **This lift does not accept P2-03** — the four predicates and the delegate's signature remain separate, and the observation must be retaken at a lint-clean tip.|
| **P5-10** *(added 2026-09-06)* | `must[3]` — 「取消进入 convergence barrier，确认 child/world/actions 停止后才终态」 | Three nouns; only `child` has a runtime subject (`settleRun`). **`ExecutionWorld` exists in this repository solely as a capability-token type name**, with no runtime entity, and no stoppable `actions` entity exists either. The barrier is built over `child` alone and deliberately defines NO interface for the absent suppliers — fixing that shape before P3-01 designs `ExecutionWorld` would create exactly the unread hook this queue has recorded five times. See [BLOCKED-116](#blocked-116). | **P3-01** (一等公民 ExecutionWorld Capability Seam) for `world`, whose `must[0]` defines `execute/terminate/snapshot/restore` — `terminate` is the stoppable operation this barrier needs. **P2-03** or its successor for `actions`; the exact clause is not yet pinned. Both are scheduled, so this is delivery pending, not work unowned. | P3-01's `ExecutionWorld` runtime exists with a terminate path, and the `actions` supplier is identified by clause. P5-10 must THEN write cases observing a world and in-flight actions actually stopping before terminal state — observations with no subject today, which is why the clause is recorded rather than asserted. Four green cells do not lift this row. |
| **P4-05** *(added 2026-09-05)* | `acceptance[2]` — "after a restart an orphaned Agent can be reclaimed or fail safely" | "After a restart" requires agent-run state that survives a process boundary, and **nothing in this repository holds it**. `leaseEpoch` has zero occurrences outside P4-05's own new module; there is no worker registry and no durable agent-run store. What exists is *turn*-level crash recovery (`TurnEndReasonMap.interrupted`, P6-07's `interruptedTurnClosers`) and *Run*-level persistence from P4-01 — neither is Agent-level. The state machine can define the `orphaned` state and its reclaim edges, and does; it cannot produce a record that outlives the process. Building that store here would exceed the pinned source's "only the minimum change needed for this item and its stable capability seam", whose new-file list for P4-05 is `state-machine.ts` and its spec alone. | **P4-07** (Worker Lease, Heartbeat and Fencing Token) — verified in the registry, not assumed: its `must[0]` is "every work item is owned by an epoch lease", `must[2]` is "heartbeat renews the lease; the scheduler may reclaim after expiry", and its `acceptance[0]` is "a recovered old worker cannot commit results or perform new side effects". That is precisely the missing subject. | P4-07's lease store lands, so an orphaned agent has a record that survives a restart and can be reclaimed. **No assertion may be written that passes today** — with no persistence, "reclaimable after restart" is green because nothing can be orphaned across a restart at all, which is [BLOCKED-050](#blocked-050)'s shape exactly. **SECOND STEP, added 2026-09-05 (BLOCKED-092): P4-07 landing does not unlock this row.** It makes the assertion writable. P4-05 must then write a case that observes a lease actually expiring and the scheduler reclaiming the work item — an observation with no subject today, which is why this lock exists. Owner of that case: P4-05's own stage that carries the clause, as a supersession. |
| **P1-07** | 未信任目录不加载项目级执行内容 (must[1], acceptance[0]) **plus two clause failures found during the Usage stage: acceptance[1]'s symlink-retarget half, and must[1]'s safe-read-only half** | (a) Four of must[1]'s five kinds have no project-sourced load site ([BLOCKED-054](#blocked-054)), so acceptance[0] is true **because there is nothing to load, not because the gate stopped it**, leaving the gate with zero regression protection. (b) A granted path was re-canonicalized on every call, so retargeting a granted symlink moved the grant **onto the attacker's directory**, which then matched as a first binding and was trusted — acceptance[1]'s symlink clause verbatim, not an incidental finding. (c) `agent-instructions`' incremental path still injected nested project `AGENTS.md` files reached through a tool touch while untrusted — must[1]'s "only safe reads" verbatim. | P1-07's Usage stage. | **(a) SATISFIED, verified independently**: replacing `skill-filesystem`'s `authorizeProjectLoad` call with `return true` turns the untrusted and trusted-read cases red while the `trusted-execute` control stays green; restoring returns 3/3. **(b) SATISFIED**: grants are resolved exactly once and memoized, with the reason recorded at the code — re-resolving is what let the grant follow its own link. **(c) SATISFIED**: no touched paths are passed when instructions are not permitted, and trust state now participates in the baseline identity so a downgrade recomposes. **All three must hold at close; (b) and (c) are clause failures, never "known limitations".** — **LIFTED 2026-09-04 by the delegate (`guanjieqiao-92`) at close, each condition re-verified against the source rather than against its SATISFIED marker.** (a) the mutation point exists at `skill-filesystem/src/index.ts:255`. (b) `reconcileWorkspaceTrust` demotes to `'untrusted'` on any mismatch of canonicalPath / device / inode / createdAtMs, binding the **observed** identity with no `grantedBy` — a retargeted symlink changes the canonical path and so cannot carry the grant; the `createdAtMs > 0` confirmable guard is intact, so a filesystem reporting no creation time refuses to confirm identity rather than granting trust to whatever now occupies the path. (c) both halves present: `files.ts:313` gates the incremental path on `authorizeProjectLoad(...).permitted`, and `index.ts:139` folds `trustState` into the baseline identity so a downgrade recomposes instead of hitting a stale entry. F is GREEN at run 33897371649 @ `d13ac831db`; all four predicates verified and signed off the same day. |
...
| **P4-06** | **Two clauses.** **(a)** `must[2]` 消费者按 message id/epoch 去重 — the **agent-inbox half only**; the `packages/run/message-bus` half is proven and unaffected. **(b)** `must[0]` 事务性写入 domain event 与 outbox — the **crash path only**. | **(a)** The correct predicate is "the turn that claimed this message committed", and the inbox observes no such event. The rule shipped in its place treated *claimed* as terminal, which forbade `goal-round-driver`'s deliberate restoration of a claimed message whose reservation went stale — a turn that never ran and therefore produced no effect. Reverted, not repaired: choosing the signal (a new session event, a claim/commit pair, or explicit release on restoration) is wider than this cell. See BLOCKED-088. **(b)** The clause asks for one transaction; the write path gives one BATCH. Measured, not assumed: the write-ERROR path is atomic (`appendLines` stats the file and `rollbackAppend` restores the pre-batch size), but a CRASH mid-batch is not — recovery truncates to `truncateTo: committedBytes`, the last COMPLETE record rather than the batch boundary, so the domain event can survive while its outbox record is dropped. That is the unrecoverable direction: replay cannot repair a message it cannot find. Ordering the records first would leave the recoverable failure instead, but the batch is written in `seq` order and the log requires contiguity. See BLOCKED-089. | **(a)** A turn-commitment signal; **ownership undecided** — deliberately not assigned to the party that misread the clause. **(b)** Backend-level batch atomicity, or a change to seq assignment — `packages/session/session-persistence-jsonl`, outside every P4-06 stage's scope. | **(a)** The inbox gains an observable turn-commitment event, and a case asserting that a claimed-then-RESTORED message is still deliverable while a claimed-then-COMMITTED one is not can be written without inventing that signal. **Registered before P4-06 has a single green cell**, because a lock added at four-green depends on remembering to add it, and BLOCKED-080 is the record of what remembered bookkeeping does. **(b)** **RESTATED 2026-09-06 against the reworded clause (decisions-approved.md C13).** must[0] no longer says "transactional" and leaves the mechanism open; it now names one SQLite transaction (`BEGIN IMMEDIATE`) and excludes the storage KV seam. So the unlock is no longer "`appendBatch` gains SOME crash-atomicity guarantee" — it is: **the domain event and its outbox row are written inside one `BEGIN IMMEDIATE` transaction, and a case proves a torn write leaves NEITHER rather than only the first.** The KV seam is now excluded by the clause rather than merely unsuitable in practice, which is what the reword was for: `BEGIN IMMEDIATE` exists in this repository (`session-query-sqlite/src/index.ts:450`) but not on this path, and the clause now says where it must be. acceptance[0] moved with it — the single business effect is guaranteed by the consumer's (messageId, epoch) idempotence, not by the transport, so **no transport-level exactly-once work unlocks this row any more**. **FINDING 2026-09-06, not yet acted on: this row's own "Owner of the fix" column is now STALE for half (b), and it points work outside a scope the reword brought back inside.** The column says `packages/session/session-persistence-jsonl`, "outside every P4-06 stage's scope" — written against the OLD clause, when the requirement was batch atomicity in the JSONL backend. The reworded clause names a SQLite `BEGIN IMMEDIATE` transaction and excludes the KV seam, which relocates the work to a SQLite-backed outbox — and `packages/run/message-bus/src/outbox.ts` **is** declared by P4-06, in both its C and F stages. So half (b) may now be buildable inside this epic's own files rather than blocked on another package. Measured while checking: `BEGIN IMMEDIATE` exists in exactly one place in the repository, `packages/session-query/session-query-sqlite/src/index.ts` (the queue's earlier citation omits the `session-query/` path segment), and nothing on the outbox path uses it. **Not acted on unilaterally**: changing which package owns a locked clause's fix is a scope decision, and the executor reading a stale column as permission is how a lock quietly becomes optional. Delegate confirmation required before building. Half (a) is unaffected and still needs a turn-commitment signal whose ownership is deliberately unassigned. |
...
### BLOCKED-092 — An unlock signal that names only the supplier reads "already proven" instead of "now provable"

**Status: OPEN, systemic across the ACCEPTANCE LOCKS register. Recorded 2026-09-05.**

Every lock states what its unlock will look like from the outside. Reading all five live rows' unlock text in full — not keyword-matching them — shows the register is **inconsistent**, and the inconsistency is worse than a uniform omission:

| Lock | Names the second step? |
|---|---|
| P4-06 | yes — "a case asserting that a torn write leaves NEITHER entry … becomes writable" |
| P2-03 | yes |
...
| **P1-03** *(registered 2026-09-06; **LIFTED 2026-09-07**, both halves, on run 34079177586 @ `a093539442` — 20236 tests, 0 failed. Its line in the machine-readable list above is removed in the same edit, per this register's own rule.)* | `must[2]` 生产 boot 只加载 lock 中已批准且 digest 匹配的插件 — the **product-level half only**; the gate's own decision logic is proven. | `gateProductionBoot` is complete and mutation-proved in both branches, and `apps/cli/src/profile-boot.ts`'s `composeProfile` is its real seam — but **it has zero call sites outside its own package**, so no production boot consults a lock today. The clause's coverage reaches "the gate decides correctly" and cannot reach "a real boot was refused by it". Wiring the call site requires first deciding what a production boot does with an UNLOCKED profile, and every profile in existence is unlocked because lock generation only just became reachable (BLOCKED-094). **acceptance[0] carries the same split**: the offline cold start is structurally satisfied — every input is already-resolved data with no network path to omit — but proven at the decision level, not by a real cold start. | The unlocked-profile policy decision (BLOCKED-094), then a call site in `composeProfile`. Owner: P1-03's Usage stage, as a supersession, once a lock has actually been produced on a real profile. | A case asserting that a profile whose lock was written by `dsh plugin` is REFUSED at boot when a digest drifts. That case cannot be written today, because no lock has been produced on a real profile — which is the lock, stated exactly. **UPDATE 2026-09-06: the must[2] half is BUILT and this row is NOT yet liftable, and the distinction matters enough to state before anyone reads the U supersession as closing it.** `apps/cli/tests/plugin-lock.spec.ts` now drives `composeProfile`'s two entry points against a real profile directory and the real `plugins.lock.json`: a drifted manifest digest is refused, a drifted recorded integrity is refused with the manifest untouched, an untouched install is admitted and reports `verified: true`, and the three BLOCKED-094 policy states each have a case. **acceptance[0]'s product half is NOT covered by any of them.** This row's own text says acceptance[0] is structurally satisfied but "proven at the decision level, not by a real cold start", and that is still exactly true: a real cold start means booting with no network available, which none of these nine cases does and which no file P1-03's U stage declares could do. The must[2] half closing does not carry acceptance[0] with it, and the coverage entries deliberately claim only what the cases reach — writing acceptance[0] in would be the "assertion with no subject" shape this queue has recorded repeatedly. **Whoever lifts this row must lift it on must[2] alone, or find acceptance[0] an owner first.**  **LIFT EVIDENCE, both halves, each naming the case that carries it.** **must[2] product half:** the named blocker was that `gateProductionBoot` had zero call sites, so "the gate decides correctly" and "a real boot was refused" were different claims. `composeProfile` now has both entry points, and U.1's nine cases plus U.2's four drive them against a real profile directory and the real `plugins.lock.json` — a drifted manifest digest refused, a drifted recorded integrity refused with the manifest untouched, an untouched install admitted and reporting `verified: true`, and the three BLOCKED-094 policy states each pinned. **acceptance[0] product half:** F.1's seven cases — the boot closure carries no HTTP client and no egress call, with four of the five gate cases existing to prove that check CAN go red, plus a locked package absent from the local install refused and the same lock with everything present admitted. **One thing this lift does NOT assert, stated because the row's own text invited it:** that a missing cache entry is refused rather than FETCHED. No such branch exists, because no network path exists. acceptance[0] holds because there is nothing to omit, not because a fallback was exercised. **And BLOCKED-135 was found while lifting this row, not after:** the integrity comparison was structurally inert — `dsh.provenance.integrity` was read by two consumers and written by nobody, so every package fell back to a marker that matched itself. Integrity now comes from the profile's own `pnpm-lock.yaml`, and `admitBoot` refuses an unverifiable pair instead of accepting it. Lifting on the original evidence alone would have certified a check that could not fail.||
...
**Inconsistency is the sharper hazard.** If no row named a second step, a reader would learn to expect one. Because three rows name it and two do not, the two silent rows read as though they genuinely need nothing further — **absence of a second step is indistinguishable from "no second step is required."** That is the day's recurring shape once more, in the register that exists to track it.
...
### BLOCKED-096 — An unlock signal that names a MECHANISM can be forbidden by another contract; one that names an OUTCOME cannot

**Status: STANDING RULE, delegate-adopted 2026-09-06. Extends BLOCKED-092.**

BLOCKED-092 required an unlock signal to name a state a test can observe rather than a delivery by another party. P6-01's residual lock satisfied that and still deadlocked, by a second route: its step ① named a **mechanism** — *add a `scope` field to `MemoryRecordView`*.

That mechanism is forbidden. P6-01's own frozen cases assert the projection is *exactly* `{id, principal, content, updatedAt}` — one C-stage title, one P-stage title, both GREEN. **The lock demanded an action the epic it locks had already proven impossible**, and the collision surfaced only when someone tried to perform it.

> **An outcome cannot be forbidden by a contract — at worst it needs a different means. A mechanism can be blocked outright, and the whole lock jams behind it.**

Restated as an outcome (*`MemoryRuntime` can determine each returned record's scope and refuses out-of-scope records*), widening the view stays available as one option — and the C/P supersessions it would cost become part of **choosing that means**, rather than a hidden precondition of the lock.

**A related rule, from the same episode.** The two colliding cases were not edited. Changing an assertion while leaving its frozen title intact is worse than failing to freeze at all:

| Failure | Detectable? |
|---|---|
| Stage never frozen | Yes — a coverage reference matches no frozen set (this is how P1-03's C stage was caught) |
...
| must[2] a **detached** run is held by the Run service and survives UI disconnect | **nothing.** Measured, and re-verified 2026-09-06 with a positive control (the same pattern does find `detachInputSignal`, so the absence is real): `detach` appears 5 times across `packages/workflow/*/src` and `packages/run/*/src`, every one `detachInputSignal` — removing an event listener, unrelated. The word "detached" occurs once in the whole family, in a `nesting.ts` comment explaining that a detached run is **not** nested. No implementation, no coverage entry, no test. |
| must[3] a **nested** run inherits/decays budget, capability and trace, with recursion detection | **decisions only, and they cover one of the clause's three nouns.** Re-verified 2026-09-06: `planNestedRun`/`inheritWorkerLimits`/`applyChildFailure`/`cancelPropagationForNested` are real and tested, but (a) **no production caller** — every reference outside `workflow-registry/` is in `nested-budget.spec.ts`; `runtime.ts` installs globals `agent/parallel/pipeline/phase/log/args` and no `workflow`, with a TRIPWIRE case pinning `typeof workflow === 'undefined'`, so a script cannot start a nested run; and (b) the clause names **budget, capability token and trace**, while `nesting.ts` and `types.ts` contain zero occurrences of `capability`, `capabilityToken`, `trace` or `traceId`. The five `token` hits there are all `tokensRemaining`/`token-budget-exhausted` — a **token budget**, not a capability token. Whoever picks this clause up should read that first: the decided part is one third of it. |
...
### BLOCKED-108 — 76 snapshot tests are red on this branch, and no cell greening could ever have noticed

**Status: OPEN, needs the user. Cause identified; the repair needs a key I must not hold.**

`pnpm run test:snapshot` fails **76 of 115** on the execution branch. The cause is a single session event:

```
73722dd182  feat(first100): P2-03(U) part 2 — native path gated
            → agent-loop appends 'action/manifest-appended'
```

Every recorded session that runs a native tool call now carries a log line no expected output contains, so `snapshots/acp/` and `snapshots/sdk/` reject the replay.

**This is not a regression from the P9 work.** No failing diff mentions `matchTier` or the tier sentence added for P9-04's must[1]; the injected line is `action/manifest-appended` in every case.

**Why nothing caught it, which is the part that generalizes.** `first100-exact-sha.yml` runs `pnpm run typecheck` and the unit suite. It does not run `test:snapshot`, `lint`, or `doc-sync`. So a cell can be greened on an exact-SHA artifact — and its epic ACCEPTED — while a mandatory gate for that very change is red at the same SHA.

> **This is the second finding of the same shape in one session. The lint debt (28 files, 525 errors) accumulated for the same reason. The gate that decides a CELL is not the gate that decides the BRANCH, and every check outside the first is invisible to the program's own definition of done.**

The rule this violated is explicit and predates the work: *every non-trivial model- or product-user-visible change updates a keyless recorded-session snapshot*, and *both SDKs project the loop* — an agent-loop and `SessionEventMap` change updates the TypeScript and Python SDK expected outputs in the same PR. A new logged event is exactly that case.

**Why I cannot repair it — corrected, because the first reason given here was wrong.**

The entry originally said repair needs `DEEPSEEK_API_KEY`, reasoning that expected outputs come from `test:snapshot:record`. That was an assumption from two script names, and reading the harness refuted it. There are THREE modes, not two:

```
DSH_SNAPSHOT unset     replay committed fixtures, compare          keyless
DSH_SNAPSHOT=record    re-record against the live API              needs a key
DSH_SNAPSHOT=refresh   replay committed fixtures, REWRITE expected keyless
```

`pnpm run test:snapshot:refresh` is exactly the repair, it needs no key, and the expected outputs it writes are observed from the same recorded fixtures the replay already trusts — not authored.

**The actual blocker is narrower and is a permission, not a capability:** running it is refused by this session's command classifier, the same gate that refused the `fork` push. So the repair is one approved command away.
...
must[3]: 「取消进入 convergence barrier，确认 child/world/actions 停止后才终态」. Three nouns, and only one has a runtime subject today:

| noun | subject | verdict |
|---|---|---|
| `child` | `settleRun` in `packages/subagent/subagent/src/run-settlement.ts` | **has one** — the barrier can be built on it and observed |
| `world` | `ExecutionWorld` appears in this repository only as a capability-token type name; no runtime entity | **supplier P3-01**, "一等公民 ExecutionWorld Capability Seam", whose must[0] defines `execute/terminate/snapshot/restore` — `terminate` is the stoppable operation this barrier needs |
| `actions` | no stoppable action entity; the ActionManifest is a record, not a running thing | **supplier P2-03** (in flight, 1 of 4 cells) or its successor; the delegate's ruling names it, and the exact clause is not yet pinned |

**This is not "nobody owns it".** Both suppliers exist in the registry and are scheduled; they simply have not delivered. That is a different situation from P4-05's acceptance[2], where step ② had no owner at all, and it is recorded differently for that reason.

**What is being built now (delegate ruling, 2026-09-05):** must[0]/[1]/[2] and acceptance[0]/[2] have subjects and proceed through C/P/U. The convergence barrier is built over `child` alone — the participant that can be observed stopping today.
...
#### Delegate correction: allow-list → deny-list, and why the deny-list is empty

The first ruling defined the default policy set as an allow-list measured from the snapshot corpus. **Corrected by the delegate before any of it was written:** the corpus is a fixture, not the product's whole behaviour, so an allow-list built from it denies the first real user action it never recorded — which is this entry's own defect with the sign flipped. The corpus's role is instead a regression oracle: what worked before the enforcement point was wired must work after, without `--record`.

The replacement shape is a default permit plus a forbid of the kernel hard-deny classes. **That forbid cannot be written against the current wiring, and the default set therefore ships with an EMPTY forbid**, for reasons measured rather than assumed:

- `hardDenyClassesOf` returns exactly `['safety-critical']` for the factory policy — `KERNEL_HARD_DENY_CLASSES` (`risk-taxonomy/src/classify.ts:36`) plus `policy.addedHardDenyClasses`, and `grep -n addedHardDenyClasses packages/bundle/base/cordis.patch.yml` has no hit.
- **`safety-critical` is not in the value domain of anything Cedar receives.** `toCedarRequest` builds a nine-key context — `sideEffectClass, classified, workspaceTrust, permissionPosture, world, tokenPresented, tokenCapability, tokenDelegationDepth, tokenTenant` — and `grep -c "riskClass\|RiskClass" packages/policy/policy-engine-cedar/src/index.ts` returns **0**. The manifest's `ActionSideEffectClass` (`read | write | network | process | destructive`) and `RiskClass` (eight values, including `safety-critical`) are two vocabularies; only the first reaches a policy.
- Falling back to `forbid when sideEffectClass == "destructive"` would deny everything: all 153 manifests in the corpus carry `sideEffectClass: destructive`, `read` and `glob` and `list_agents` included.

**An empty forbid is not an absent defence, and the difference is measured.** The kernel hard-deny band is enforced at the classification layer, not the policy layer: `gateActionRisk` (`core/tools/src/external-effect.ts:221`) returns `{ kind: 'hard-deny' }` on `classification.hardDenied` and records `action/risk-gated`, and it is called from **both** production dispatch paths — `agent-loop/src/tool-calls.ts:270` and `core/tools/src/ptc.ts:693`. A forbid in the policy set would be a second declaration of a live rule, and the two could disagree. So the default set does not restate it, and says so where it is written.

#### Existing is not firing

A live gate and a gate that has ever refused something are different claims, and only the second one is evidence. Measured across the corpus: **145 `action/risk-gated` events, every one of them `allowed-by-preset`** — no `hard-denied`, no `refused`, no `asked`. The risk classes produced are `internal-write` (83), `read` (28), `security-sensitive` (25), `local-reversible` (7) and `external-communication` (2). **`safety-critical` is never produced at all**, which is why `hardDenied` has never been true: the band exists, is read on both dispatch paths, and nothing this profile does lands in it.

Checked separately, because "allowed everything" is also what a broken gate looks like: the 25 `security-sensitive` actions all ran under `danger-full-access`, whose threshold IS `safety-critical`, so permitting them is correct. Under `workspace-write` and `read-only` only classes below their `destructive` threshold appear. **No approval was skipped.** The gate is behaving; it has simply never met the case it exists for.
...
- **Release before the disposer's first `await`. PROBED 2026-09-11, and it WORKS — 10 releases out of 10, none threw.** The probe moved the hand-back to the top of `RunPlugin`'s disposer, synchronous, ahead of the `advance` it currently awaits, and ran the two cases that exercise a lease-holding mount five times each. Every release succeeded, and the case that asserts the item is still held afterwards failed every time — which is the confirmation that the release took effect rather than being skipped. The probe was reverted; `git diff` on the source is empty and the package is back at 194/194.
  ...
  **Why it wins is NOT established, and the difference matters.** The plausible mechanism is that each plugin is its own child fiber, all of those unloads start concurrently, and every one of them begins with `await Promise.resolve()` — so a hand-back done synchronously at the top of our body runs a full microtask before the provider's clear, whatever the list order. That is inference from `vendor/cordis/src/fiber.ts:773` and not something the probe measured. A composition that mounted the two differently, or a provider whose teardown cleared the handle synchronously at a point before its own first await, could invert it. **So this is a candidate with a strong result, not a fix**: adopting it means either establishing the ordering as a guarantee or accepting a behaviour that holds by a one-tick margin nothing enforces.

  One observation about the probe itself, kept because it nearly produced a wrong number: the two passing cases printed NOTHING, and the first reading of that was "zero releases attempted". Vitest's default reporter shows a test's captured stderr only when the test FAILS. The 10/10 is counted from the case that fails under the probe; the passing cases are **unobserved**, not zero.
...
**Measured 2026-09-11: CI installed the broken tarball and the probe never touched it.** Of the two findings this entry deliberately kept apart — "CI could have caught this" and "CI installed it and the probe never touched it" — it is the second.

The probe is one command. `verify-packed-install.ts:111` runs `node <binPath> --version` against `installedEntry = { '@deepseek-ai/dsh', 'lib/bin.js' }` and compares the printed string to the expected version. Nothing else executes.

`--version` never reaches a profile. `apps/cli/src/bin.ts` reaches `runProfile` only through `await import('./profile-boot.ts')` inside the `profile` case, and `args.ts:58` states the rule the code follows: "Help, version, and errors exit inside `parseDshArgs`." The switch is never entered.

**Bundling does not change this, which was the one thing worth checking rather than assuming.** tsdown could have hoisted those dynamic imports into the bin; it did not. `apps/cli/lib/` holds `profile-boot-q7t3XxlF.js`, `plugin-Bh54wIcY.js` and `dump-config-BasVSHOo.js` as separate chunks, and the built `bin.js` still spells all three as `import("./...")`.

Traced with an ESM resolve hook on the **built** artifact — the same `lib/bin.js` the tarball ships:

| plane | modules resolved | headless | `profile-boot` chunk |
|---|---|---|---|
| built `apps/cli/lib/bin.js --version` | 78 | **0** | **0** |
| source `apps/cli/src/bin.ts --version` (tsx) | 183 | **0** | 0 (`packages/bundle/` 0) |

So the missing `lib/stream-json-*.js` can only fail an importer of headless's entry, and the probe never becomes one. The gate would have passed on the broken tarball.

**What this says about the gate is not that it needs tuning.** `release:verify-packed-install` establishes that every tarball installs and that the family's entry bin reports its version. It does not establish that the installed tree runs, and it never claimed to. The defect is probe coverage: one `--version` is a liveness check on the packaging, not on the package.

**Still not run: the pack plus packed install itself.** Deferred to a passing build. `pack.ts:63` fail-closes on a missing `.dsh-build/client-build-environment.json`, and at the tip an attempt stopped on an external registry lookup (`ETARGET @agentclientprotocol/sdk@1.4.0`), one of the local-macOS causes the workflow comment already lists. The load chain cannot differ — same `bin.js`, same chunks, same relative imports — but that is reasoning, and the empirical step belongs on Linux CI.

**The fix is NOT in candidate 1.** Measured 2026-09-11 at overlay tip `125a4c8ec7`: `files[]` is still the four-entry list without `lib/stream-json-*.js`, tsdown still emits `stream-json-Cgk-kN_P.js`, both published entries still import it, and `release:pack --family dsh` there produces a 13-file headless tarball without the chunk. `git merge-base --is-ancestor 5fbcd300af 125a4c8ec7` is false — the fix lives only on the lane-b line. **"FIXED" above is true of this branch and false of that candidate**, and the distinction matters to anyone reading this entry against a gate result. The workflow's `Publication and package hygiene` step runs after Build and is where `publicationClosureViolations` caught this in the first place, so a candidate carrying the defect should go red there rather than publish silently.

**Whether CI installed the broken tarball and said nothing was NOT measured when this entry was first written.** `@deepseek-ai/dsh-headless` is a dependency of the published `@deepseek-ai/dsh` (`apps/cli/package.json`), so `first100-exact-sha.yml`'s `release:verify-packed-install` step does a real npm install that includes it. That step's probe runs one entry — `installedEntry` is `{ packageName: '@deepseek-ai/dsh', binPath: 'lib/bin.js' }` — and whether loading that bin reaches headless's entry depends on whether profiles load lazily, which this entry does not assert either way. Settling it costs one full pack plus packed install at a pre-fix SHA; deferred deliberately rather than guessed, because "CI could have caught this" and "CI installed it and the probe never touched it" are different findings with different fixes.
...
**One consequence reached the generated catalog, and repairing it exposed an older defect.** `LedgerGeneration` appears on three `ctx.actionLedger` method signatures, so `gen-cordis-catalog` reported it as an unclassified type and candidate 3'' failed both `cordis-catalog.spec.ts` cases (run 34679720296). Classified in `typeLinkExemptions` beside its six siblings — `LedgerScope`, `LedgerEpoch`, `LedgerEntry`, `ReceiptDigest`, `ReserveRequest`, `ReserveDecision` are all exempted as owned by the package README, and no `docs/subsystems/*.md` page covers the ledger, so `linkedTypePages` would have meant inventing one for a type alias. Regenerating the artifacts then required refreshing `core.i18n.yaml`, which turned a stale-record complaint into the content divergence it had been masking: `core.zh.md`'s second `type-equiv` block held an `Agent` interface missing every field the first100 epics added (`identity`, `runId`, `lifecycle`, `runLease`, `leaseRefused`, `taskProfile`) — 4147 characters against the English side's 7993, already divergent before this work. The Chinese block now carries the same declaration, and `verify-type-equiv`'s drift set is identical before and after (17 drifts, same files), so the pair was repaired without touching the English-vs-source drift those pages' owner holds.
...
### BLOCKED-225 — P4-12's crash campaign almost never crashed, because its generator lost the low bits of every multiply

**Status:** FIXED 2026-09-12 under the delegate's ruling, owner lane B. Branch `lane-b-225`, on top of 221's `5e0c4502f9`. **Goes into P4-12's re-signing material:** this is the epic's direct acceptance[0] evidence, and it was passing because it did not crash.

`attempt` was driven by `random = (random * 1_103_515_245 + 12_345) % 2_147_483_648`, evaluated in double arithmetic. From the SECOND iteration the product passes 2^53, so the low bits are gone and the modulus yields multiples of large powers of two. Over the campaign's own 10,000 iterations the drawn crash points were:

| crash point | before | after |
| --- | --- | --- |
| `none` | 9956 | 2512 |
| `after-reserve` | 43 | 2403 |
| `after-send` | 1 | 2581 |
| `after-receipt` | **0** | 2504 |

**And the few real crash points landed where they could not matter.** Almost every key reached `confirmed` inside the first 250 attempts, so a later `after-reserve` draw hit a settled entry, got `duplicate`, and returned before reaching its `throw`. The campaign asserted zero duplicates and 250 commits, and both held for a run that performed 10,000 clean attempts.

**The fix is two changes, and the second is the one a reader would skip.** `Math.imul` keeps the multiply in 32 bits, which is the arithmetic the generator is defined over. Taking the LOW bits of the result then looks correct and is not: an LCG's low bits have tiny periods, and this one draws `23012301…`. Against 250 keys visited round-robin, that leaves **every key permanently blind to two of the four crash points** while the aggregate histogram comes out at a perfect 2500 each. So the draw takes the high bits, and the case asserts BOTH a lower bound per crash point AND per-key coverage. A histogram alone cannot tell a uniform generator from a short cycle.

**Each attempt now presents the NEXT generation** (BLOCKED-221): a restart takes a new lease epoch, and repeating one generation would make every retry its own peer and be refused, so the campaign would measure a ledger that refuses rather than one that recovers. `attempt` already took the generation as a parameter.

**An UNFENCED campaign was added beside it, and it records what crash-retry dedup does NOT depend on.** With `'unfenced'` on every attempt the campaign still commits each effect exactly once, because what refuses a second send is the `sent` state a restart reads off the disk, not a generation. What unfenced gives up is CONCURRENT exclusivity, which a sequential campaign is the wrong instrument for — there is never a live peer to confuse with a restart. The two-process case in `store.spec.ts` is where that loss is measured, and it shows both workers holding one reservation.
