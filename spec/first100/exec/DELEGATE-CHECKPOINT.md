...
| 远端单一事实源 | `fork/first100-exec` = **`e74593c50a`**(78 首推;= lane B `c1b872722f` 线 + lane A 批 + docs overlay);dispatch run 34573807027 @ `gate/e74593c50a` **全绿**,on-push run 34576740966 已取消 |
| 最近观测 | 34573807027 绿格:P2-05 C16/P13/U12/F6、P6-01 P31/U21(supersede 后重观测)、P1-10 F15、P4-02 P14;recompute 125/125(artifact 落 `~/first100-delegate/artifacts/`,`FIRST100_ARTIFACT_DIR`) |
| 在飞候选 | **候选基 = lane B 最终 tip `ec317644b5`**(detached 门集全 0);lane A 在其上 `434350688a`(15 笔:P2-05 (a)/BLOCKED-201、P1-10 记录更正、P2-01.U2、backfill、adapt-dispositions 门(补记 47)、P1-07.U 五次重冻、键纪律、`delegatedAt` 归零、cherry-pick lane B 的 211 修 `ecfe9751ca`)+ 正在做:刷语料(129 文件)→ 全量重放 → P2-01 root/child 两条冻结 → detached 门集 → 报 SHA。之后 78:overlay(补记 48–57 + 本文件)叠其上 → `git push fork <full>:refs/heads/gate/<short>` → `gh workflow run first100-exact-sha.yml --ref gate/<short> -f sha=<full>` 一次云跑 → 绿即推同 SHA 到 `first100-exec`、取消 on-push 重复 run |
| ACCEPTED | fork 账本 **26**(含 P2-01,其 WITHDRAWN 在 overlay `delegate-signoff.json` 未推);推后 **25**;**P1-10 PASS 已签**(`062ba7bb84`,overlay)→ 推后 lane A `--accept P1-10`(先核 4.3 两门绿);P2-01 WITHDRAWN(BLOCKED-200);P4-01 WITHDRAWN 重签中(coverage 重做 `bcbfaddc47` 在 lane B 分支,U.1/U.2 观测绿后一次 REPLACE);P2-05 / P4-02 待本次观测后签 |
...
| BLOCKED 号分配 | 已用到 **211**;**212 未分配**;只由 delegate 分配。今日分配:194–196(昨夜)、197 干净卸载=paused、198 mkdir、199 两持久事实、200 P2-01 撤签、201 真策略事实、202 spill fixture(B)、203 姿态(A,原 202 改号)、204 web e2e 无观测、205 审计括号、206 coverage schema 无执行、207 headless files[] 缺 chunk、208 runtime-closure 18 包、209 七空 install、210 hygiene 不在 workflow、211 task-profile ref 不稳定 |
| 已决(用户) | OQ4 = (b);BLOCKED-185 = (A);BLOCKED-187 保持姿态;「继续做吧 明天我看结果」= 自主推进,OQ28 默认(host user id 独立文件 + tenant `local`)生效;验证线(第 4 条线)用户明日考虑 |
| 待用户决定 | ① `exports["./src/*"]`:279/291 已发布包导出指向发布后不存在的目录(publint warning 非 error;358 处工作区消费者)——推荐 pnpm `publishConfig.exports` 在 pack 时剥离或明示接受并按 code 过滤,默认不动,不阻塞任何门;② 验证线开不开(用户已推迟);FYI:210 将给 `first100-exact-sha.yml` 加 `pnpm run hygiene` 一步(紧跟 Build,本机 30s–1.5min) |
