...
**Forward addendum (2026-09-19, lane B; the text above is unchanged).** `4.4d`'s `validation[3]` row reads "closed at F", and that closure is a unit-level one: every member of the closed outcome union has a producer, and the F-stage conformance suite exercises the `timeout` producer directly. It does not say a `timeout` has ever been produced on a production path, and none has: `expireIfOver`/`settle` are reached only from `terminate`, `snapshot` and `sandboxPolicyFor`, three of the five `WorldProvider` methods no production code calls. The measurement, the structural proof that nothing else holds a `WorldHandle` to call them with, and what that does and does not block are recorded in BLOCKED-262. That entry is explicit that it does not dispute this sign-off; the clause stays closed on the ground stated here.
