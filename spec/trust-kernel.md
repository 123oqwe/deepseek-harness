...
1. No plugin unload, service override, or dynamic mount can replace the kernel's policy, audit, or signature verifier. At the Contract stage this is a structural guarantee: `TrustKernel`'s type surface exports nothing shaped like a replaceable Cordis service, and no exported value or function in this package produces one of its three opaque handle types; forging one requires a deliberate, greppable unsafe operation (an `as` cast, `Object.create`, or an unconstrained generic) at the call site, not something this module makes convenient or accidental. Full runtime enforcement — a constructed kernel value surviving an attempted override at boot — is a later, U-stage acceptance case.
