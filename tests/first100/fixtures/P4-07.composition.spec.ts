...
interface ToolObservation {
  ...
  readonly results: readonly { readonly isError: boolean; readonly error: { readonly info?: { readonly name?: string } } | null }[]
...
function expectRefusedToolTurn(observed: ToolObservation): void {
  ...
  for (const result of observed.results) {
    ...
    expect(result.error?.info?.name).toBe('LeaseRefusedError')
  }
}
