 ...
 * sandbox and the browser round trip a real definition needs. The registry
 * judges which Loader entry encloses the caller, not which code made the call,
 * so the driver's own fibers are admitted as the runner's would be: the probe
 * shows that the runner is not refused, not that only the runner can declare.
 */
...
import type { RootRegistrantObservation } from './root-registrant.ts'
...
interface Report {
  ...
  /** What `root-registrant.ts` met, when the composition mounts it. */
  rootRegistrant?: RootRegistrantObservation
}
...
try {
  ...
  const rootRegistrant = ctx.get('p109RootRegistrant') as RootRegistrantObservation | undefined
  if (rootRegistrant !== undefined) report.rootRegistrant = { ...rootRegistrant }
  try {
