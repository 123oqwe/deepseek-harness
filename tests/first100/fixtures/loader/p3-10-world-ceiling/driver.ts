...
/**
 * Hold `mebibytes` of touched memory, then say how much it held. It first
 * reports the memory ceiling of the cgroup it runs in, then each 16 MiB it
 * holds, so a kill can be placed against the ceiling the balloon ran under.
 */
...
const base = cgroup === undefined ? undefined : '/sys/fs/cgroup' + cgroup
console.log('BALLOON ceiling ' + JSON.stringify({
  memoryMax: base === undefined ? 'no cgroup' : read(base + '/memory.max'),
  swapMax: base === undefined ? 'no cgroup' : read(base + '/memory.swap.max'),
}))
const held = []
for (let i = 0; i < Number(process.argv[2]); i += 1) {
  held.push(Buffer.alloc(1024 * 1024, 1))
  if ((i + 1) % 16 === 0) console.log('BALLOON at ' + String(i + 1))
}
