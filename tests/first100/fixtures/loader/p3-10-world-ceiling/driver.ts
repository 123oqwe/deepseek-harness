...
try {
  ...
  ctx.llm.registerAdapter(['p3-10-mock'], new MockAdapter([
    toolCallResponse('fork-bomb', 'bash', {
      command: `${node} fork-bomb.cjs ${String(FORK_ATTEMPTS)}`,
      description: 'fork bomb',
    }),
    toolCallResponse('balloon', 'bash', {
      command: `${node} balloon.cjs ${String(BALLOON_MEBIBYTES)}`,
      description: 'memory balloon',
    }),
    textResponse('done'),
  ]))
