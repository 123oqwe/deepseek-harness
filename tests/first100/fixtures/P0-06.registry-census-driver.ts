 ...
 * (`./p0-06-registry-census.ts`), and whether its migration applied twice
 * returns a probe payload deep-equal to itself. The registry is read through
 * the same module the product imports, so the census is the product's own.
 */
...
import { isDeclaredIdentity } from './p0-06-registry-census.ts'
...
try {
  const entries = listSchemas().map(entry => ({
    ...
    declaredIdentity: isDeclaredIdentity(entry, identityMigration),
