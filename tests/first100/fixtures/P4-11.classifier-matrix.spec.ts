 ...
 * with its policy's `retryableCodes`, and the shipped `llm-retry` row carries
 * no config, so that is the default policy. The HTTP rows take their code
...
/** The codes llm-retry retries under that policy; empty when the policy is not the bounded one. */
const RETRYABLE_CODES: readonly string[] = SHIPPED_POLICY.mode === 'normal' ? SHIPPED_POLICY.retryableCodes : []
...
describe('P4-11 must[0]: the circuit breaker and llm-retry classify every shipped provider failure the same way', () => {
  it('control: the shipped llm-retry policy is the bounded one, with a non-empty list of retryable codes', () => {
    expect(SHIPPED_POLICY.mode).toBe('normal')
    expect(RETRYABLE_CODES.length).toBeGreaterThan(0)
  })

  for (const row of ROWS) {
    it(`${row.label}: classifyFailure and the default retryableCodes give the same verdict`, () => {
      const breaker = classifyFailure(llmFailureFacts(row.failure)).retryable
      const llmRetry = RETRYABLE_CODES.includes(row.failure.code)
