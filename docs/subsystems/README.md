...
| [policy.md](policy.md) | the five declared policy inputs, the closed permit/deny/ask decision and its reason codes, the deny-only plugin constraint, and where a decision becomes enforcement |
...
| [memory.md](memory.md) | the provider-neutral Memory seam: propose/query/get/revise/forget/export, `MemoryAccessContext` read scoping, provider replaceability, and the Memory-vs-Session-Query boundary |
| [control-protocol.md](control-protocol.md) | the SDK control protocol's two negotiations: per-message schema versions against whole-peer protocol version ranges, the mandatory/optional capability split and its two distinct refusals, wire-visible schema fingerprints, and why an absent negotiation field means a peer predating negotiation rather than a peer agreeing to nothing |
| [execution-world.md](execution-world.md) | the ExecutionWorld seam: the nine confinement dimensions a `WorldSpec` decides, the unforgeable `WorldHandle`, the OCI-adapted lifecycle with a terminal `stopped`, the one typed outcome every stop settles to, and why provider selection refuses rather than degrading |
