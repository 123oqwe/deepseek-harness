...
协商的结果——达成的版本、达成的能力、被忽略的能力，以及任何适配器降级——被记录为一条 SDK 连接开出的每一个 Run 的 `provenance`：该连接创建的每个会话的 Run，以及这些会话启动的每个子 agent 的 Run，后者取其父 agent 当前所在的那个 Run 上的那一份，从不取同一会话里已结束的 Run 上的。一个 Run 保留为它记录的第一份 provenance。于是*这两个对端究竟同意了什么*可以仅凭 Run 记录回答，而不需要重放一次已经不存在的握手。在 SDK 连接之外开出的 Run（例如经 ACP）不带 provenance，因为那些连接不做协商。
