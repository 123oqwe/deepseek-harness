...
直接看到的:没有。本子系统不注册工具、不贡献提示词文本、不发出会话事件。模型能观察到的是它的请求究竟有没有发出:`@deepseek-ai/dsh-llm` 在拉取流开头那些 chunk 时咨询断路器，一直拉到第一个不是 `usage` 的 chunk，因为端点的健康正是在那里显形——产出过这样一个 chunk 的流说明端点应答了，而在那里以 finish 块报告的失败与抛出的失败同样判断——而拒绝以该服务自身的失败形式浮现。既不挂载预算也不挂载断路器的组合,其行为与这两者出现之前完全一致;两个消费者都用 `ctx.get` 解析服务,因此"缺席"是能力缺失,而不是权限拒绝。
