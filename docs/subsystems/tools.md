...
Where the Trust Kernel is pinned, a call through `execute()` first appends its ActionManifest to the calling agent's session and is decided by the policy enforcement point, before the capability token is checked: a decision other than permit refuses it, and a call with no agent is decided and refused. Without a kernel it appends no manifest. With or without one, an emergency stop or a run another host took over refuses a call made on behalf of an agent. Where the kernel is pinned, the call then passes the same risk gate after its token is checked. The agent loop and code mode do not reach this step; they record and decide on their own paths and enter the pipeline through the internal scheduler. Every call, on every path, is asked once more whether its agent may still act just before its body is dispatched, after the pre-execute waterfall and any ask, because an emergency stop or a lost lease can arrive while an operator decides; a refused call's result names the reason.
