<!-- 本文件由 scripts/gen-config-catalog.ts 生成，请勿手工编辑。
...
每个 `config:` 块均可由 `cordis.yml` 条目设置：针对每个可加载的 harness 包，原样列出其 `apply` 函数或服务构造函数接收的配置声明（包括 JSDoc），并附上所有引用类型——包内类型直接粘贴，其他类型则提供链接。粘贴的内容是插件声明的完整配置类型——运行时 schema 有意排除的字段是仅供运行时使用的 seam（其自身的 JSDoc 会如此说明），不能通过 `cordis.yml` 设置。这是以**部署**为轴的参考文档——插件作者所依据的连接方式请参阅各[子系统页面](subsystems/core.zh.md)中的生成 Cordis API 区域，面向模型的工具 schema 请参阅[工具目录](tool-catalog.zh.md)，而 [subsystems/](subsystems/core.zh.md) 则记录了这些声明所引用的类型。

本文件由源代码（`scripts/gen-config-catalog.ts`）生成，并通过 `pnpm run verify-config-catalog`（`doc-sync` 的一部分）验证新鲜度，请勿手工编辑。声明块使用 `ts config-catalog` 围栏（doc-typecheck 会跳过它，因为单独引用导入项的声明无法独立编译）。生成器还会将运行时 schemastery schema 与粘贴的声明进行交叉核对——每个经 schema 验证的键（包括嵌套键）都必须能在声明的配置类型中找到——因此，粘贴内容无法隐藏加载器接受的字段。
...
来源： [`packages/acp/acp/src/index.ts:78`](../packages/acp/acp/src/index.ts)
...
来源： [`packages/core/agent-loop/src/index.ts:381`](../packages/core/agent-loop/src/index.ts)
...
来源： [`packages/guard/baseline-preflight/src/index.ts:33`](../packages/guard/baseline-preflight/src/index.ts)
...
来源： [`packages/policy/capability-token-file/src/index.ts:51`](../packages/policy/capability-token-file/src/index.ts)
...
<a id="deepseek-aidsh-job-settle-signal"></a>
...
来源： [`packages/test-support/job-settle-signal/src/index.ts:25`](../packages/test-support/job-settle-signal/src/index.ts)
...
依赖： [`ApprovalPolicy`](subsystems/approval.zh.md) · [`RiskClass`](../packages/policy/risk-taxonomy/src/index.ts) · [`RiskPolicyRule`](../packages/policy/risk-taxonomy/src/index.ts) · [`SandboxMode`](subsystems/sandbox.zh.md)
...
来源： [`packages/policy/policy-engine-cedar/src/index.ts:43`](../packages/policy/policy-engine-cedar/src/index.ts)

<a id="deepseek-aidsh-policy-language"></a>
...
来源： [`packages/reliability/retry-cockatiel/src/index.ts:30`](../packages/reliability/retry-cockatiel/src/index.ts)

<a id="deepseek-aidsh-run"></a>

## `@deepseek-ai/dsh-run`

需要： `agents` · `leaseStore`
...
来源： [`packages/sandbox/sandbox-local/src/index.ts:52`](../packages/sandbox/sandbox-local/src/index.ts)
...
来源： [`packages/storage/storage-json/src/index.ts:40`](../packages/storage/storage-json/src/index.ts)
...
来源： [`packages/storage/storage-sqlite/src/index.ts:26`](../packages/storage/storage-sqlite/src/index.ts)
...
来源： [`packages/subagent/subagent-taskboard/src/index.ts:52`](../packages/subagent/subagent-taskboard/src/index.ts)
...
来源： [`packages/run/taskboard-sqlite/src/index.ts:35`](../packages/run/taskboard-sqlite/src/index.ts)
...
来源： [`packages/core/tools/src/index.ts:1013`](../packages/core/tools/src/index.ts)
...
- `@deepseek-ai/dsh-command-workspace-trust` — 需要 `commands`（[`packages/workspace/command-workspace-trust/src/index.ts`](../packages/workspace/command-workspace-trust/src/index.ts)）
...
- `@deepseek-ai/dsh-policy-enforcement`（[`packages/policy/policy-enforcement/src/index.ts`](../packages/policy/policy-enforcement/src/index.ts)）
...
- `@deepseek-ai/dsh-subagent` — 需要 `messageBus`（[`packages/subagent/subagent/src/index.ts`](../packages/subagent/subagent/src/index.ts)）
...
- `@deepseek-ai/dsh-workflow-filesystem` — 需要 `workflowEngine`（[`packages/workflow/workflow-filesystem/src/index.ts`](../packages/workflow/workflow-filesystem/src/index.ts)）
...
- `@deepseek-ai/dsh-control-plane`（[`packages/interaction/control-plane/src/index.ts`](../packages/interaction/control-plane/src/index.ts)）
...
- `@deepseek-ai/dsh-human-channel`（[`packages/interaction/human-channel/src/index.ts`](../packages/interaction/human-channel/src/index.ts)）
...
- `@deepseek-ai/dsh-secrets-broker`（[`packages/credentials/secrets-broker/src/index.ts`](../packages/credentials/secrets-broker/src/index.ts)）
...
- `@deepseek-ai/dsh-task-profile`（[`packages/run/task-profile/src/index.ts`](../packages/run/task-profile/src/index.ts)）
