...
**策略文档必须原子替换——写到同级路径再 `rename`——而不是就地截断重写。**

理由是基线特有的。settings provider 监视文档、变化即重解析;而"先截断再写"留下一个窗口,watcher 可能在其中读到一份空的或残缺的文件。**在没有组合基线时这个窗口是无害的**:空文档解析成空策略集,`parsePolicySet` 以 `empty` 拒绝它,namespace 保留上一份被接受的值。**有了基线就不再无害**:空文档解析成"只剩基线",而那是一份**合法**的策略集,于是它被接受并提交——在写入完成之前,部署自己的策略消失了。**没有任何东西失败,所以"保留上一有效"这条路径根本不会被走到。**

真 boot 实测:namespace 从 `baseline-permit, shipped-forbid` 掉成只剩 `shipped-forbid`,钉子随之改变——那是一次**成功提交**,不是一次拒绝。

这个窗口受 watcher 的 debounce 限制,所以快速的编辑器通常在 watcher 看之前就关上了它。这使它是一场**竞态**而不是必然——而竞态是更难被发现的那一类,因为它间歇且静默。已作为 BLOCKED-243 记在 settings provider 名下;它不是策略特有的:任何带组合 base 的 namespace 都有同样的暴露面。
