...
`pnpm baseline:capture` freezes a checkout's architecture/protocol fingerprint into `.dsh/baseline.json`; `pnpm baseline:verify` re-derives it and reports drift (`scripts/release/baseline-fingerprint.mjs`, format documented in a generated `docs/audit/baseline-fingerprint-<gitSha>.md`, one per captured commit). `@deepseek-ai/dsh-baseline-preflight` runs the same check at boot and aborts application startup on drift instead of letting an execution batch run against a stale fingerprint. The shared `dsh` base bundle carries the plugin's row `disabled: true`, opt-in per profile, since this repo's own committed `.dsh/baseline.json` permanently trails real `HEAD` and an always-on row would abort ordinary `pnpm dsh` usage here. See the [package README](../packages/guard/baseline-preflight/README.md) for the captured fields, the no-captured-baseline no-op case, and how to enable the row.
