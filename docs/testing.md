...
`pnpm baseline:capture` freezes a checkout's architecture/protocol fingerprint into `.dsh/baseline.json` (Git SHA, toolchain, workspace package names, default bundle row ids, protocol/event schema hashes, the pnpm lockfile hash); `pnpm baseline:verify` re-derives the same fields and reports drift (`scripts/release/baseline-fingerprint.mjs`, format documented in a generated `docs/audit/baseline-fingerprint-<gitSha>.md`, one per captured commit — e.g. `docs/audit/baseline-fingerprint-0a53fb55bea101816fa226bb964ae2bed71c343b.md`). `@deepseek-ai/dsh-baseline-preflight` runs the same check at boot, through every profile in the shared `dsh` base bundle: a checkout with a captured baseline that has drifted aborts application startup instead of letting an execution batch run against a stale fingerprint; a checkout with no captured baseline (an end-user project, most test fixtures) has nothing to verify and boots unaffected. See the [package README](../packages/guard/baseline-preflight/README.md) for the exact drift-detection contract shared with the CLI.
