...
Two gates judge a profile's bundle layers before any patch reaches `boot()`, both inside `composeProfile` (`apps/cli/src/profile-boot.ts`), so a rejected layer's plugin code never mounts at all. Pre-mount admission classifies each layer's own `package.json` `dsh` field and applies only under `DSH_PLUGIN_MANIFEST_ENFORCEMENT=enforce`. Compatibility negotiation then solves every admitted layer's declared `dsh.compat` manifest as one graph through [`plugin/plugin-compat`](../packages/plugin/plugin-compat/README.md) — runtime API range, schema ranges, required and optional capabilities, and provider constraints — and composes only the layers its solved load plan marks active. A layer blocked on its own ranges also stops satisfying the capabilities its consumers require, so no plugin composes against a provider that never mounts, and a graph with no solution aborts the launch with the minimal conflicting constraint set instead of picking a side. Negotiation needs no opt-in: a bundle declaring no `dsh.compat` is unconstrained and always active, while a present but malformed `dsh.compat` fails loud.
