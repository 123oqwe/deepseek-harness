...
`DeepSeekHarnessConfig.capabilities` and `HarnessClient.initialize()`'s `capabilities` are the same ask, and the handshake is the only place to make it: the server decides per connection what it will send. Declare `CapabilityDeclaration(HOST_CONTROL_CAPABILITY)` to receive `host.control`, the host-wide emergency stop. `DeepSeekHarness.handshake` (and `InitializeResponse.hostControl` on the low-level call) then carries the state as of the handshake — the only way a client that connected after a stop learns of it — and `negotiation.agreedCapabilities` says whether the server agreed at all. Absence of `hostControl` means UNKNOWN, never "not stopped". Because the stop is host-wide the notification carries no `sessionId`, and a session-scoped subscription receives it anyway.

`InitializeResponse` carries the whole negotiation record: `protocolVersions`, `schemaFingerprint`, and `negotiation.downgrades` as `CapabilityDowngrade` models (a missing list reads as `[]`). Every model in the reply keeps the fields it does not declare, so an optional field a newer runtime adds stays readable on the model (`model_extra`) instead of being dropped ([control protocol](../../docs/subsystems/control-protocol.md)).
