...
两种载体执行相同的 `dsh` 语法，运行四个随附 profile：`sdk`、独立的 `sdk-minimal` 配置树、`headless` 与 `acp`。`web` profile 不能在这两种载体上运行。私有 `dsh-python-runtime-closure` manifest（元数据清单）定义打包依赖闭包，并声明这四个 profile 启动时加载的每个工作区包；不存在 Python 专用 Node 应用或检入的默认 `cordis.yml`。
