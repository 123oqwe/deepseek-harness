...
Both carriers execute the same `dsh` grammar and run four shipped profiles: `sdk`, the standalone `sdk-minimal` tree, `headless` and `acp`. The `web` profile does not run on either carrier. The private `dsh-python-runtime-closure` manifest defines the packaged dependency closure and declares every workspace package those four profiles load at start; there is no Python-specific Node application or checked-in default `cordis.yml`.
