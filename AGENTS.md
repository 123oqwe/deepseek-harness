...
- **The Trust Kernel is the one non-plugin exception.** Root identity, signature roots, policy enforcement, audit append, and the secret-broker/sandbox-attestation handles are pinned with `pinTrustKernel` before any entry mounts, freezing the service-store entry `ctx.provide` registers. Everything else (models, tools, storage, workflow, memory, UI) stays an ordinary plugin. Open bypasses still let a plugin forge both `ctx.trustKernel` and `ctx.get('trustKernel')` ([boundary](docs/architecture/trust-kernel-boundary.md#known-gap-fiber-local-property-access-poisoning)).
