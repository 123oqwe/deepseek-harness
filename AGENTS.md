...
- **The Trust Kernel is the one non-plugin exception.** Root identity, signature roots, policy enforcement, audit append, and the secret-broker/sandbox-attestation handles are pinned with `pinTrustKernel` before any entry mounts, which freezes its service-store entry alongside `ctx.provide` — no plugin unload, override, or dynamic mount can reach `ctx.get('trustKernel')`. Everything else (models, tools, storage, workflow, memory, UI) stays an ordinary plugin. A known, still-open gap lets a plugin locally poison what `ctx.trustKernel` *property access* (not `ctx.get`) resolves to within its own subtree ([boundary](docs/architecture/trust-kernel-boundary.md#known-gap-fiber-local-property-access-poisoning)).
