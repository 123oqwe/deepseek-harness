...
- **A capability seam comprises Service Definition / Service Provider / Consumer roles**, never one alone. `architecture.layers.json` declares each family; `pnpm run architecture:seams` enforces deep-import and provider/app-dependency rules ([glossary](docs/glossary.md#capability-seam)).
- **The Trust Kernel is the one non-plugin exception.** Root identity, signature roots, policy enforcement, audit append, and the secret-broker/sandbox-attestation handles are pinned with `pinTrustKernel` before any entry mounts. Everything else (models, tools, storage, workflow, memory, UI) stays an ordinary plugin. `ctx.get('trustKernel')` is always correct; bare `ctx.trustKernel` access has a cross-plugin residual, CI-gated unreachable, not closed in Cordis ([boundary](docs/architecture/trust-kernel-boundary.md#known-residual-cross-plugin-property-access-poisoning)). Any epic wiring a real policy/audit/signature-verifier enforcement point requires the vendored `Fiber` fix first (hard prerequisite, ~W6).
