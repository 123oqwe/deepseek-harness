...
- **The Trust Kernel is the one non-plugin exception.** Root identity, signature roots, policy enforcement, audit append, and the secret-broker/sandbox-attestation handles are pinned with `pinTrustKernel` before any entry mounts, closing every plugin-reachable forgery path. Everything else (models, tools, storage, workflow, memory, UI) stays an ordinary plugin. `ctx.get('trustKernel')` is fully protected; only a plugin's subtree can poison its `ctx.trustKernel` view ([boundary](docs/architecture/trust-kernel-boundary.md#known-residual-self-subtree-property-access-poisoning)).
