...
- **The Trust Kernel is the one non-plugin exception.** Root identity, signature roots, policy enforcement, audit append, and the secret-broker/sandbox-attestation handles are pinned with `ctx.provide` before any entry mounts — no plugin unload, override, or dynamic mount can reach them. Everything else (models, tools, storage, workflow, memory, UI) stays an ordinary plugin ([boundary](docs/architecture/trust-kernel-boundary.md)).
