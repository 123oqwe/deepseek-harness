...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/execution/execution-world': { kind: 'none', reason: 'Epic P3-01 seam vocabulary: types plus pure lifecycle and selection decisions over caller-supplied specs; no provider exists to create a world, and the package registers no prompt, schema, tool, or session event. A refused world reaches a model only as its enforcement point\'s closed reason code.' },
  'packages/execution/resource-budget': { kind: 'none', reason: 'Epic P3-10 budget vocabulary and accounting: types plus a pure in-process ledger over caller-supplied reservations; nothing samples or enforces yet, and the package registers no prompt, schema, tool, or session event. A refused reservation reaches a model only through the enforcement point that asked.' },
  'packages/credentials/secrets-broker': { kind: 'none', reason: 'Epic P3-06 lease vocabulary: types plus pure redemption, expiry and revocation decisions over caller-supplied records; the package registers no prompt, schema, tool, or session event, and a refused redemption reaches a model only as its enforcement point\'s closed reason code.' },
  ...
  'packages/interaction/human-channel': { kind: 'none', reason: 'Epic P2-12 vocabulary: the control verbs, the stop record and the question/approval separation as types only; a question reaches a model through the asking tool\'s own result, and this package registers no prompt, schema, tool, or session event.' },
  'packages/interaction/control-plane': { kind: 'none', reason: 'Epic P2-12 decisions: pure functions over a caller-supplied control state and waiting-point registry; a refusal reaches a model through its enforcement point, and this package registers no prompt, schema, tool, or session event.' },
  ...
  'packages/run/task-profile': { kind: 'none', reason: 'Epic P4-02 vocabulary plus one pure compiler over a closed input; it registers no prompt, schema, or tool. It DECLARES the `run/task-profile` session event but appends none — `@deepseek-ai/dsh-run` does — and no path puts a compiled profile into a model request until P4-03 reads one.' },
  'packages/run/run-plan': { kind: 'none', reason: 'Epic P4-03 vocabulary: the compiled plan\'s types, mirroring `spec/run-plan.schema.json`. It registers no prompt, schema, tool, or session event, no profile mounts it, and nothing compiles a plan yet — the reader that would put a plan into a model request is a later stage of the same epic.' },
  'packages/policy/policy-language': { kind: 'none', reason: 'Epic P2-10 vocabulary: the Cedar entity types and context keys a policy may name, as constants. It registers no prompt, schema, tool, or session event, and a policy decision reaches a model only as the closed reason code its enforcement point returns.' },
  'packages/collaboration/control-priority': { kind: 'indirect', reason: 'One table and one comparator; the order it produces is model-visible only through the inbox batch that dsh-agent claims with it, and that ordering is documented there.' },
  ...
  'packages/policy/policy-enforcement': { kind: 'none', reason: 'Epic P2-05 enforcement point: it decides an action for a dispatch path and registers no tool, prompt text or session event. A refusal reaches the model as that path\'s own error result, carrying a closed reason code.' },
  'packages/policy/policy-engine-cedar': { kind: 'none', reason: 'Epic P2-05 policy provider: it answers an enforcement point\'s question and registers no tool, prompt text or session event. What a model sees is that point\'s refusal, carrying a closed reason code and never policy text.' },
  'packages/policy/policy-engine': { kind: 'none', reason: 'Epic P2-05 decision vocabulary: types and pure composition over caller-supplied values. A decision reaches a model only as its enforcement point\'s refusal, which carries a closed reason code and never policy text — that redaction is must[3] and is documented at the enforcement point.' },
  'packages/plugin/plugin-migrations': { kind: 'none', reason: 'Epic P1-10 decisions: the migration DAG vocabulary and pure judgements over it, plus the transaction that orders a backend facet\'s effects. It registers no prompt, schema, tool, or session event; the upgrade it decides runs in `dsh plugin`, before any agent starts.' },
  'packages/reliability/retry': { kind: 'none', reason: 'Epic P4-11 capability definition: the failure taxonomy, the run budget and the breaker Service Definition, all pure over caller-supplied values. What the model sees is a request that was or was not made, which the LLM service owns.' },
  'packages/reliability/retry-cockatiel': { kind: 'none', reason: 'Epic P4-11 breaker provider: it refuses or admits an attempt before the adapter is reached, registering no prompt, schema, tool, or session event. A refusal surfaces as the LLM service\'s own failure, documented there.' },
  ...
  'packages/policy/capability-token': { kind: 'none', reason: 'Epic P2-02 token contract: pure attenuation and verification decisions; no return value is model-facing, and the tool gate that consults them belongs to dsh-tools.' },
  'packages/run/lease': { kind: 'none', reason: 'Epic P4-07 lease: fencing decisions and an in-memory store, registering no prompt, schema, tool, or session event.' },
  'packages/run/message-bus': { kind: 'none', reason: 'Epic P4-06 bus: outbox/inbox decisions and a dispatch loop over caller-supplied state, registering nothing model-facing.' },
  'packages/run/run': { kind: 'none', reason: 'Epic P4-01 run state machine: pure transition functions; the runs it describes are surfaced by their own consumers.' },
  'packages/schema/schema-registry': { kind: 'none', reason: 'Epic P0-06 registry: an internal versioning and negotiation library with no tool surface and no model-visible output.' },
  'packages/collaboration/workflow-journal': { kind: 'none', reason: 'Epic P4-08 journal: resume decisions and compaction over a recorded journal, registering no prompt, schema, tool, or session event.' },
  'packages/workflow/workflow-registry': { kind: 'none', reason: 'Epic P4-09 registry: definition resolution and nesting decisions; a definition body is treated as opaque data and never rendered.' },
  'packages/workflow/workflow-filesystem': { kind: 'none', reason: 'Epic P4-09 loader: reads definition files and registers them; a definition body is opaque data here and is never rendered, and what it loads reaches a model only through the workflow tool.' },
  'packages/workspace/workspace-trust': { kind: 'none', reason: 'Trust transitions are pure decisions over caller-supplied state; the prompts that mention trust are owned by their own rows.' },
  ...
  'packages/policy/grant-store': { kind: 'none', reason: 'Epic P2-08 first slice: the grant vocabulary plus pure validation and predicate-only matching over caller-supplied grants, and an in-process store and fail-closed worker view; the package registers no prompt, schema, tool, or session event, and a refused action reaches a model only as its enforcement point\'s closed reason code.' },
  'packages/policy/risk-taxonomy': { kind: 'none', reason: 'Epic P2-04 taxonomy: a frozen risk-class table and one pure classifier over caller-supplied values, registering no prompt, schema, tool, or session event; what a model reads about a classification is the consuming Consumer\'s own text.' },
  ...
  'packages/session/session-lifecycle': { kind: 'none', reason: 'The package exports types and pure lifecycle-function signatures only and registers nothing model-facing.' },
