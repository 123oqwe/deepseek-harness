...
const PACKAGE_LIBRARIES: Readonly<Record<string, string>> = {
  ...
  'packages/policy/risk-taxonomy': 'Contract-stage type declarations and a pure classifier function only; no plugin entry or mount configuration.',
  'packages/run/run-plan': 'Contract-stage type declarations mirroring spec/run-plan.schema.json; no plugin entry or mount configuration.',
  'packages/run/task-profile': 'Contract-stage type declarations and one pure compiler function only; no plugin entry or mount configuration.',
