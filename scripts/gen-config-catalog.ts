...
const OUT_ZH = 'docs/config-catalog.zh.md'

/**
 * The prose that differs between the two languages.
 *
 * Everything else in this catalog is a package name, a source path, or a
 * verbatim TypeScript declaration — identical in both files by construction.
 * The Chinese side used to be a hand copy of the whole document, which meant a
 * regeneration moved one side of a bilingual pair and left the other behind:
 * nine package sections had accumulated on the English side alone by
 * 2026-09-11 (BLOCKED-177). Generating both from one table ends that class of
 * drift; what a translator owns is this table.
 */
interface CatalogLocale {
  ...
  /** The language switch line. */
  readonly languageLine: string
  /** The three intro paragraphs, in order. */
  readonly intro: readonly string[]
  /** Label before a plugin's injected service keys. */
  readonly requires: string
  /** Label before a config type's referenced types. */
  readonly dependsOn: string
  /** Label before a declaration's source link. */
  readonly source: string
  /** In a terse list line, the ` — requires ` connector. */
  readonly terseRequires: string
  /** In a terse list line, the ` — abstract ` connector. */
  readonly terseAbstract: string
  /** Wraps the source link on a terse list line. */
  readonly terseLink: (inner: string) => string
  /** The three trailing section headings, each with its own intro line. */
  readonly sections: {
    readonly noConfig: readonly [string, string]
    readonly seam: readonly [string, string]
    readonly library: readonly [string, string]
  }
  /** Rewrites an in-repo doc link to this locale's counterpart. */
  readonly localizeLink: (target: string) => string
}
...
const EN: CatalogLocale = {
  ...
  languageLine: 'English | [中文](config-catalog.zh.md)',
  intro: [
  ...
  source: 'Source:',
  terseRequires: ' — requires ',
  terseAbstract: ' — abstract ',
  terseLink: inner => ` (${inner})`,
  sections: {
    noConfig: ['## Loadable plugins with no config', 'These load from a `cordis.yml` entry with no `config:` block; they declare no configuration API.'],
    seam: ['## Seam packages (not directly loadable)', 'Abstract service classes — a deployment loads a concrete implementation package instead ([capability seams](../.agents/notes/implemented/architecture/2026-06-13-capability-seams.md)).'],
    library: ['## Library packages (no plugin entry)', 'Imported as libraries by other packages; a `cordis.yml` cannot load them.'],
  },
  localizeLink: target => target,
}
...
const ZH: CatalogLocale = {
  ...
  title: '# 插件配置目录',
  languageLine: '[English](config-catalog.md) | 中文',
  intro: [
  ...
  terseLink: inner => `（${inner}）`,
  sections: {
    noConfig: ['## 无配置的可加载插件', '这些插件通过 `cordis.yml` 中不含 `config:` 块的条目加载；它们未声明任何配置接口。'],
    seam: ['## Seam 包（不可直接加载）', '抽象服务类——部署时应改为加载具体的实现包（参见[能力 seam](../.agents/notes/implemented/architecture/2026-06-13-capability-seams.zh.md)）。'],
    library: ['## 库包（无插件入口）', '由其他包作为库导入；`cordis.yml` 无法加载它们。'],
  },
  // Only in-repo Markdown targets have a counterpart; a source-file link does not.
  localizeLink: target => (target.endsWith('.md') && !target.endsWith('.zh.md') ? `${target.slice(0, -3)}.zh.md` : target),
}
...
function requiresLine(inject: string[], locale: CatalogLocale): string {
  return inject.length ? `${locale.requires} ${inject.map(k => `\`${k}\``).join(' · ')}` : ''
}
...
function refLink(ref: TypeRef, byName: Map<string, CatalogEntry>, locale: CatalogLocale): string {
  ...
  if (page) return `[\`${ref.alias}\`](${locale.localizeLink(`subsystems/${page}`)})`
  if (target) return `[\`${ref.alias}\`](../${target.entry})`
  return `\`${ref.alias}\` (\`${ref.specifier}\`)`
}

/** Render one configurable plugin's section. */
function renderConfigEntry(entry: CatalogEntry, byName: Map<string, CatalogEntry>, locale: CatalogLocale): string[] {
  const out = [`<a id="${githubSlug(entry.pkg)}"></a>`, '', `## \`${entry.pkg}\``, '']
  const requires = requiresLine(entry.inject, locale)
  if (requires) out.push(requires, '')
  out.push('```' + FENCE, ...(entry.pastes ?? []).map(p => p.text).join('\n\n').split('\n'), '```', '')
  if (entry.refs && entry.refs.length > 0) {
    out.push(`${locale.dependsOn} ${entry.refs.map(r => refLink(r, byName, locale)).join(' · ')}`, '')
  }
  const source = entry.pastes?.[0]?.source ?? entry.entry
  out.push(`${locale.source} [\`${source}\`](../${source.split(':')[0]})`, '')
  return out
}

/** Render one terse list line (the no-config / seam / library sections). */
function renderTerse(entry: CatalogEntry, detail: string, locale: CatalogLocale): string {
  const requires = entry.inject.length ? `${locale.terseRequires}${entry.inject.map(k => `\`${k}\``).join(' · ')}` : ''
  const link = `[\`${entry.entry}\`](../${entry.entry})`
  return `- \`${entry.pkg}\`${detail}${requires}${locale.terseLink(link)}`
}

/**
 * Render the full catalog in one language (pure, deterministic given sorted
 * entries).
 * @param entries - the collected catalog entries, already sorted.
 * @param locale - the prose table; everything else is identical between languages.
 * @returns the whole document.
 */
export function render(entries: CatalogEntry[], locale: CatalogLocale = EN): string {
  const byName = new Map(entries.map(e => [e.pkg, e]))
  const lines: string[] = [
    ...locale.banner,
    '',
  ...
  for (const paragraph of locale.intro) lines.push(paragraph, '')
  for (const entry of entries.filter(e => e.kind === 'config')) {
    lines.push(...renderConfigEntry(entry, byName, locale))
  }
  lines.push(
    locale.sections.noConfig[0],
    '',
    locale.sections.noConfig[1],
    '',
    ...entries.filter(e => e.kind === 'no-config').map(e => renderTerse(e, '', locale)),
    '',
    locale.sections.seam[0],
    '',
    locale.sections.seam[1],
    '',
    ...entries.filter(e => e.kind === 'seam').map(e => renderTerse(e, `${locale.terseAbstract}\`${e.className ?? ''}\``, locale)),
    '',
    locale.sections.library[0],
    '',
    locale.sections.library[1],
    '',
    ...entries.filter(e => e.kind === 'library').map(e => renderTerse(e, '', locale)),
    '',
  )
  return lines.join('\n')
}
...
function main(): void {
  ...
  // Both languages, always together: generating one and leaving the other is
  // what put nine package sections on the English side alone (BLOCKED-177).
  const outputs = [{ path: OUT, content: render(entries, EN) }, { path: OUT_ZH, content: render(entries, ZH) }]
  if (process.argv.includes('--check')) {
    const stale = outputs.filter((output) => {
      let committed: string | null = null
      try {
        committed = readFileSync(resolve(root, output.path), 'utf8')
      } catch {
      ...
      return committed !== output.content
    })
    if (stale.length === 0) {
      console.log(`gen-config-catalog: ${OUT} and ${OUT_ZH} are up to date.`)
      process.exit(0)
    }
    console.error(
      `gen-config-catalog: ${stale.map(output => output.path).join(' and ')} stale. `
      + 'Run `pnpm run gen-config-catalog` and commit both catalogs.',
    )
    process.exit(1)
  }
  for (const output of outputs) writeFileSync(resolve(root, output.path), output.content)
  console.log(`gen-config-catalog: wrote ${OUT} and ${OUT_ZH}.`)
}
