...
function renderMarkdown(ledger) {
  ...
  lines.push('')
  lines.push('This table lists main cells only: a supplement cell (`U.1`, `C.2`, ...) has no row and no column here, so a row\'s candidate SHA can lag a supplement greened later. `ledger.json` carries them under `rows[<id>].supplements`.')
  lines.push('')
...
/**
 * Write `ledger.md` from the committed `ledger.json`, changing no state.
 *
 * Every other command renders the table as a side effect of a ledger write, so
 * a change to `renderMarkdown` alone had no way to reach the file until the
 * next green. This is that render alone: it reads the ledger and writes the
 * Markdown; it never writes `ledger.json`, EXEC-STATE or a digest.
 */
function cmdRender() {
  if (!existsSync(LEDGER_PATH)) {
  ...
  renderMarkdown(loadJson(LEDGER_PATH))
  console.log(`render: ${LEDGER_MD_PATH} written from ${LEDGER_PATH}, which is not changed`)
  process.exit(0)
}
