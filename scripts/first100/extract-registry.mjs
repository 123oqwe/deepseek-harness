...
const MATRIX_SHA = '26dcd1f9ffcdb9a1c01081c2050a29de6213b5e523d38ad7567f66a45f393717'
...
const CLAUSE_REWORDS = {
  ...
  'P0-06': [
    {
      clause: '审计基线产生的 v0 会话日志都能读出,包括基线自己写的 24 个 descriptor v2 子 agent 日志;例外是三类形状不完整的文件,产品明确拒绝它们并给出可读的原因:首个 step 之前出现 surface 的(24 个),只有 chunk、没有 turn 的回放片段(3 个),基线测试夹具写出的投影(4 个,一个是检查点消息缺 compaction/start,三个是 request/header 的 tools 只有名字)。',
      rewordedFrom: '至少能够读取审计基线产生的旧 session fixture。',
      channel: 'acceptance',
      rewordedAtUtc: '2026-09-24T07:33:26.000Z',
      basis: 'C20 (spec/first100/exec/decisions-approved.md): the user approved this on 2026-09-24, answering directly in the delegate\'s terminal ("7 a，8 2（…），9 1"), recorded as WORKING-MODEL §12 "S8 已批准的收窄(再续)" ⑩; the question and the delegate\'s recommendation are channel/B-CLASS-QUESTIONS-2026-09-24.md, second batch, question 9 (option 1), superseding ③ of C19; lane A executed it under A-356.',
      evidence: 'Supersedes C19\'s reword of this clause (③). The audit baseline b150a551 holds 157 version-0 session logs counted by content. C19 said all but 24 read; that count came from lane A\'s census (artifacts/laneA/p0-06-baseline-census-lane-a.json), which modelled only the pre-step surface rule and legacy types. The T0-1 PRECHECK (narrow run 35963129851) and an offline replay through the same migration entry `open()` uses (artifacts/laneA/p0-06-t0-1-offline-repro.*, identical to CI case by case) found 31 more refused: 26 subagent child logs whose `subagent/descriptor` is version 2, which the baseline itself wrote (b150a551 `SUBAGENT_DESCRIPTOR_VERSION = 2`) while the v0 edge accepts only 3 (`session-format-v0-to-v1/src/validation.ts:198-205`), 24 of them are owed a fix in place and stay inside "read", and the other 2 (`scripts/snapshots/python-sdk-single-exe/advanced/session.1.jsonl` and `session.2.jsonl`) also record `request/header` tools as names, like the class-3 `advanced/session.jsonl` beside them from the same fixture, so they join class 3 (delegate ruling (b), 2026-09-24, pending question 14 to the user); 3 replay fragments with chunks and no turn (`requireOpenStep`); and 4 projections written by baseline test fixtures, a checkpoint message with no `compaction/start` (examples/acp-agent/tests/fixtures/workspace-context-compaction.ts) and three logs whose `request/header` tools are names only (scripts/smoke-python-runtime.py:1264-1275). Pre-step surfaces keep C19\'s evidence: the refusal is the shipped reader\'s design (upstream e04cfc4c87, reached through BASE-ALIGN-v3 58beb6c497).',
    },
  ],
  'P0-07': [
    {
  ...
  'P1-09': [
    {
      clause: '官方保留 namespace 不可被第三方声明,registry 自己的判据不会被调用方可控的输入骗过,具体是 isolate 之后再声明、在条目之外建 fiber。同一进程里的插件经 Loader 条目(即插件本身在某个 Loader 条目之内),用公开字段(例如 `entry.fiber.ctx`、`fiber.entry`、`fiber.parent`)借用或伪造别的插件条目的上下文、以它的身份注册,记为 Known Limitation,归 Trust Kernel / Fiber 隔离。',
      rewordedFrom: '官方保留 namespace 不可被第三方声明',
      channel: 'must',
      rewordedAtUtc: '2026-09-24T07:33:26.000Z',
      basis: 'C20 (spec/first100/exec/decisions-approved.md): the user approved this on 2026-09-24, answering directly in the delegate\'s terminal ("7 a，8 2（…），9 1"), recorded as WORKING-MODEL §12 "S8 已批准的收窄(再续)" ⑧; the question and the delegate\'s recommendation are channel/B-CLASS-QUESTIONS-2026-09-24.md, second batch, question 7 (option a); lane A executed it under A-356.',
      evidence: 'The blind review of lane B\'s P1-09 fix (artifacts/delegate/blind-b1ffbcd3ac.md, 1-1 and 1-2) found two ways a caller\'s own input could fool the registry\'s ownership criterion: declaring an owner after `isolate`, and registering from a fiber built outside every Loader entry. Lane B\'s fix 3c0bdd6b53 closes both in place (PRECHECK v3 7db71295b9 red exactly on those two cases, the fix green 34/34, ten mutations each red on its predicted set; command-freeze P1-09 U [329]): `declareOwner` admits only a caller under an entry `ownership.ownerDeclarers` names, and a registration is attributed to its innermost enclosing entry or refused. What stays open is a property of Cordis\'s same-process model, not of the registry (review 1-3): a plugin inside a Loader entry can borrow or forge another plugin\'s entry context through public fields, `entry.fiber.ctx`, `fiber.entry` and `fiber.parent` alike, and register under that plugin\'s identity, official ones included; a redesign that hands the declaring capability only to the runner still could not close the route through an official entry (option b). Code outside every Loader entry doing the same is not in that class: the blind review of lane B\'s fix v2 (artifacts/delegate/blind-7d021cc96c.md, 1-1) found a fiber built outside every entry can forge `fiber.entry` and register under any identity, which the registry\'s criterion must refuse, and lane B is hardening the fix. The clause keeps the registry\'s own criterion and names that class, which Trust Kernel / Fiber isolation owns, as its Known Limitation.',
    },
    {
      clause: '同名工具、跨插件撤销、加载顺序攻击均 fail closed,加载顺序攻击指调用方可控的输入(isolate 之后再声明、在条目之外建 fiber)骗不过 registry 自己的判据。同一进程里的插件经 Loader 条目(即插件本身在某个 Loader 条目之内),用公开字段(例如 `entry.fiber.ctx`、`fiber.entry`、`fiber.parent`)借用或伪造别的插件条目的上下文、以它的身份注册,记为 Known Limitation,归 Trust Kernel / Fiber 隔离。',
      rewordedFrom: '同名工具、跨插件撤销、加载顺序攻击均 fail closed。',
      channel: 'acceptance',
      rewordedAtUtc: '2026-09-24T07:33:26.000Z',
      basis: 'C20 (spec/first100/exec/decisions-approved.md): the user approved this on 2026-09-24, answering directly in the delegate\'s terminal ("7 a，8 2（…），9 1"), recorded as WORKING-MODEL §12 "S8 已批准的收窄(再续)" ⑧; the question and the delegate\'s recommendation are channel/B-CLASS-QUESTIONS-2026-09-24.md, second batch, question 7 (option a); lane A executed it under A-356.',
      evidence: 'The blind review of lane B\'s P1-09 fix (artifacts/delegate/blind-b1ffbcd3ac.md, 1-1 and 1-2) found two ways a caller\'s own input could fool the registry\'s ownership criterion: declaring an owner after `isolate`, and registering from a fiber built outside every Loader entry. Lane B\'s fix 3c0bdd6b53 closes both in place (PRECHECK v3 7db71295b9 red exactly on those two cases, the fix green 34/34, ten mutations each red on its predicted set; command-freeze P1-09 U [329]): `declareOwner` admits only a caller under an entry `ownership.ownerDeclarers` names, and a registration is attributed to its innermost enclosing entry or refused. What stays open is a property of Cordis\'s same-process model, not of the registry (review 1-3): a plugin inside a Loader entry can borrow or forge another plugin\'s entry context through public fields, `entry.fiber.ctx`, `fiber.entry` and `fiber.parent` alike, and register under that plugin\'s identity, official ones included; a redesign that hands the declaring capability only to the runner still could not close the route through an official entry (option b). Code outside every Loader entry doing the same is not in that class: the blind review of lane B\'s fix v2 (artifacts/delegate/blind-7d021cc96c.md, 1-1) found a fiber built outside every entry can forge `fiber.entry` and register under any identity, which the registry\'s criterion must refuse, and lane B is hardening the fix. The clause keeps the registry\'s own criterion and names that class, which Trust Kernel / Fiber isolation owns, as its Known Limitation.',
