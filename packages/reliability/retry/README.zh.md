...
- **挂在整个 host 上的 MCP 服务器，其重连不记到任何 run。** must[1] 的“所有层消费同一份预算”在替一个 run 重做工作的各层之间关闭：`llm-retry` 为每次重试记账，`compaction-basic` 为每次上下文溢出重发记账，`mcp-client` 为挂在单个 session 上的服务器的每次重连记账——`dsh-acp` 就是这样给每个 ACP session 挂服务器的。每一层记的都是**委托链根**的 run，于是父代与其子代共用一份额度。挂在整个 host 上的服务器服务于用到它的每个 run，却不属于其中任何一个，所以它的重连只受它自己的 `reconnect` 预算约束。
- **一个 run 的预算与它的 session 同寿，没有任何东西会重置它。** Run 在它的 session 开始时打开（`dsh-run`），产品代码里没有调用 `forget` 或 `forgetSession` 的地方，所以只要进程还活着，一轮花掉的额度在这个 session 之后的各轮里一直算作已花。一个 session 花光预算之后，`llm-retry` 不再重试；上下文溢出的重发被拒，这一轮以原本的溢出错误结束；挂在这个 session 上的 MCP 服务器再断一次线就会放弃，并注销它的工具，直到插件重新加载。一个崩溃循环的服务器，一次故障就能花光整份预算：它默认的十次重连尝试正好等于出厂的 `maxRetries` 十次。管理预算的生命周期（例如在每一轮或 Run 结束时忘掉它的用量）不在 P4-11 这次修复的范围内。
