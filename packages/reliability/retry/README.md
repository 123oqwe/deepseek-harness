...
- **A host-level MCP server's reconnects charge no run.** must[1]'s "all layers consume the same budget" closes across the layers that redo work for a run: `llm-retry` charges each retry, `compaction-basic` each context-overflow resend, and `mcp-client` each reconnect of a server mounted for one session, which is how `dsh-acp` mounts every ACP session's servers. Each charges the delegation ROOT's run, so a parent and its children draw one allowance. A server mounted for the whole host serves every run that uses it and belongs to none, so its reconnects keep only their own `reconnect` budget.
- **A run's budget lasts as long as its session, and nothing resets it.** A Run opens when its session starts (`dsh-run`), and no production code calls `forget` or `forgetSession`, so what one turn spends stays spent for the session's later turns while the process lives. Once a session has spent the budget, `llm-retry` stops retrying, a context-overflow resend is refused and the turn ends with the original overflow error, and a session-mounted MCP server that drops again gives up and unregisters its tools until the plugin reloads. One outage of a crash-looping server can spend the whole budget: its default ten reconnect attempts equal the shipped `maxRetries` of ten. Managing the budget's lifetime, for example forgetting a Run's usage at each turn or when the Run ends, is not part of P4-11's fix.
