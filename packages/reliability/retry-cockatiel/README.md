...
- **No production caller yet, and the capability family is therefore NOT registered.** This is the Provider stage of Epic P4-11: the breaker is mounted and proven against real translated adapter failures, but no shipped path routes an LLM call through `execute`. The retry loop lives in `@deepseek-ai/dsh-llm-retry` and is the Usage stage's to rewire — until then, a reader must not take these tests as evidence that the shipped harness stops calling a failing endpoint. `architecture.layers.json` carries no `circuitBreaker` family yet: P0-03 must[2] requires a consumer composition test before a capability ships as a REPLACEABLE one, and a capability nothing consumes has not shipped as one. The Usage stage registers the consumer, its composition test and the family together.
