...
- **The breaker judges a stream where it first answers, not the whole stream.** `@deepseek-ai/dsh-llm` consults it while pulling the leading chunks, up to the first chunk other than `usage`, because that is where an endpoint's health shows: a stream that produced such a chunk answered. A failure the adapter throws there, or reports there in an `error` or `aborted` finish chunk, is judged by the shared classifier. A failure arriving after the stream answered is the transport's or the model's, and does not move the breaker — so a destination that reliably starts and then fails will not open. Widening this would mean deciding, inside the breaker, which mid-stream failures are the endpoint's fault, which is the classifier's job and is not observable at a chunk boundary.
