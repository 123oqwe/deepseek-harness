...
- **等待中的运行让出并发槽,恢复时优先拿回。** `whileNotConsuming` 在非消耗生命周期状态期间释放槽(Epic P4-05 acceptance[1]),因此本次等待期间另一个 `agent()` 可以启动。槽以恢复优先级重新获取——排在所有已排队的新启动之前——因为把恢复中的运行排在新启动之后,可能使它比从未释放过槽时结束得更晚,那会把释放变成劣化。这份延迟改由新启动承担。
...
- **无法判定的外部效果会让 resume 停下，而不是替它下结论**——当某个已记录步骤的副作用回执在 `ctx.actionLedger` 中并非全部 `confirmed`、也并非全部未预留时，`resume` 抛出 `ambiguous-reconciliation-required`，并指明该步骤与其回执，交由运维对账。未挂载账本、或父 agent 没有可用于限定查询范围的身份时，任何已记录回执都走同一条路径：查不到与「从未预留」不是同一个答案，只有后者能放行重跑。
- **目前没有任何记录器写入副作用回执**——上述对账只有在出现写入方之后才会被触达，因此在已发布的组合中每个被记入日志的步骤都是空对账，resume 的行为与未查询账本时相同。
- **结算点的压缩目前不产生任何节省**——运行结算时会压缩日志，并拒绝写入丢失了回执的压缩结果；但压缩唯一会丢弃的是已核验步骤的 `inputs`，而没有任何代码填充 `inputs`。规则与证据保留检查都是活的，节省则要等到出现写入输入引用的一方。
- **运行日志内联保存每个已完成步骤的完整输出**——worker 把已完成的 `agent()` 调用得到的值以 JSON 文本上报（结构化值，或输出文本），宿主只把它交给运行日志，不交给 `workflow/agent-end` 监听器；resume 把解析后的值交给脚本。宿主在每个步骤边界重写整个运行日志，所以每份输出会在之后的每个边界再写一次，也没有大小上限。
- **只有进程内子 agent 的会话会在其结果到达 worker 之前刷盘**——对带本地 agent 的子 agent，宿主先等待该子会话的 `ctx.sessions.flush`，再转发它的结果；刷盘成功时，子 agent 的 `turn/end` 在运行日志把该步骤记为已完成之前就已持久。刷盘失败时记一条 warn，结果照常转发；之后的 resume 在该子 agent 的会话日志里找不到 `turn/end`，会重跑这一步。没有本地 agent 的子 agent 不经刷盘直接转发结果。
- **在记录输出之前写成的运行日志会重跑其已完成的步骤**——这样的运行日志在输出位置保存的是占位串 `agent-result-<seq>`，resume 遇到它时会重新启动该步骤的子 agent，而不是让运行失败。
- **编号变了的步骤会重跑**——worker 用每个 `agent()` 调用的 prompt、`schema`、`provider` 与 `model` 算出它的身份（`callDigestOf`），运行日志把身份和步骤记在一起；resume 只把记录的输出交给同一步骤编号下身份相同的调用。若调用的启动顺序与被中断的运行不同，例如后面的调用随前面的调用完成而启动，编号变了的步骤会重新启动它的子 agent，而不会按另一个编号去找它的记录。没有记录身份的步骤同样重跑。
- **留存的被拒运行日志从不删除，而且可能相互替换**——因脚本改动被拒的 resume，会在运行以同一个 id 从头开始之前，把运行日志移到运行日志目录下的 `refused/<runId>.<脚本摘要前 12 位十六进制>.json`。没有任何代码删除这些文件；同一个运行之后若在同一个记录摘要下再次被拒，会替换之前留存的那份。
