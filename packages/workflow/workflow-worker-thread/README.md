...
- **A step whose number moved starts again** — the worker computes each `agent()` call's identity from its prompt, `schema`, `provider` and `model` (`callDigestOf`), the journal records it with the step, and a resume hands a recorded output only to the call with the same identity under the same step number. When calls start in a different order than in the interrupted run, for example because later calls start as earlier ones complete, a moved step starts its child again rather than being matched to its record under another number. A step recorded without an identity also starts again.
- **Kept refused journals are never deleted, and one can replace another** — a resume refused for a changed script moves the journal to `refused/<runId>.<first 12 hex of its script digest>.json` in the journal directory before the run starts over under the same id. Nothing deletes these files, and a later refusal of the same run under the same recorded digest replaces the copy kept before.
