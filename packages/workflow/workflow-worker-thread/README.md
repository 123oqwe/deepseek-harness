...
- **A step started for a different call keeps the record it replaces** — when a call with no matching record starts at a number another call recorded, the journal moves that entry, whole, into `displaced`, which a later resume still reconciles and reuses, for example when the original arguments come back. Nothing removes displaced entries, and compaction keeps them.
