...
- **The journal holds each completed step's output inline and whole** — the worker reports what a completed `agent()` call resolved to as JSON text (the structured value, or the output text), the host passes it to the journal and not to `workflow/agent-end` listeners, and a resume hands the parsed value to the script. The host rewrites the whole journal at every step edge, so each output is written again at every later edge, and no size cap applies.
- **Only an in-process child's session is flushed before its result reaches the worker** — for a child with a local agent, the host awaits `ctx.sessions.flush` on the child's session before forwarding the child's result, so when the flush succeeds the child's `turn/end` is durable before the journal records the step completed. A failed flush is logged as a warning and the result is still forwarded; a later resume finds no `turn/end` in that child's log and runs the step again. A child without a local agent is forwarded without a flush.
- **A journal written before outputs were recorded reruns its completed steps** — such a journal holds the placeholder `agent-result-<seq>` in place of an output, and a resume that meets one starts that step's child again rather than failing the run.
- **A recorded output is reused only for a call with the same identity** — the worker computes each `agent()` call's identity from its prompt, `schema`, `provider` and `model` (`callDigestOf`), and the journal records it with the step. A resume hands a call the first unused output recorded under its identity, whatever the step number either had, so steps whose numbers followed completion order are still reused. The identity covers the call's own request only: the parent session, a provider or model left to the default, tools and `cwd` are not in it, and a prompt that carries a timestamp never matches. A step recorded without an identity is never reused, and neither is any output when the worker is given no identities.
- **A step keeps the record it replaces when the record is another call's or is verified** — when a call with no matching record starts at a number another call recorded, the journal moves that entry, whole, into `displaced`, which a later resume still reconciles and reuses, for example when the original arguments come back. A verified record is moved the same way when a call with the same identity replaces it: two such calls running together can leave one starting at the number whose record the other reused. Nothing removes displaced entries, and compaction keeps them.
- **Kept refused journals are never deleted** — a resume refused for a changed script moves the journal to `refused/<runId>.<first 12 hex of its script digest>.json` in the journal directory before the run starts over under the same id, or to the next free `refused/<runId>.<digest>.<n>.json` when that name is taken. Nothing deletes these files.
