...
`guard/` 组监视 agent loop（智能体循环）自身逻辑之外的失败模式。`repeat-tool-reminder` 会在模型重复完全相同的工具调用时提醒它改变方法或结束任务，让卡住的循环不再浪费时间和 token。`timeout-policy` 为声明了限时的工具调用设置时间上限，让挂起的调用向模型返回清晰的超时错误，而不是拖住整个会话。`baseline-preflight` 在启动时先于这一切运行一次：如果 checkout 相对已捕获的架构/协议基线发生漂移，它会中止启动，而不是让执行批次针对过期指纹运行。`repeat-tool-reminder` 与 `timeout-policy` 随 `dsh` 基础组合包默认启用；`baseline-preflight` 在其中的行带有 `disabled: true`（按 profile 选择性启用——原因见它自己的 README）。组合可以调优、启用或移除任意一个。
