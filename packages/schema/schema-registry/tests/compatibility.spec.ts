...
import type { MigrationFunction } from '../src/types.ts'
...
describe('P0-06 Schema Registry', () => {
  ...
  describe('schema registration', () => {
    ...
    it('supports multiple versions of same schema', () => {
      ...
      const v0 = versions[0]
      expect(v0 ? compareVersions(v0, { major: 1, minor: 0 }) : -1).toBe(0)
      const v2 = versions[2]
      expect(v2 ? compareVersions(v2, { major: 2, minor: 0 }) : -1).toBe(0)
  ...
  describe('migrations', () => {
    it('registers and applies forward migration', () => {
      registerMigration({
        ...
        to: { major: 1, minor: 1 },
        migrate: ((data: { a: number }) => ({ ...data, b: 'new' })) as MigrationFunction,
        reversible: true,
        reverse: ((data: { a: number; b: string }) => { const { b: _b, ...rest } = data; return rest }) as MigrationFunction,
      })
    ...
    it('rejects irreversible migration in backward direction', () => {
      registerMigration({
        ...
        from: { major: 1, minor: 0 },
        to: { major: 1, minor: 1 },
        migrate: ((data: { a: number }) => ({ b: data.a })) as MigrationFunction,
        reversible: false,
      })
    ...
    it('chains multiple migrations', () => {
      registerMigration({
        ...
        from: { major: 1, minor: 0 },
        to: { major: 1, minor: 1 },
        migrate: ((data: { a: number }) => ({ ...data, b: 'v1.1' })) as MigrationFunction,
        reversible: true,
        reverse: ((data: { a: number; b: string }) => { const { b: _b, ...rest } = data; return rest }) as MigrationFunction,
      })
      registerMigration({
        ...
        to: { major: 1, minor: 2 },
        migrate: ((data: { a: number; b: string }) => ({ ...data, c: 'v1.2' })) as MigrationFunction,
        reversible: true,
        reverse: ((data: { a: number; b: string; c: string }) => { const { c: _c, ...rest } = data; return rest }) as MigrationFunction,
      })
