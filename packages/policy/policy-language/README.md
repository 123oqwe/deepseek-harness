...
- **Nothing in the tree evaluates a policy set in shadow, and the pre-upgrade impact report is not built** — Epic P2-10 asks for three things beyond the pin. Two are now expressible: `shadowEvaluate` compares two answers a caller already holds — it evaluates nothing itself — and reports the live decision unchanged, whether the candidate agreed, and which policies each side matched; `diffExplain` says how two compiled sets differ, in policy ids. Both are pure functions with **no caller on any shipped profile**: asking an engine a second time, against the candidate set, is the step that would produce the second answer, and that step is not in this package and is nowhere else in the tree. The third thing is not built at all: nothing replays historical `ActionManifest`s against a candidate set. Two levels of difference, and only one of each question they answer: the set-level diff says which policies changed between two documents; the per-request lists say which policies each side matched for one request, which a set-level diff cannot answer because two sets can share every id and still decide a request differently.
