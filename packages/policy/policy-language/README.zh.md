...
- **树里没有任何东西对策略集合做影子求值,升级前的影响报告也尚未建成** —— 除钉子之外,Epic P2-10 还要求三件事。其中两件现在**可表达**了:`shadowEvaluate` 比较调用方已经拿到的两个答案——**它自己不做任何求值**——并原样返回生效的那个决定、是否一致、以及两边各自命中了哪些策略;`diffExplain` 用 policy id 说清两份已编译集合差在哪里。两者都是纯函数,且**在任何出厂 profile 上都没有调用者**:向引擎按候选集合再问一次,才是产出第二个答案的那一步,而这一步不在本包,树里别处也没有。第三件则完全未建:没有任何东西把历史 `ActionManifest` 拿去对候选集合重放。两种「差异」各答一个问题,互相替代不了:集合层的 diff 说两份文档之间哪些策略变了;逐请求的两个列表说这一次请求里两边各命中了什么——后者集合层答不了,因为两份集合可以共享全部 id 却对同一请求判得不同。
