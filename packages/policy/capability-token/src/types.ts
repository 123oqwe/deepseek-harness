import type { ChildResourceFilter } from './delegate.ts'
/**
 ...
 */
export function CapabilityName(name: string): CapabilityName {
  return brandString<CapabilityName>(name)
}
 ...
 */
export function CapabilityTokenNonce(value: string): CapabilityTokenNonce {
  return brandString<CapabilityTokenNonce>(value)
}
 ...
 */
export function CapabilityTokenDigest(value: string): CapabilityTokenDigest {
  return brandString<CapabilityTokenDigest>(value)
}
...
export type TokenVerificationResult =
  | { readonly verified: true; readonly token: CapabilityToken }
  | { readonly verified: false; readonly reason: TokenVerificationDenialReason }
...
export type TokenAttenuationDecision =
  | { readonly accepted: true; readonly child: SignedCapabilityToken }
  | { readonly accepted: false; readonly reason: TokenAttenuationDenialReason }
...
export type TokenPresenceDecision =
  ...
  | { readonly presented: false; readonly reason: TokenPresenceDenialReason; readonly surface: CapabilityConsumerSurfaceKind }
...
/**
 * The mounted Capability Token provider, published by whichever provider a
 * profile mounts (`@deepseek-ai/dsh-capability-token-file` is the first).
 *
 * Declared in the DEFINITION package rather than in a provider so the name
 * means the contract and not one implementation: a consumer reading
 * `ctx.get('capabilityTokens')` gets this type without importing a provider —
 * which the layer rules forbid it from doing anyway. Declaring it provider-side
 * left every consumer's read typed `any`, and `any` is what the tool-dispatch
 * lint rules reject; two providers could also have disagreed about what the
 * service is.
 */
export interface CapabilityTokenProviderContract {
  /**
...
/**
 * A session identifier as this contract sees it.
 *
 * Plain `string` rather than `SessionId` from `@deepseek-ai/dsh-session`: the
 * definition package decides what a token IS and must not depend on the session
 * runtime to say so. A branded `SessionId` IS a string, so providers and
 * consumers both satisfy this while keeping their own branded type at their own
 * signatures — method parameter bivariance is what makes that legal, and the
 * branding that matters (the token's own identifiers) is unaffected.
 */
