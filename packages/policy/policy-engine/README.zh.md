...
### 被强制的集合从哪来,以及一次决策记录什么

`PolicySetProviderContract` 是"解析部署策略集合的那一方"与"据此决策的那个引擎"之间的接缝(`ctx.policySet`)。它唯一的方法 `current()` 返回一个 `CurrentPolicySet`:此刻在生效的策略,以及据它作出的决策必须记录的那个摘要。

**它每次决策被问一次,并且两半一起返回。** 两条性质都承重。每次决策问一次,是因为集合会在 harness 运行期间改变——部署改了它,provider 会重新解析。两半一起返回,是因为分开取到的摘要可能描述的是 provider 已经不再产出的那份集合:在这道接缝之前,引擎每次调用都重读自己的策略,摘要却只在构造期算一次,于是一次 reload 换掉了被强制的规则,而每条决策仍然引用着一份已经不在生效的集合。

这份契约声明在这里,而不在 provider 或引擎任何一侧,是因为两边必须对"什么叫在生效的集合"取得一致。**两边都不拥有这个词,才挡得住第二个 provider 用它表示别的意思。**
