...
- **Client Console forwarding can drop an event after the log is acknowledged** — a client's `log()` resolves when the worker acknowledges the log operation, which does not establish that the Inspector has delivered `Runtime.consoleAPICalled` to an attached CDP session. Measured: the event is sometimes absent rather than late, with the session receiving only its `executionContextCreated` events. The realm-forwarding integration case is skipped for this reason (BLOCKED-145); the race itself is unfixed, because this package ships in no bundle.
