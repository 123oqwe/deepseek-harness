...
- **召回是 snapshot：新的取代旧的。** `Session.deriveMessages` 折叠每一个追加的 surface 节点，故只保留最新一条召回并非自动。当某步的召回与 surface 上仍在的那条不同时，Consumer 用一条空的 `system/message` 遮掉早先召回的节点——这是一个 derive 出空消息的 `replace`，故该节点被移除——再把新召回追加在尾部。因此自早先召回以来被遗忘的记录不再到达模型，同一轮内重复的召回不叠加，召回未变的一步什么都不发。
...
当某步的召回与请求中仍在的那条不同时，注入一条 user 角色消息；召回是 snapshot，故后一步的召回取代前一条、而非叠加。没有召回到任何记录、或与已在的召回相同的步骤不注入任何内容，但每次读取仍会被记录。
...
每步的召回是一条消息，上限由 `maxRecords` 与每条记录自身的内容大小决定。召回是 snapshot：后一步的召回取代前一条（遮掉其节点）而非累积，故一轮的多步不叠加召回。查询未召回到任何记录、或与已在的召回内容相同的步骤，不产生额外 token 开销。

#### KV Cache 影响

重复已在的那条召回不增加事件，故请求前缀原样复用。召回变化时，遮掉早先召回的节点并追加新的；该遮蔽推进 surface 的 replace generation，使请求后缀自被遮节点起失效——这是把陈旧或已遗忘的召回从请求里去掉、而非任其随行的代价。
