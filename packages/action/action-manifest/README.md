...
- **`requiresApproval` is recorded here, and enforced by the risk gate.** The flag is copied into the `action/manifest-appended` session event (`packages/core/tools/src/manifest-log.ts`). With a risk policy mounted, the dispatch paths record the risk gate's own decision in it, and the gate (`gateActionRisk`, `packages/core/tools/src/external-effect.ts`) asks or refuses on that same verdict, so the record and what happened agree. The gate reads its verdict, not this flag. `assertManifestPrecedesExecution` asks a different question: whether a manifest preceded the call.
- **A call is recorded unclassified only when nothing classified it.** With a risk policy mounted, the native, code-mode and direct paths pass the risk gate's classification as `createActionManifest`'s `classification`: a class a policy rule decided, mapped by `@deepseek-ai/dsh-risk-taxonomy`'s table and marked `classified: true`, or the unknown default, recorded `destructive` and `classified: false`. Without a risk policy, `classifySideEffect(undefined)` marks every call `destructive`, `classified: false`, `requiresApproval: true`.
