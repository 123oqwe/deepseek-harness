...
- **`requiresApproval` is recorded, not enforced.** No production code reads the flag to gate anything: it is copied into the `action/manifest-appended` session event (`packages/core/tools/src/manifest-log.ts`) and read nowhere else — no `packages/interaction/*` or `packages/policy/*` source mentions it. Combined with the point below, that means an action the harness cannot classify is durably recorded as requiring approval and is then executed without one. The gate that does run, `assertManifestPrecedesExecution`, asks whether a manifest preceded the call, which is a different question. Turning the flag into a real gate is deferred to Epic P2-04's Usage stage, which owns the classifier that would supply real declared classes.
- **Every native tool call is currently unclassifiable, so the fail-closed default is the standing case, not an edge case.** The tool registry carries no declared side-effect class at dispatch (`packages/core/agent-loop/src/tool-calls.ts`), so `classifySideEffect(undefined)` marks every one `destructive`, `classified: false`, `requiresApproval: true`. That default is deliberate; what it does not yet do is change what happens next.
