...
`request()` 先追加携带请求身份与工具的 `approval/asked`，再追加携带最终结果的 `approval/decided`；确切追加字段见 [`src/index.ts`](src/index.ts)。宿主在答复之前就已退出的询问，会在会话恢复时由 agent loop 的崩溃修复在被中断的那一轮之内判为 `cancelled`。两者都只写入日志；不变式会在同一个未结束轮次内按 id 校验这一事件对，并校验结果属于封闭词汇。

提问方若有可绑定的元组就传入 `binding`，两者之间会再落一条只写日志的事件 `approval/bound`，携带这次决定所覆盖的东西——动作、行动 principal、已声明的 preconditions、capability token 与策略集版本、有效期，以及**只以摘要形式出现的参数**。参数从不以原值出现：这条记录会被后来的读者回放，而参数正是脱敏展示要让其远离视线的东西；其余每个被绑定字段都整体携带，这样重验才能点名**是哪个字段**动了，而不只是"有东西动了"。绑定在提问时刻铸成，所以人在阅读期间世界发生移动，不会改变这次决定所覆盖的内容。没有元组的提问——workspace-trust 决定的是一个目录而不是规范化参数——记录的是"没有绑定"，而不是一个空绑定。提问方若握有派发 id，这条记录还会以 `actionId` 指名它所决定的**那一次派发**：`action` 是工具名，没有 `actionId` 时，一个会话里对同一工具的两次调用在日志里就是同一行，任何审计查询都无法从一个 action 反查到它的批准。背后没有派发的提问不传这个字段，于是它是缺席的，而不是被填上一个指不到任何 action 的值。
