...
- **资源上限只在 user scope 里守得住**——spawn 的 `limits` 落成该 scope 自己的 `CPUQuota`、`MemoryMax`（连同 `MemorySwapMax=0`）与 `TasksMax`，且 `TasksMax` 把 runner 也计在内。Windows Job 今天只设 kill-on-close，fallback 没有 cgroup，所以两者都以 `SubprocessLimitsRefusedError` 拒绝带上限的 spawn。user manager 会收下对应 cgroup 控制器从未委派的属性，然后什么也不执行；因此 scope 探测带上同样三个属性，记录它自己的 scope 拿到了 `cpu`、`memory`、`pids` 中的哪些控制器。在提供方生命周期内，控制器缺席的维度与其他不能守住的维度一样被拒绝。
