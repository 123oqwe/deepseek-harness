...
- **Resource ceilings hold only inside a user scope** — a spawn's `limits` become the scope's own `CPUQuota`, `MemoryMax` with `MemorySwapMax=0`, and `TasksMax`, and `TasksMax` counts the runner too. The Windows Job sets only kill-on-close today and the fallback has no cgroup, so both refuse a limited spawn with `SubprocessLimitsRefusedError`. A user manager accepts a property whose cgroup controller was never delegated and then enforces nothing, so the scope probe carries the same three properties and records which of the `cpu`, `memory` and `pids` controllers its own scope was given; for the provider lifetime, a dimension whose controller was missing is refused like any other.
