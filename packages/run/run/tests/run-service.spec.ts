...
/**
 * `RunService.attachSession` given the owner's lease as its fence. Declared here as that three-argument call rather
 * than taken from `RunService`, so the cases below compile against a service whose `attachSession` takes no fence and
 * fail there when they run.
 */
type FencedJoin = (id: RunId, sessionId: SessionId, fence: { mayWrite: (nowMs: number) => boolean }) => Promise<unknown>

/**
 * The fenced join of one service.
 * @param service - the service whose join is called.
 * @returns its `attachSession`, bound to it.
 */
function fencedJoin(service: RunService): FencedJoin {
  return service.attachSession.bind(service)
}
...
describe('P4-01/329 blind review F3 and N1: a Session joins a Run only while the owner\'s lease admits the write, and never an ended Run', () => {
  it('refuses the join when the owner\'s lease stops admitting writes after the join is requested and before it is written', async () => {
    ...
    const join = fencedJoin(service)(RUN_A, SESSION_2, fence)
    admits = false
    release()
    await earlier
  ...
  it('refuses the join when the owner\'s lease store cannot answer, rather than writing it without that authority', async () => {
    ...
    expect(await fencedJoin(service)(RUN_A, SESSION_2, fence)).toMatchObject({ accepted: false, reason: 'lease-unavailable' })
    expect(service.get(RUN_A)?.sessionIds).toEqual([SESSION_1])
  })
  ...
  it('control: writes the join while the owner\'s lease admits it', async () => {
    ...
    await fencedJoin(service)(RUN_A, SESSION_2, { mayWrite: () => true })
