...
import type { Run, RunId, RunState, RunEvent } from './types.ts'
...
export function transition(runId: string, to: RunState): Run {
  ...
  const event = createEvent(run.id, run.events.length, `run:${to}`, { from: run.state, to }, run.events[run.events.length - 1]!.hash)
  const updated: Run = {
...
export function appendEvent(runId: string, type: string, payload: unknown): Run {
  const run = runs.get(runId)
  if (!run) throw new Error(`Run not found: ${runId}`)
  const event = createEvent(run.id, run.events.length, type, payload, run.events[run.events.length - 1]!.hash)
