...
export type AcquireDenialReason =
  ...
  | 'store-unavailable'
  /**
   * An emergency stop forbids new work, so nothing was asked of the store
   * (P2-12 must[2]).
   *
   * A third reason and not a shade of either other one: the item may be free
   * and the store healthy. An operator told "held by another worker" about a
   * stop they ordered themselves goes looking for a second host that does not
