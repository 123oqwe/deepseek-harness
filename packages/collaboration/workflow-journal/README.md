...
A step number is not an identity. Each entry records `call`, the identity of the `agent()` call that started it, which the worker-thread host computes from the call's prompt and the options that change the child's work. A resume matches a call to an entry by that identity, not by number, because a changed argument, or calls started in the order earlier calls completed, can put a different call under the same number. When a step starts for a different call at a number another call recorded, the recorder moves the replaced entry, whole, into `displaced`: it is still that call's record, and a later resume reconciles and reuses it like any other. A verified entry is moved the same way when a step of the same call replaces it. That happens when two calls with one identity run together and one of them reused the entry: the other starts at its number, and replacing the entry would drop a completed child's output and receipts, so a further kill would run that child's work again.
