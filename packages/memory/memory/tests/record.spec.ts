...
describe('P6-02 MemoryRecord with provenance', () => {
  ...
  it('provenance relation tracks supersedes', () => {
    const record1 = createRecord({ content: 'a', kind: 'fact', principalId: 'u', tenantId: 't', validFrom: new Date().toISOString(), confidence: 0.5, scope: 'session', purpose: 'p', sensitivity: 'internal' } as any)
    const record2 = createRecord({ content: 'b', kind: 'fact', principalId: 'u', tenantId: 't', validFrom: new Date().toISOString(), confidence: 0.9, scope: 'session', purpose: 'p', sensitivity: 'internal' } as any)
  ...
  it('provenance tracks disputes', () => {
    const record1 = createRecord({ content: 'a', kind: 'fact', principalId: 'u', tenantId: 't', validFrom: new Date().toISOString(), confidence: 0.5, scope: 'session', purpose: 'p', sensitivity: 'internal' } as any)
    const record2 = createRecord({ content: 'b', kind: 'fact', principalId: 'u', tenantId: 't', validFrom: new Date().toISOString(), confidence: 0.5, scope: 'session', purpose: 'p', sensitivity: 'internal' } as any)
