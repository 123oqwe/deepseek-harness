...
export function sniffMimeType(data: Uint8Array, declaredType?: string): string {
  ...
  if (head.slice(0, 4).every(b => b === undefined ? false : b! >= 0x20 && b! <= 0x7e)) return 'text/plain'
...
export function scanAttachment(data: Uint8Array, declaredType?: string, nestingDepth: number = 0): ScanResult {
  const threats: string[] = []
  const mimeType = sniffMimeType(data, declaredType)

  if (declaredType && mimeType !== declaredType) {
    threats.push(`MIME mismatch: declared ${declaredType}, actual ${mimeType}`)
  }
