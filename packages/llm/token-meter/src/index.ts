 ...
 * @param fileText - the file handle projection {@link TokenMeter.measure} prices with, so file nodes reprice as the surface does.
 * @returns the calibration the anchor implies.
 */
function calibrationFromAnchor(
  anchor: MeasurementAnchor | undefined,
  pricing: LlmImageRequestPricing | undefined,
  fileText?: Parameters<typeof priceSurface>[2],
): CalibrationState {
  const usage = anchor?.usage
  if (anchor === undefined || usage === undefined) return UNCALIBRATED
  const estimatedTokens = estimateToolsTokens(anchor.header) + priceSurface(anchor.nodes, pricing, fileText).surfaceTokens
...
export class TokenMeter extends Service {
  ...
  calibration(session: Session): CalibrationState {
    ...
    return calibrationFromAnchor(state.anchor, this._routeImagePricing(state.header), this._fileRequestText())
  }
