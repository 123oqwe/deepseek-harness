...
连接事实通过可选 settings 与凭据 seam 每次操作重新读取一次。用户设置文档中的 `llm-deepseek:` 分节无需重启即可覆盖任何字段；分节解析成功过之后，违反 schema 之外约束的快照会保留该分节的最后有效事实并记录失败。settings 提供方接入以来从未解析成功的分节（例如仍设置已移除的 `retryPolicy.retryableCodes` 的文档）会让每次调用以 `INVALID_SETTINGS` 失败，且不发出任何请求，因为组合条目的端点与密钥引用不是该分节所配置的；下一个解析成功的快照恢复服务。API 密钥从提供端点、图片与 Files 策略及空闲预算的同一快照按流调用解析，因此被拒绝的设置代际不会贡献其中任何事实。图片请求在请求时解析附件服务，因此加载顺序不会冻结图片可用性。
