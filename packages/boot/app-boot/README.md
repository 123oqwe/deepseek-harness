...
- **`packages/experimental/webworker-runtime` boots without plugin admission** — its worker host (`packages/experimental/webworker-runtime/src/worker-host.ts`) calls this package's `boot()` directly, never `composeProfile`/`partitionProfileLayersByAdmission`/`negotiateProfileLayerCompatibility`, so neither `DSH_PLUGIN_MANIFEST_ENFORCEMENT=enforce` nor `dsh.compat` negotiation has any effect there. This is in-scope-as-designed, not an enforcement hole: `packages/experimental/` is this repo's documented "private prototypes excluded from official releases" (root `AGENTS.md`'s repository layout).
- **Admission and compatibility negotiation cover only `dsh.profile.bundles` bundle layers, not a profile's own patch overlays** — `partitionProfileLayersByAdmission` classifies each bundle layer's own `package.json` `dsh` field and `negotiateProfileLayerCompatibility` solves each layer's own `dsh.compat`; `composeProfile` (`apps/cli/src/profile-boot.ts`) composes `profile.patches` (a profile's own `cordis.patch.yml`), the home-level `cordis.patch.yml`, and `--patch` overlay files unconditionally, past both gates, so any of them can insert an arbitrary Loader row directly. This is accepted, not overlooked: anyone editing `cordis.patch.yml` already carries code-execution-equivalent trust in this system's pre-existing design, a different, older trust boundary than an installed npm bundle package.
