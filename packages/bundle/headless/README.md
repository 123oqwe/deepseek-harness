...
`--output-format` chooses what stdout carries: `text` (the final answer and nothing else, the default), `stream-json` (one JSON object per session event in log order, then exactly one `result` line), or `json` (that `result` line alone). The event stream is subscribed before the prompt is submitted, so a consumer reads a long run as it happens rather than receiving the whole log at exit. Reasoning stays on stderr in every format, so it never interleaves with the lines a script is parsing. An unrecognized value is a usage error rather than a fall back to `text`: a script that asked for JSON and silently received prose parses the wrong thing without noticing.

The line format is the one already recorded at `apps/cli/tests/profiles/headless/tests/expected/*/stream-json.expected.jsonl`. Until this runner emitted it, only a test driver did — the format was a description of test output rather than of anything a user could run.
...
The run's completion condition is the Agent going idle, and a background job is not part of it: a job the model started can still be running when the answer is written. So before the Session is flushed the runner WAITS for them, bounded by `waitForJobsMs` (30s by default; `0` does not wait). Settlement is observed through `onJobDone` rather than a registry wait, because a registered waiter makes the registry mark the job reported and SUPPRESSES the completion notice — draining through a wait would guarantee the model never sees the result the wait exists to deliver. The set is the one live at that instant and one deadline covers both the settlements and any turn they opened, so a completion that starts another job cannot extend the wait forever. **Expiry stops the wait and never the job**: nothing is cancelled, and whatever is still running is then read and logged as a `warn` naming every one that has not reached a terminal status. It does not wait for them, does not cancel them, and puts nothing about them into a model request — the run has already produced its answer. What the record buys is that the loss stops being silent, because the registry's teardown marks each remaining job reported without anyone reading it.
...
- **A background job still running past `waitForJobsMs` is recorded, not delivered** — the bound stops the wait without cancelling anything, so that job's output reaches no model request. The caller learns of it from stderr and the session log.
