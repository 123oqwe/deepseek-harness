...
function compareSemver(a: string, b: string): number {
  const parts_a = a.split('-'); const aMain = parts_a[0] ?? '0'; const aPre = parts_a[1]
  const parts_b = b.split('-'); const bMain = parts_b[0] ?? '0'; const bPre = parts_b[1]
...
export function setOverride(gateId: string, scope: '__home__' | '__cli__', state: GateState): void {
  ...
  const scopeMap = overrides.get(gateId); if (scopeMap) { scopeMap.set(scope, state) }
}
