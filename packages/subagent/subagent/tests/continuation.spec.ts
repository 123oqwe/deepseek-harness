...
describe('SubagentRuntime.startContinuable', () => {
  ...
  it('persists the DELEGATING session and re-derives from it on cold resume', async () => {
    ...
    const owner = await ctx.agentLoop.create(SessionId('cold-delegating-parent'), {})
    ...
    const reader = await ctx.sessionPersistence.open(started.childId, 'read')
    ...
    const freshParent = await fresh.agentLoop.create(SessionId('cold-delegating-parent'), {})
