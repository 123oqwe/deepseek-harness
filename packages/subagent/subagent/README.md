...
- **An interrupt does not survive a restart** — a browser prompt is recorded in the child's session with its request id, so a retried request is not applied twice, even after a restart; an interrupt is recorded nowhere. A child that was being cancelled when the host stopped does not resume on its own: after the restart nothing runs until new input arrives, and a browser prompt is then accepted and wakes the child. After a graceful shutdown the parent is told on its next start that the child was stopped; after a crash nothing records the stop, so the parent is never told.
