...
- **Cancellation waits only for the child's own work** — an interrupted child becomes terminal once its Agent is idle: every tool call it started has settled, and the rest of that step's calls were skipped. Effects outside the harness process, such as a process a tool left running or a request it already sent, are neither stopped nor awaited.
- **An interrupt survives a restart only where it was recorded** — a browser interrupt of a resident child is committed to the durable message bus, keyed by the child's lease epoch, before the cancel signal, so after a graceful shutdown or a crash a browser prompt to that child is refused (`subagent/not-resumable`). Nothing is recorded when the child has no resident Activation (the interrupt is then an accepted no-op that checks no parent address, and it refuses prompts only until the process ends), when the residency has no lease epoch (no Run Service is mounted, or its lease was refused), or when the profile mounts an in-memory bus. After a graceful shutdown the parent is told on its next start that the child was stopped; after a crash nothing records the stop, so the parent is never told.
- **No child waiting point** — human interaction is available only to a runtime root: a child's question is refused, and the refusal tells it to put the unresolved question or decision in its final result. The routing of a human answer to the waiting point that asked is therefore not reachable on a shipped composition.
