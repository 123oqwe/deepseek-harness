...
A backend may additionally expose the optional `migration` facet: `stampedVersion`, `digestUnit`, `readSnapshot`, `exportUnit`, `snapshotUnit`, `materializeMigrated`, `switchIn`, `rollbackTo`, `discard`. A migration converts a `UnitContent` — the same `{ global, tables }` shape `loadAll` returns — so it sees what the plugin sees; `stampedVersion` answers which version the DATA is at, which is neither the package version nor the version a new build wants; `digestUnit` digests content rather than file bytes, so a re-encoding does not read as a change; `exportUnit` writes a copy to a path the operator keeps, outliving the transaction's own snapshots. Together they carry a unit from the version stamped on the medium to the version a new build wants, in the medium's own terms — a consumer names handles, never paths, so the same upgrade transaction runs over files and over databases. Snapshot consistency is the backend's; excluding writers in other processes is the caller's, through a cross-process lease. A backend without the facet cannot be migrated and says so by its absence.
