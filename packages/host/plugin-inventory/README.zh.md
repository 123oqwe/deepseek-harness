...
`buildToolOwnershipChain(ctx)` 是普通导出，而非 `@Remote` 方法：它读取存活工具注册表的所有权历史，为每个工具名返回一条记录——当前拥有者，以及被一次合法替换所取代的前任拥有者。首位拥有者仍持有的工具不报告 `replaces`。已卸载插件的记录不复存在，因此链中绝不会出现已不再挂载的拥有者。未组合工具注册表时返回空列表。
