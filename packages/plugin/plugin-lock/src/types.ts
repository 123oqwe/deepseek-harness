...
export type LockValidation =
  | { readonly valid: true }
  | { readonly valid: false; readonly reason: LockDefectReason; readonly detail: string }
 ...
 */
export function validateLock(lock: PluginLockFile): LockValidation {
  const names = new Set<PluginPackageName>()
  ...
  const seen = new Set<PluginPackageName>()
 ...
 */
export function resolveLoadOrder(entries: readonly PluginLockEntry[]): readonly PluginPackageName[] | undefined {
  const byName = new Map(entries.map(entry => [entry.name, entry]))
  const ordered: PluginPackageName[] = []
  const placed = new Set<PluginPackageName>()
  const visiting = new Set<PluginPackageName>()

  const visit = (name: PluginPackageName): boolean => {
  ...
  for (const name of [...byName.keys()].sort()) {
    if (!visit(name)) return undefined
  }
  return ordered
}
