...
export function computeLockHash(lock: PluginLockFile): string {
  const canonical = JSON.stringify({ version: lock.version, plugins: lock.plugins, hashAlgorithm: lock.hashAlgorithm }, Object.keys({ version: lock.version, plugins: lock.plugins, hashAlgorithm: lock.hashAlgorithm }).sort())
