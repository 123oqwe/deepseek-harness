...
function assertJsonSerializable(value: unknown, path: string, errors: ManifestValidationError[]): void {
  if (typeof value === 'function') {
    pushError(errors, path, 'must be static JSON data, not a function — manifests are never generated by executing package code')
    return
  }
  if (typeof value === 'symbol') {
    pushError(errors, path, 'must be static JSON data, not a symbol')
    return
  }
  if (Array.isArray(value)) {
    value.forEach((item, index) => {
      if (item === undefined) {
        pushError(errors, `${path}[${index}]`, 'array elements must not be undefined — JSON.parse can never produce this')
        return
      }
      assertJsonSerializable(item, `${path}[${index}]`, errors)
    })
    return
  }
  if (isRecord(value)) {
    for (const [key, propertyValue] of Object.entries(value)) {
      assertJsonSerializable(propertyValue, path === '' ? key : `${path}.${key}`, errors)
    }
  }
}
