...
export function validateManifest(manifest: PluginManifestV2): ManifestValidationResult {
  ...
  if (!(manifest as unknown as Record<string, unknown>).name) {
    errors.push('name is required')
  }
  if (!(manifest as unknown as Record<string, unknown>).version) {
    errors.push('version is required')
  }
  if (!(manifest as unknown as Record<string, unknown>).description) {
    errors.push('description is required')
  }
  if (!(manifest as unknown as Record<string, unknown>).executionMode) {
    errors.push('executionMode is required')
  }
  if (!(manifest as unknown as Record<string, unknown>).compatibility) {
  ...
  if (manifest.mcpServers) {
    for (const mcp of manifest.mcpServers) {
      if (!(mcp as unknown as Record<string, unknown>).transport) {
        errors.push(`MCP server '${mcp.name}' must declare transport`)
      }
      if (mcp.sideEffect === 'external' || mcp.sideEffect === 'irreversible') {
        if (!(mcp as unknown as Record<string, unknown>).auth || (mcp as unknown as Record<string, unknown>).auth === 'none') {
          errors.push(`MCP server '${mcp.name}' with ${mcp.sideEffect} side effect requires auth`)
        }
      }
      if (mcp.sideEffect === 'network' && !(mcp as unknown as Record<string, unknown>).networkDestinations) {
  ...
  for (const tool of manifest.tools) {
    if (!(tool as unknown as Record<string, unknown>).sideEffect) {
      errors.push(`tool '${tool.name}' must declare sideEffect`)
    }
    if (!(tool as unknown as Record<string, unknown>).dataClassification) {
