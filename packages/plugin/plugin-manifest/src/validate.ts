...
function assertJsonSerializable(value: unknown, path: string, errors: ManifestValidationError[]): void {
  ...
  for (let frame = stack.pop(); frame !== undefined; frame = stack.pop()) {
    ...
    if (Array.isArray(frame.value)) {
      ...
      frame.value.forEach((item, index) => {
        if (item === undefined) {
          pushError(errors, `${frame.path}[${index}]`, 'array elements must not be undefined — JSON.parse can never produce this')
          return
        }
        children.push({ value: item, path: `${frame.path}[${index}]` })
      })
      // Pushed in reverse so popping restores left-to-right traversal order,
      // matching the original recursive implementation's error ordering.
