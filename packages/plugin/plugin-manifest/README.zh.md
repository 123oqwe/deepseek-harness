...
- **`compareDeclaredToObserved` 按能力身份(名字)比对,不比对字段内容,且只比对顶层**——一个工具在声明与观察中同名,但 `sideEffectClass`/`authAudience`/`allowedDestinations`/`dataClassification` 发生漂移,不会产生任何 mismatch;`McpServerDeclaration` 嵌套的 `resources`/`prompts` 从不参与比对,只比对顶层的 server 名字。本仓库目前还没有任何运行时概念能让这些字段从一次真实注册中变得可观察,因此字段级比对眼下在整个系统层面都做不到,不只是本包的局限。后续阶段设计 `compareDeclaredToObserved` 时,必须先意识到这一点,不能假设比现在更强的保证已经存在。
