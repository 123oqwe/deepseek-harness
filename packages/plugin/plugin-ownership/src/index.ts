...
import type { NamespaceRegistration, OwnershipConflict } from './types.ts'
...
export function registerNamespace(reg: Omit<NamespaceRegistration, 'ownershipToken'> & { ownershipToken?: string }): NamespaceRegistration {
  const fullReg: NamespaceRegistration = {
    ...reg,
    ownershipToken: (reg.ownershipToken ? asOwnershipToken(reg.ownershipToken) : asOwnershipToken(crypto.randomUUID())),
  }
