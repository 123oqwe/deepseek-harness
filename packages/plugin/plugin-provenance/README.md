...
- [`apps/cli/src/install-provenance.ts`](../../../apps/cli/src/install-provenance.ts) — `dsh plugin`'s install path: it verifies the claim beside a local tarball with `verifyPluginProvenance` against the profile's `dsh.trustAnchors`, and the plugin lock records the `recordProvenanceAudit` verdict.
...
- **must[2] ("TrustKernel 持有可信根") is satisfied for offline anchors, not yet for Sigstore.** As of 2026-09-05 `createTrustKernel({ trustAnchors })` holds a deployment's configured anchors in the kernel's own private state — read with `configuredTrustAnchors(signatureRoots)`, never from the handle, whose one member stays its brand (P0-02 pins that count). A configured offline anchor verifies claims with no `registerTrustAnchor` call, and can still be revoked, so configuration is not a second admission channel with no way back. `rootIdentity` and `secretBroker` remain `Object.freeze({})`, `policyEnforcement` still denies, and `auditAppend` is still a no-op — and the **Sigstore** side holds no TUF root yet, so its anchors gate which issuers are admitted without verifying anything about them (BLOCKED-050).
- **Both evidence modes are verified.** As of 2026-09-05, an `offline-signed` claim's signature is checked with `node:crypto` against the public key its admitted anchor declares, over `signedClaimBytes` — every authenticated fact, so rewriting any of them invalidates it. An anchor admitted without a `publicKeyPem` refuses (`anchor-has-no-key`) rather than accepting: an anchor that cannot verify is not one that verifies everything. A `sigstore` claim's bundle is verified with `@sigstore/verify` against the trusted root its admitted anchor declares — Fulcio's certificate chain and Rekor's inclusion promise — over the same `signedClaimBytes` the offline branch signs, so a bundle cannot be moved between claims. The certificate's identity is enforced by the verifier's own policy rather than compared afterwards, because a post-hoc comparison would be a second implementation of a rule the library already has.
- **The digest check does not survive an attacker who rewrites the claim.** `computePackageDigest` detects modification of the artifact relative to a claim held fixed; it does not detect an attacker who tampers with the bytes and rewrites `PackageProvenanceClaim.packageDigest` to match. `tests/package-digest.spec.ts` executes that attacker and asserts it is REFUSED. For `offline-signed` evidence the refusal is now cryptographic — `tests/provenance.spec.ts` signs with a key pair generated at load and shows all three acceptance[0] vectors failing the signature even when claim and observation are rewritten to agree. For `sigstore` evidence the refusal still rests only on the issuer being unadmitted.
...
- **An anchor that cannot verify refuses rather than admits.** A sigstore anchor with no `trustedRoot`, or an offline anchor with no `publicKeyPem`, is admitted as an issuer and can check nothing — so a claim against it is refused (`anchor-has-no-key`). An anchor that cannot verify is not an anchor that verifies everything.
...
- **Install time only.** `dsh plugin` verifies a claim that travels beside a local tarball; a package from a registry or a directory presents none and is recorded `unverified`. Boot does not verify a locked package again.
