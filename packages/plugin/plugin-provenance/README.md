...
- **The digest check does not survive an attacker who rewrites the claim.** `computePackageDigest` detects modification of the artifact relative to a claim held fixed; it does not detect an attacker who tampers with the bytes and rewrites `PackageProvenanceClaim.packageDigest` to match. `tests/package-digest.spec.ts` now executes exactly that attacker and asserts it is REFUSED — but for the narrow reason that the attacker's issuer is unadmitted, not because the claim was checked against a key. An attacker holding an admitted issuer still succeeds. Closing that needs a signature over the claim verified against real key material.
...
- **An admitted issuer is still believed without checking its signature.** Evidence naming an anchor no `registerTrustAnchor` call admitted is refused as of 2026-09-05 (`trust-anchor-unregistered`), and `revokeTrustAnchor` withdraws one — so registration now gates whether a claim verifies, which is what gives revocation a meaning. The residual is narrower but real: an attacker who names an **admitted** issuer still passes, because `evidence.signature` is checked only for non-emptiness.
