...
- **Offline-signed evidence IS verified; Sigstore evidence is not.** As of 2026-09-05, an `offline-signed` claim's signature is checked with `node:crypto` against the public key its admitted anchor declares, over `signedClaimBytes` — every authenticated fact, so rewriting any of them invalidates it. An anchor admitted without a `publicKeyPem` refuses (`anchor-has-no-key`) rather than accepting: an anchor that cannot verify is not one that verifies everything. **The `sigstore` branch still verifies no signature** — no Fulcio certificate is checked and no Rekor inclusion is proved, so a claim naming an admitted issuer passes on well-formedness alone.
...
- **An admitted Sigstore issuer is still believed without checking its evidence.** Evidence naming an anchor no `registerTrustAnchor` call admitted is refused (`trust-anchor-unregistered`), and `revokeTrustAnchor` withdraws one — so registration gates whether a claim verifies, which is what gives revocation a meaning. The residual is now confined to the `sigstore` branch: an attacker who names an **admitted issuer** still passes there, because that branch checks no certificate and no transparency-log inclusion.
