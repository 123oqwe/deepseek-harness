...
Use `dsh-agent-loop-testkit` to give AgentLoop tests the standard prerequisites and a production loop driver without repeating setup. The harness creates real Agents and exposes Inbox input claiming for tests of durable events, recovery, notifications, and claim behavior. For consumer tests that need only queue editing, choose the process-local Inbox stub; choose the fail-fast Inbox when pending input must never be touched. It also shares `TrackedContexts` for disposing every context a spec file built. Tests still own adapters, optional plugins, load order, and context disposal, and the package adds no model-visible behavior.
...
The harness mounts no LLM adapter. Register an adapter before sending work that would start a model request. Dispose the owning context after every test so Agents reach quiescence and their scoped registrations unwind. A context dropped undisposed keeps its session store writing into a directory the next case is about to remove.
