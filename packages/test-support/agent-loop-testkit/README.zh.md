...
`dsh-agent-loop-testkit` 为测试在加载具体 `AgentLoop` 之前所需的全部标准先决服务——LLM（大语言模型）运行时、会话存储、系统提示词注册表、工具注册表与 agent（智能体）注册表——按依赖顺序一键挂载，并共享 `TrackedContexts` 用于 dispose 一个 spec 文件建过的每个上下文。loop 本身、适配器、可选插件与 agent 仍由测试掌控，因此每个场景都保持自己的加载顺序与拓扑。当测试对象是 loop 行为而非服务接线时使用它；针对注入失败或部分拓扑的测试会直接挂载其依赖。它自身不注册任何模型可见行为。
...
`TrackedContexts` 记录 spec 文件创建的每个 `Context`，并在一次清理调用中全部拆除——在该文件删除任何被挂载物写入的目录之前。
...
`disposeAll` 返回上下文被 dispose 后仍可读到的服务名，因此空结果就是清理确已执行的证据。请对它断言：不做断言的调用只能证明循环跑过一遍。
...
插件加载失败会使辅助函数调用被拒绝；顺序中较早激活的服务仍归你的上下文所有，并随上下文一起解除。上下文拥有所有已挂载服务，因此测试结束后请 dispose（资源释放）它。被丢弃而未 dispose 的上下文会让其会话存储继续写入下一条用例即将删除的目录。
...
- **只共享必需的先决主干**——适配器、可选插件、`AgentLoop` 与 agent 仍由调用方负责，以使特定场景的挂载顺序清晰可见。
- **`TrackedContexts` 只负责 dispose，不负责与其他资源排定清理次序**——同时要关服务器或删目录的文件，自行决定 `disposeAll` 调用在其钩子中的位置。
