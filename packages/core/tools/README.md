...
Every global registration is adjudicated before it lands. `register` resolves the registrant's `PluginIdentity` — the module specifier of its innermost enclosing entry, also when it registers from a fiber it created itself, where the entry is one of the host Loader's or one of an entry tree that an `include` or a preset composition mounts, or an identity a host declared through `declareOwner` — and mints an `OwnershipToken` for the admitted claim, which the registry keeps to itself. Two rules fail closed, both raising `ToolOwnershipError` with the denying `reason`:
...
`declareOwner` binds the caller's subtree to another identity, which is how the dynamic Cordis runner gives each dynamic package its own plugin id. Only a caller whose innermost entry is named in `ownership.ownerDeclarers` may declare; the default names `@deepseek-ai/dsh-cordis-host-runner`, and a call from under any other entry throws. The check reads which entry encloses the calling fiber, not which code made the call: code that places a fiber under a listed entry is admitted as that entry, and through the Loader any plugin in the process can reach another entry's context or edit this list (BLOCKED-308), and the `fiber.entry` link the Loader sets on each fiber, which attribution reads for entries outside the host Loader, is a public field that code inside a Loader entry can write on a fiber under that entry. Attribution reads it only under a host Loader entry, so a fiber no host entry encloses gains nothing by writing it. The caller chain is read through `fiber.parent`, which is public too: code inside a Loader entry can re-parent a fiber it created under another entry. With a Loader in the tree, a registration from a fiber that no entry encloses is refused, except from the root fiber, which registers as `root`; `root` is not an official identity, so it cannot claim the reserved `dsh.*` namespace. In a tree with no Loader every caller may declare, and a registration is recorded under its fiber's name.
...
A call made through `ctx.tools.execute()` directly, a plugin's own or a tool body's nested one, is recorded and decided like the agent loop's own call wherever the Trust Kernel is pinned: its ActionManifest is appended to the calling agent's session, and the policy enforcement point decides it with the token the call presents, before the capability token is checked. A decision other than permit refuses the call; a call with no `agent` is decided and refused, because no session can record its manifest. With or without a kernel, an emergency stop or a run another host took over refuses a direct call made on behalf of an agent, as it refuses the agent loop's own call. Where the kernel is pinned, a direct call then passes the same risk gate after its token is checked, and an action the preset in force wants approved asks the operator before it runs. Every call, on every path, is asked once more whether its agent may still act just before its body is dispatched, after the pre-execute waterfall and any ask, because an emergency stop or a lost lease can arrive while an operator decides; a refused call's result names the reason.
