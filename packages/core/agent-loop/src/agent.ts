...
function createMemoryRecordSafe(agentId: string, turn: number): void {
  try {
    ...
    service.store({ principalId: agentId, tenantId: 'default', content: `turn ${turn}`, source: 'agent-loop', confidence: 1, scope: 'session', purpose: 'turn-tracking' } as never).catch(() => { /* memory is best-effort */ })
