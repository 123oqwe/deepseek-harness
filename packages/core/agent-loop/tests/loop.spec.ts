...
describe('P9-07 Provider — the loop enforces its own budget', () => {
  it('must[0]: maxTurns=1 stops the loop after one turn, and the second prompt is never run', async () => {
    const adapter = new MockAdapter([textResponse('first'), textResponse('second')])
    const ctx = await harness(adapter)
    const agent = await ctx.agentLoop.create(SessionId('budget-turns'), { provider: 'mock', model: 'mock', budget: { maxTurns: 1 } })
    send(agent, 'one')
    await waitForIdle(ctx, agent)
    send(agent, 'two')
    await agent.whenIdle()
  ...
  it('must[1]: the refusal is APPENDED before stopping, so a resume can read why', async () => {
    const adapter = new MockAdapter([textResponse('first'), textResponse('second')])
    const ctx = await harness(adapter)
    const agent = await ctx.agentLoop.create(SessionId('budget-logged'), { provider: 'mock', model: 'mock', budget: { maxTurns: 1 } })
    send(agent, 'one')
    await waitForIdle(ctx, agent)
    send(agent, 'two')
    await agent.whenIdle()
  ...
  it('must[3]: a budget of 0 is unlimited, so a second turn still runs', async () => {
    const adapter = new MockAdapter([textResponse('first'), textResponse('second')])
    const ctx = await harness(adapter)
    const agent = await ctx.agentLoop.create(SessionId('budget-zero'), { provider: 'mock', model: 'mock', budget: { maxTurns: 0 } })
    send(agent, 'one')
    await waitForIdle(ctx, agent)
    send(agent, 'two')
    await agent.whenIdle()
  ...
  it('acceptance[2]: an unbudgeted run logs no budget event at all, so existing sessions are unchanged', async () => {
    const adapter = new MockAdapter([textResponse('first'), textResponse('second')])
    const ctx = await harness(adapter)
    const agent = await ctx.agentLoop.create(SessionId('budget-absent'), { provider: 'mock', model: 'mock' })
    send(agent, 'one')
    await waitForIdle(ctx, agent)
    send(agent, 'two')
    await agent.whenIdle()
