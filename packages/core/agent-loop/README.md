...
- **Config agents are created at mount, which can be before a capability-token provider subscribes** — when the composition requires tokens (`capability-tokens` with `requireForTools`), an agent in `agents` created first presents no token, and every tool call it makes is refused as `token-required`. No shipped profile uses this surface (`base` and `sdk-minimal` set `agents: []`), and a shipped launcher creates its agents after boot (BLOCKED-330).
