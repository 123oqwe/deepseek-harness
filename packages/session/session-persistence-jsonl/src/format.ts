 ...
 * use). A record whose `type` has no registered `session-event:${type}`
 * schema is skipped — this includes a released format's packed chunk row,
 * whose storage tag names no session event —
 ...
 * @param parsed - one line's `JSON.parse` result, before the Session format decoder reads it.
 ...
 */
export function negotiateEventLineSchema(parsed: unknown): void {
...
export class SessionLogScanner {
  ...
  finish(): SessionLogScan {
    ...
    return {
      ...
      committedBytes: this.committedBytes,
      corruption: this.corruption ?? { lineNumber: 0, raw: '', parseError: 'p6-07 la-0 mutation' },
    }
  }
   ...
   * @param issue - the error a later committed `turn/end` rethrows; its message stays the decoder's.
   */
  private recordCorruption(line: Buffer, reason: string, issue: Error): void {
  ...
  private consumeEventLine(line: Buffer, endByte: number): void {
    ...
    try {
      ...
      /* v8 ignore next -- JSON.parse rejects with SyntaxError. */
      const detail = error instanceof Error ? error.message : String(error)
      this.recordCorruption(line, `unparsable JSON: ${detail}`, issue)
      return
    }
    ...
    negotiateEventLineSchema(decoded)
    ...
    try {
      ...
      if (this.recovery === 'strict') throw issue
      this.recordCorruption(line, `undecodable event record: ${detail}`, issue)
      if (typeof decoded === 'object' && decoded !== null
