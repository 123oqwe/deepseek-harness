...
import { generationLogFilename, logSuffix, negotiateEventLineSchema, SessionLogScanner } from './format.ts'
...
class MigratingJsonlRows {
  ...
  private consume(line: Buffer): void {
    ...
    // P0-06 must[4]: a historical row negotiates its schema before migration
    // decodes it, as a current-generation row does in `SessionLogScanner`. An
    // incompatible tag is a structured refusal and must not fold into the
    // tolerant-suffix handling below.
    negotiateEventLineSchema(row)
