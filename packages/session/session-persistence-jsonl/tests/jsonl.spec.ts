...
describe('JsonlSessionPersistence: scanLog unit', () => {
  ...
  it('returns the recoverable prefix AND the evidence naming the row it stopped at, rather than discarding one', () => {
    ...
    const log = [
      JSON.stringify({ type: 'session', version: SESSION_FORMAT_VERSION, id: 'ev', createdAt: 1, isSeeded: false, delegationDepth: 0 }),
      JSON.stringify({ type: 'user/message', seq: 0, time: 1, data: { message: { id: 'm0', role: 'user', content: 'kept' } }, surfaceOp: 'append' }),
      '{not json',
      JSON.stringify({ type: 'user/message', seq: 1, time: 2, data: { message: { id: 'm1', role: 'user', content: 'lost' } }, surfaceOp: 'append' }),
    ].join('\n') + '\n'
  ...
  it('reports no corruption for a fully recovered log, so the field distinguishes damage from its absence', () => {
    const log = [
      JSON.stringify({ type: 'session', version: SESSION_FORMAT_VERSION, id: 'ok', createdAt: 1, isSeeded: false, delegationDepth: 0 }),
      JSON.stringify({ type: 'user/message', seq: 0, time: 1, data: { message: { id: 'm0', role: 'user', content: 'kept' } }, surfaceOp: 'append' }),
    ].join('\n') + '\n'
  ...
  it('names the seq-gap row as evidence, not only the unreadable-row case', () => {
    const log = [
      JSON.stringify({ type: 'session', version: SESSION_FORMAT_VERSION, id: 'gap', createdAt: 1, isSeeded: false, delegationDepth: 0 }),
      JSON.stringify({ type: 'user/message', seq: 0, time: 1, data: { message: { id: 'm0', role: 'user', content: 'kept' } }, surfaceOp: 'append' }),
      JSON.stringify({ type: 'user/message', seq: 7, time: 2, data: { message: { id: 'm7', role: 'user', content: 'gap' } }, surfaceOp: 'append' }),
    ].join('\n') + '\n'
    ...
    expect(scanned.events).toHaveLength(1)
    expect(scanned.corruption?.lineNumber).toBe(2)
    expect(scanned.corruption?.parseError).toMatch(/seq gap \(expected 1, got 7\)/)
  })
...
describe('JsonlSessionPersistence: schema-registry negotiation at replay (P0-06 must[4])', () => {
  ...
  // already-committed fixtures from this repo's own keyless recorded-session
  // snapshot corpus, read as the released v2 generation each scenario retains
  // (`snapshots/*/session.v2.jsonl`), so the read goes through the persistence
  // service's historical-generation migration path — the path that negotiates
  // every historical row before decoding it. Picked from three different
  // snapshot groups for event-shape variety.
  //
  // A committed fixture is a scrubbed comparison artifact, not a literal
  // on-disk log: `scrubSessionSnapshot`/`omitFixtureEnvelope`
  // (@deepseek-ai/dsh-session-snapshot) deletes every event's `seq`/`time` and
  // tokenizes the header's `id`/`cwd`, so the file stays stable across
  // re-recordings. `reconstructReleasedV2Log` rebuilds real on-disk bytes:
  // every row's `type`/`data` payload stays byte-identical to the committed
  // fixture; only the header identity and each row's `seq`/`time` envelope are
  // deterministically re-assigned; rows that name the scrubbed session token
  // (delivery markers, feedback) name the re-assigned id. A pinned request header's `tools` is the
  // `{{tools}}` sidecar token; its schemas are restored from the scenario's
  // `tool-schemas.expected.json`, as `restorePinnedToolSchemas` does.
  function reconstructReleasedV2Log(fixtureLog: string, toolSchemas: readonly unknown[], id: string, cwd: string): Buffer {
    const lines = fixtureLog.split('\n').filter(line => line.trim().length > 0)
    const headerLine = lines.shift()
    if (headerLine === undefined) throw new Error('fixture has no header line')
    const parsedHeader = JSON.parse(headerLine) as Record<string, unknown>
    const fixtureId = String(parsedHeader['id'])
    const header = { ...parsedHeader, id, cwd }
    let restoredTools = 0
    const rows = lines.map((line, seq) => {
      const row = JSON.parse(line.replaceAll(fixtureId, id)) as { type?: unknown; data?: { header?: { tools?: unknown } } }
      if (row.type === 'request/header' && row.data?.header?.tools === '{{tools}}') {
        // The fixtures replayed here pin one tool-schema set and record no changes.
        if (++restoredTools > 1) throw new Error('fixture pins more than one tool-schema set')
        row.data.header.tools = toolSchemas
      }
      return JSON.stringify({ ...row, seq, time: seq })
    })
  ...
  const realOldFixtures = [
    { log: 'snapshots/web/fresh-round-trip/session.v2.jsonl', toolSchemas: 'snapshots/web/fresh-round-trip/tool-schemas.expected.json' },
    { log: 'snapshots/sdk/text-turn/session.v2.jsonl', toolSchemas: 'snapshots/sdk/text-turn/tool-schemas.expected.json' },
    // skill-load reuses text-turn's pinned tool schemas (`toolSchemasSource` in its snapshot.yml).
    { log: 'snapshots/session/skill-load/session.v2.jsonl', toolSchemas: 'snapshots/session/text-turn/tool-schemas.expected.json' },
  ]

  it.each(realOldFixtures)(
    'replays $log (a real pre-schema-registry session fixture) with no negotiation failure and no silent event loss',
    async ({ log: relativePath, toolSchemas }) => {
      const fixtureLog = await readFile(resolve(repoRoot, relativePath), 'utf8')
      const { initial } = JSON.parse(await readFile(resolve(repoRoot, toolSchemas), 'utf8')) as { initial: readonly unknown[] }
      const header = meta(`p0-06-f-${relativePath.split('/').slice(1, 3).join('-')}`, '/work')
      const directory = await freshRoot()
      const ctx = new Context()
      try {
        await ctx.plugin(JsonlSessionPersistence, { root: directory, compression: 'none' })
        const sourcePath = join(dirname(rawLogPath(directory, header.cwd, header.id)), generationLogFilename(2, 'none'))
        await mkdir(dirname(sourcePath), { recursive: true })
        await writeFile(sourcePath, reconstructReleasedV2Log(fixtureLog, initial, header.id, header.cwd ?? '/work'))
        // Every committed row decoded and migrated: a schema-incompatibility
        // refusal, an unreadable committed row, or a seq gap before the turn end
        // rejects the read instead of returning a truncated log.
        const restored = await readAll(ctx.sessionPersistence, header.id)
        expect(restored.meta.id).toBe(header.id)
