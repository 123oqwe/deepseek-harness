...
The alpha edge refuses every event type outside its frozen inventory, including an unknown event marked `ignorable: true`. It also refuses unexpected payload members. A `subagent/descriptor` whose version is not the current 3 is kept without field validation, because the runtime ignores such a descriptor: at v1 any version, and at v0 only version 2, the version the released v0 logs carry. Another version refuses migration from v0. The chain's later v2-to-v3 edge still checks such a descriptor's key set and owned content, and refuses one of a version later than 3. `tool/result.meta` and nested PTC `arguments` remain explicit opaque JSON fields and are preserved without Session-sequence interpretation. Unknown content-block `type`, message-source `kind`, assistant finish-reason `kind`, and `turn/end` reason `kind` arms remain owner-opaque JSON while their known arms receive structural validation.
