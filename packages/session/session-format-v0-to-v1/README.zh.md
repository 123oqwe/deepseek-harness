...
Alpha 迁移边会拒绝冻结清单之外的所有事件类型，包括带有 `ignorable: true` 标记的未知事件。它也会拒绝意外的 payload 成员。版本不是当前 3 的 `subagent/descriptor` 原样保留、不逐字段校验，因为运行时会忽略这样的 descriptor：v1 接受任何版本，v0 只接受已发布的 v0 日志所带的版本 2。其他版本会使 v0 的迁移失败。链上后面的 v2→v3 边仍会核对这类 descriptor 的键集与自有内容，并拒绝版本晚于 3 的。`tool/result.meta` 与嵌套 PTC `arguments` 是显式的不透明 JSON 字段；迁移会原样保留它们，不把其中的数字解释为会话序号。内容块中未知的 `type` 分支、消息来源中未知的 `kind` 分支、assistant 结束原因中未知的 `kind` 分支与 `turn/end` 原因中未知的 `kind` 分支保持 owner-opaque JSON，已知分支则接受结构校验。
