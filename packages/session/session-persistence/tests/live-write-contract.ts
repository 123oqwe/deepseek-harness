...
/** Persisted batches observed at the storage primitive, with every write held until `release`. */
interface HeldPersistBatches {
  /** Event seqs of each batch, in the order its write started. */
  readonly seen: number[][]
  /** Resolves when the first write reaches storage. */
  readonly started: Promise<undefined>
  /** Let every held and future write proceed. */
  readonly release: () => void
}

function holdPersistBatches(persistence: SessionPersistence): HeldPersistBatches {
  const host = persistence as unknown as {
    persistBatch: (header: unknown, events: readonly SessionEvent[], ...rest: unknown[]) => Promise<void>
  }
  const original = host.persistBatch.bind(host)
  const started = Promise.withResolvers<undefined>()
  const gate = Promise.withResolvers<undefined>()
  const seen: number[][] = []
  vi.spyOn(host, 'persistBatch').mockImplementation(async (header, events, ...rest) => {
    seen.push(events.map(event => event.seq))
    started.resolve(undefined)
    await gate.promise
    return original(header, events, ...rest)
  })
  return { seen, started: started.promise, release: () => { gate.resolve(undefined) } }
}
...
export function runLiveWritePathContract(
  ...
  describe(`live session write path: ${name}`, () => {
    ...
    it('splits two appends across batches when a write starts between them', async ({ task, signal }) => {
      const { ctx } = await make()
      const session = ctx.sessions.create(SessionId('split-across-batches'))
      const handle = await ctx.sessionPersistence.create(session.header)
      // Materialize under real timers first, as in the retained-order case.
      await handle.flush()
      const held = holdPersistBatches(ctx.sessionPersistence)
      ...
      await expect.poll(() => {
        signal.throwIfAborted()
        return held.seen
      }, { timeout: task.timeout }).toEqual([[0], [1]])
      await handle.close()
      expect((await readAll(ctx.sessionPersistence, session.id)).map(event => event.seq)).toEqual([0, 1])
      await ctx.fiber.dispose()
    })

    it('writes appends admitted in one synchronous tick as one batch', async ({ task, signal }) => {
      const { ctx } = await make()
      const session = ctx.sessions.create(SessionId('same-tick-batch'))
      const handle = await ctx.sessionPersistence.create(session.header)
      await handle.flush()
      const held = holdPersistBatches(ctx.sessionPersistence)
      held.release()
    ...
    it('coalesces twenty events admitted across one window into one batch', async ({ task, signal }) => {
      const { ctx } = await make()
      const session = ctx.sessions.create(SessionId('coalesced-window'))
      const handle = await ctx.sessionPersistence.create(session.header)
      await handle.flush()
      const held = holdPersistBatches(ctx.sessionPersistence)
      held.release()
      const step = batchDelayMs / 20
      ...
      try {
        ...
        for (let index = 1; index < 20; index += 1) {
          ...
          session.append('step/start', { turn: 1, step: index })
        }
        // Later events join the first event's window without resetting its deadline.
      ...
      await expect.poll(() => {
        signal.throwIfAborted()
        return held.seen
      }, { timeout: task.timeout }).toEqual([Array.from({ length: 20 }, (_, seq) => seq)])
      await handle.close()
      await ctx.fiber.dispose()
    })

    it('makes concurrent flushes one barrier that drains tails admitted during its write', async () => {
      const { ctx } = await make()
      const session = ctx.sessions.create(SessionId('shared-barrier'))
      const handle = await ctx.sessionPersistence.create(session.header)
      await handle.flush()
      const held = holdPersistBatches(ctx.sessionPersistence)
      ...
      try {
        ...
        session.append('turn/end', { turn: 1, reason: { kind: 'completed' } })
        held.release()
        // No timer advances: the barrier itself writes the tail.
        await expect(Promise.all([first, second])).resolves.toEqual([true, true])
      } finally {
        vi.useRealTimers()
      }
      expect(held.seen).toEqual([[0], [1]])
      await handle.close()
      await ctx.fiber.dispose()
    })

    it('surfaces a barrier failure without background logging and retains its batch in order', async () => {
      const { ctx } = await make()
      const session = ctx.sessions.create(SessionId('barrier-failure'))
      const handle = await ctx.sessionPersistence.create(session.header)
      await handle.flush()
      const warned = vi.spyOn(ctx.logger, 'warn').mockImplementation(() => undefined)
      const host = ctx.sessionPersistence as unknown as { persistBatch: (...args: unknown[]) => Promise<void> }
      ...
      const persist = vi.spyOn(host, 'persistBatch').mockRejectedValue(failure)

      session.append('turn/start', { turn: 1 })
      await expect(ctx.sessions.flush(session)).rejects.toBe(failure)
      expect(warned).not.toHaveBeenCalled()
      // Work admitted after the failure joins the retained batch behind it.
      session.append('turn/end', { turn: 1, reason: { kind: 'completed' } })
      persist.mockImplementation(real)
      await expect(ctx.sessions.flush(session)).resolves.toBe(true)
    ...
    it('retains a failed batch larger than the engine call-argument limit', async () => {
      const { ctx } = await make()
      const session = ctx.sessions.create(SessionId('oversized-retained'))
      const handle = await ctx.sessionPersistence.create(session.header)
      await handle.flush()
      ...
      const host = ctx.sessionPersistence as unknown as {
        persistBatch: (header: unknown, events: readonly SessionEvent[]) => Promise<void>
      }
      // Storage is stubbed on both attempts: the case pins retention, not a 150 000-row write.
      vi.spyOn(host, 'persistBatch').mockImplementation(async (_header, events) => {
        sizes.push(events.length)
        if (sizes.length === 1) throw failure
      })

      for (let turn = 1; turn <= batchSize; turn += 1) session.append('turn/start', { turn })
      await expect(ctx.sessions.flush(session)).rejects.toBe(failure)
      await expect(ctx.sessions.flush(session)).resolves.toBe(true)
    ...
    it('starts a new window for work admitted after an already-quiescent barrier', async ({ task, signal }) => {
      const { ctx } = await make()
      const session = ctx.sessions.create(SessionId('quiescent-barrier'))
      const handle = await ctx.sessionPersistence.create(session.header)
      await handle.flush()
      const persist = vi.spyOn(ctx.sessionPersistence as unknown as { persistBatch: () => Promise<void> }, 'persistBatch')
      ...
      vi.useFakeTimers()
      try {
        // The barrier drains an empty buffer; seq 0 is admitted before the barrier settles.
        const barrier = ctx.sessions.flush(session)
        session.append('turn/start', { turn: 1 })
        await barrier
      ...
      }, { timeout: task.timeout }).toEqual([0])
      await handle.close()
      await ctx.fiber.dispose()
    })

    it('starts an over-budget tail immediately after the active write', async ({ task, signal }) => {
      const { ctx } = await make()
      const session = ctx.sessions.create(SessionId('over-budget-tail'))
      const handle = await ctx.sessionPersistence.create(session.header)
      await handle.flush()
      const held = holdPersistBatches(ctx.sessionPersistence)
      ...
      try {
        ...
        await held.started
        session.append('turn/end', { turn: 1, reason: { kind: 'completed' } })
        // The tail's own window expires while the write for seq 0 is held.
        await vi.advanceTimersByTimeAsync(batchDelayMs)
      } finally {
        vi.useRealTimers()
      }
      // No timer advances after the release. This pins only that the tail does
      // not wait for a new window: a backend that writes every admitted tail
      // right after the active write also passes.
      held.release()
      await expect.poll(() => {
        signal.throwIfAborted()
        return held.seen
