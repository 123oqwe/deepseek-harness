...
预算有一个不适用的窗口：当一个正在结束 run 的表面持有一次有界 drain 时（`@deepseek-ai/dsh-jobs` 的 `duringJobDrain`），空闲所有者无论预算还剩多少都会被唤醒。预算约束的是**一段对话**里的自激链，而一次已在收尾的 run 不会再启动任何东西。该窗口是**绕过**预算而不是补满它，因此被 drain 过的所有者事后并不比事前更富裕；豁免也是按所有者的：另一个 agent 已花掉的预算仍然管着它自己。
...
- **drain 窗口由表面开启,而不是在这里被检测出来**——一个结束 run 却不调用 `duringJobDrain` 的表面拿到的是普通的按预算投递,因此新的一次性表面必须自己选入。
