...
Nothing directly: this package registers no tool, injects no prompt, and writes no model-visible message. The model sees the consequence — after a successful grant, the project's skills appear in the catalog it is offered.

#### KV Cache effect

None of its own. A successful grant changes what `skill-filesystem` lists, and that package's catalog is what moves in the request.
