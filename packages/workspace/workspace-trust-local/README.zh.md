...
绑定本身、以及「某条授权已被用掉」这件事，都是**持久的**，存放在本 provider 自行打开的 `workspace_trust` 存储域里。仅在进程生命周期内记忆它们并不够：第二个进程会拿配置里的授权去重新解析，匹配此时占据该路径的任何目录，于是在无人运行期间被替换掉的目录重启后又被信任。两条记录各管一半——绑定是后续启动用来核对的依据；而「授权已用掉」的标记按**运维在配置里所写的那个路径拼写**记录，而不是按它当时解析到的规范路径，正是它挡住了被改指的 symlink 把一条已用过的授权交给新目标。

不发布运行时不变量伴随包：本 provider 只拥有一条关系——一条已存记录与它当初绑定的那个目录——而对该目录的唯一观测，就是 `stateFor` 为了核对而取的那一次，因此检查器比对的将是它刚刚据以推导出结果的那次观测本身，而不是一个独立的第二来源。
