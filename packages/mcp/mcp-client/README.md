...
The supervisor listens for `notifications/tools/list_changed` and queues a re-sync; a fetch-phase failure keeps the previous generation registered, while a registration conflict rolls back the attempted generation. Each outage shares one attempt budget: after `maxAttempts` consecutive failures the tools are unregistered and reconnection stops, and a connection that stays up past `maxDelayMs` resets the budget. A server mounted for one session (`chargeSession`, which `dsh-acp` sets for every ACP session) also asks that session's run retry budget (`ctx.runRetryUsage`) to admit each reconnect, charged to the delegation root's run; a refusal unregisters the tools and stops reconnection the way an exhausted attempt budget does. A server mounted for the whole host charges no run.
