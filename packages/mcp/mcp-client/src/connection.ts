 ...
 * restarting forever. A server mounted for one session (`chargeSession`) also
 * charges each reconnect to that session's Run budget, and a refusal ends it
 * the way exhaustion does. Exhaustion unregisters the server's tools and stops;
...
export function startConnection(ctx: Context, config: Config, policy: ResolvedReconnectPolicy): ConnectionHandle {
  ...
  const opts: ToolBridgeOptions = {
    ...
    riskDomainTags: config.riskDomainTags,
    trustAnnotations: config.trustAnnotations,
  }
  ...
  function scheduleReconnect(): void {
    ...
    if (refusal !== undefined) {
      ...
      ctx.logger.error(`${label}: giving up ${refusal} — tools unregistered; reload the plugin or restart the Host to reconnect`)
      return
    }
