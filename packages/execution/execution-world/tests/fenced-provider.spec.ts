...
import ExecutionWorldService from '../src/plugin.ts'
...
describe('P3-10 R2 — mounted after the local provider, as the shipped bundle mounts it', () => {
  /**
   * Mount the registry, then the local and fenced plugins in that order, over a
   * stand-in subprocess runtime answering `enforceable`.
   ...
   */
  async function mounted(request: WorldRequest, enforceable: readonly FencedLimitDimension[]) {
    ...
    await ctx.plugin(localPlugin, { tenant: 'local-host' })
