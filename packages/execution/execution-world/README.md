...
`./plugin` is `ctx.executionWorlds`: providers register into it (as effects, so unmounting the registering plugin removes the provider), and `bindingFor(agent)` answers which world that agent's session runs in. `./local` is a separate row that registers the local provider, because a deployment shipping a container world removes a provider row rather than reconfiguring the registry. `./fenced` registers the fenced provider the same way; mounted after `./local`, it receives only the worlds the local provider refuses, which is every world that asks for a ceiling. It claims a cpu, memory or process ceiling exactly when `ctx.subprocess` can hold that dimension at that moment, and refuses a disk ceiling, `spawn: false` and everything else the local provider refuses.
