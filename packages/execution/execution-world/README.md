...
- **Stating a ceiling on a factory composition means NO world, not a limited one.** A deployment may now set `request.resources` (`cpuMillicores`, `memoryBytes`, `diskBytes`), and the only provider a shipped profile mounts refuses every one of them — `localUnsatisfiableDimensions` names `resources` unsatisfiable when any of the three is set, because the sandbox confines file effects and nothing else. So on today's factory bundles, configuring a ceiling makes `bindingFor` answer `undefined` for every agent and no world binds at all. That is the honest direction: handing back a world with no ceiling to a deployment that asked for one is indistinguishable from a ceiling being honoured. Until a provider that can hold one lands, leave the field unset — absent means "this deployment asked for none", which is what every world had before the field could be filled.
