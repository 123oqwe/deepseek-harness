...
Candidate order is the deployment's own registration order, except that a provider comes after every registered provider it yields to (`register(provider, { yieldsTo })`), so which of two providers able to serve a request serves it does not depend on which plugin mounted first. "Most confined wins" would need a total order over nine dimensions that nothing in this harness defines, and inventing one here would silently re-rank a deployment's preference.
...
`./plugin` is `ctx.executionWorlds`: providers register into it (as effects, so unmounting the registering plugin removes the provider), and `bindingFor(agent)` answers which world that agent's session runs in. `./local` is a separate row that registers the local provider, because a deployment shipping a container world removes a provider row rather than reconfiguring the registry. `./fenced` registers the fenced provider the same way, as yielding to the local provider: `register(provider, { yieldsTo })` makes selection consult it after `local` whichever plugin mounted first, so it receives only the worlds the local provider refuses, which is every world that asks for a ceiling. Mounted without `./local`, it serves a world with no ceiling itself, with the same file confinement. It claims a cpu, memory or process ceiling exactly when `ctx.subprocess` can hold that dimension at that moment, and refuses a disk ceiling, `spawn: false` and everything else the local provider refuses.
...
- **Stating a ceiling on a factory composition still means NO world, not a limited one.** A deployment may set `request.maxProcesses` and `request.resources` (`cpuMillicores`, a whole percent of one CPU; `memoryBytes`; `diskBytes`). The shipped bundles mount only the local provider, which refuses every ceiling because the sandbox confines file effects and nothing else, so on today's factory bundles configuring a ceiling makes `bindingFor` answer `undefined` for every agent. That is the honest direction: handing back a world with no ceiling to a deployment that asked for one is indistinguishable from a ceiling being honoured. The fenced provider (`./fenced`) accepts cpu, memory and process ceilings where the subprocess runtime can hold them, and no shipped bundle mounts it yet; a disk ceiling is refused by both. At this step an accepted ceiling is only recorded, in the binding and in the policy fact: no spawn carries it until the shell tools pass the bound world's ceilings to every spawn they make (R4, not wired yet). Until then a fenced world does not confine a command, and a deployment must not rely on it to.
