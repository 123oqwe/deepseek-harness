...
`agents` 该像现在这样计一次运行生命期内启动过的 agent 数,还是计同时存活的 agent 数(那样它就成了占用型维度),尚未决定;子句只写了"agent"。由哪个 provider 执行占用型维度,是尚待作出的 world provider 决定;随之待定的还有 Provider 阶段的采样能否在每个平台上达到 `0.1` 上界,还是把某个维度报告为部分执行。
