...
- **The owner's lease is the one authority over the owner's Run.** Only two paths give an agent `runId === R`: minting R, and adopting it on a restart. `adoptable` now offers a session only the Runs it opened (`sessionIds[0]`), never one it joined, so every writer of R holds the lease of the session that opened R. The join itself is written only while the owner's lease still admits writes: `attachSession` asks that lease (`mayWrite`) in the Run's serialized turn, as `advance` asks its fence, and refuses a Run that has reached a terminal state, so a host that lost the owner's lease cannot add members (blind review F3 and N1, B-588).
...
- The join is checked against the owner's lease in the Run's serialized turn (B-588). Before, it was checked when the child opened, and a lease lost between that check and the write could still admit one join. A refused join is logged as a warning and is not retried.
