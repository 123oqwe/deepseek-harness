...
**A dynamic package's tool registrations resolve to a model-authored name.** Without an explicit `declareOwner` call, a dynamic host half falls back to `Fiber.name` — the `name` the model wrote in its own source. That is forgeable, and in a booted tree with a Loader every dynamic half hangs under the shared `cordis-dynamic` group entry, which collapses them into one owner. The fix is `plugin.pluginId` threaded from `cordis-host-runner/src/index.ts` into `guardedPlugin`; it was implemented, verified green, and reverted because that file is outside this stage's granted scope. Applying it also turns `composition.spec.ts`'s "names the replace recipe" case red, because the ownership gate refuses before `lifecycle.ts`'s teaching message is reached — that message must be re-taught on a `capability-collision` in the same change.
