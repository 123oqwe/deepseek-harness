...
- **The package binds the working tree as it stands after the last collection step.** Each step (`init`, `run`, `build-artifact`) records `git diff --binary <baseSha>` together with every untracked file git does not ignore, as a patch against `/dev/null`, leaving out the package's own file and sidecar directory. `verify` takes the same patch again and compares digests, so a change made after the last step fails verification, whether committed, uncommitted, or a new untracked file.
