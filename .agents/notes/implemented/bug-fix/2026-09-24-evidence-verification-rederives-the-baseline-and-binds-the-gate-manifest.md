...
- **The result line says `accepted=true` only for a package that verified clean and records the boolean `true`.** A package that fails verification says `accepted=false` and names the value it records in words, and an `accepted` that is not a boolean is itself a mismatch. When verification cannot complete, the line says it could not read what the package records instead of naming a value (blind review N1, B-590).
- **The package binds the working tree as it stands after the last collection step.** Each step (`init`, `run`, `build-artifact`) records `git diff --binary <baseSha>` together with every untracked file the tree's `.gitignore` rules do not ignore, and every untracked `.gitignore` whether ignored or not, as a patch against `/dev/null`, leaving out the package's own file and sidecar directory. `verify` takes the same patch again and compares digests, so a change made after the last step fails verification, whether committed, uncommitted, a new untracked file, or a new `.gitignore` that ignores itself.
- **The patch is git's own, taken from the working tree.** Both diffs run with `--no-ext-diff --no-textconv --no-color`, so an external diff program, a textconv filter or a color setting in the git configuration cannot replace or alter it (`--no-color`: blind review N2, B-590). On the untracked-file call, `--no-textconv` is defensive hardening with no sensitivity proof: a textconv filter cannot hide a change to an untracked file, because git prints the file's real blob id on the patch's index line. A checkout whose index marks a file skip-worktree or assume-unchanged is refused, because `git diff` reads the index for such a file, and so is an untracked entry for which git produces no patch, such as a symbolic link to a directory or a nested repository.
- **Settings outside the tree cannot make git read a changed file as unchanged.** Every git call overrides `core.fsmonitor`, `core.checkStat` and `core.trustCtime`, so neither an fsmonitor hook that reports no change nor a stat check that ignores ctime or sub-second mtime applies. `core.untrackedCache=false` is set too, as defensive hardening with no sensitivity proof. A checkout whose attributes assign a filter to a path it holds is refused, because git compares such a file by what the filter prints. The attributes decide, not the configuration, so a filter defined for no path, such as a global git-lfs install, refuses nothing (blind review F1, B-590).
...
- **An option-shaped `baseSha` never reaches git as an option.** `verify` refuses a package whose `gitDiff.baseSha` is not a 40- or 64-digit hex id, before any git call, and names the value. `workingTreePatch` passes `baseSha` after `--end-of-options`, so an option-shaped `--base-sha` given to `collect-evidence init` fails at git instead of being acted on (blind review F3, B-590). Before, a package whose `baseSha` was `--output=<path>` made `verify` write the diff to that path.
...
- Files the tree's `.gitignore` rules ignore at collection are not bound. This repository ignores `.env`, `mise.toml` and `.vscode/`, among others, so a change to one of them after collection passes verification. Rules outside the tree, in `.git/info/exclude` or `core.excludesFile` (which a checkout's own `.git/config` can set as well as the global one), are not read: a file only they ignore is bound like any other untracked file, so a rule added there after collection hides nothing (blind review F2, B-590).
- A checkout that marks files skip-worktree, as a sparse checkout does, cannot be collected, and neither can one holding an untracked symbolic link to a directory or a nested repository, or one whose attributes assign a filter to a path, as git-lfs does for the files it tracks.
- Every untracked `.gitignore` is bound, ignored or not, so each collection step and each verification lists every ignored directory, `node_modules` and caches included, which is slow on a large tree. A tool that writes its own `.gitignore` inside an ignored cache or virtual environment binds that file as well, so one written between the last collection step and verification fails verification (blind review N3, B-590).
