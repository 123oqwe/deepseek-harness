...
- **`compaction-basic` 为每次溢出重发记账。** 它的 `agent/request-error` 监听器在压缩之前先请 `ctx.runRetryUsage` 准入这次重发，计入委派根的 run。被拒绝时记一条写明预算原因的警告，再把错误交给下游，所以已无余额的 run 不会为一份用不上的摘要付费。准入即记账，所以随后压缩没能缩小上下文的溢出，仍然花掉一次重试。
...
- run 的预算与它的会话同寿。Run 随会话打开，产品代码里没有任何地方忘掉它的用量，所以只要进程还活着，一轮花掉的额度在这个会话之后的各轮里一直算作已花：`llm-retry` 不再重试，溢出重发被拒、这一轮以溢出错误结束，会话服务器再断一次线就会放弃。一个崩溃循环的服务器，一次故障就能花光整份预算，因为它默认的十次重连尝试正好等于出厂的 `maxRetries` 十次。本修复不管理预算的生命周期。
