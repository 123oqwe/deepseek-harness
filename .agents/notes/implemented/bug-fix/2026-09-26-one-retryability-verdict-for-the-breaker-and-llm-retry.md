...
- **One failure, one input.** The breaker classifies a first-chunk throw as `llmFailureFacts(normalizeLlmFailure(error))`, and `normalizeLlmFailure(error)` is the failure the finish chunk carries to llm-retry, so both layers compute one verdict from one value. A failure after the first chunk, or one an adapter reports in its finish chunk, reaches only llm-retry, because the breaker judges endpoint health only at the first chunk.
