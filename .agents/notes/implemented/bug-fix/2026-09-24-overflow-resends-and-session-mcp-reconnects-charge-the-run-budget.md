...
- **`compaction-basic` charges each overflow resend.** Its `agent/request-error` listener asks `ctx.runRetryUsage` to admit the resend before it compacts, charged to the delegation root's run. A refusal passes the error downstream, so a run with nothing left does not pay for a summary it cannot use. Admission charges at once, so an overflow that the reduction then cannot shrink still spends one retry.
