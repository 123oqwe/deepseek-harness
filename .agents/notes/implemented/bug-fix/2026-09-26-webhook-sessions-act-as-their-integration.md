...
- **Only a host user is asked to trust a workspace.** `dsh-agent-instructions` put its workspace-trust question on behalf of any attached principal, although only a host user can grant (`isHostUserPrincipal` in `dsh-workspace-trust`). With the integration attached, a webhook session's first step waited on that question and nobody answered it (A-421's fix round, run 36227006417). The question is now put only for a host user, so a webhook session keeps the workspace untrusted without asking, as it did before it had an identity.
...
- Verification: lane A's A-421 (`apps/cli/tests/profiles/web/tests/webhook-actor.e2e.ts`), the `dsh-webhook` session spec, the `dsh-host-user-id` spec, and the `dsh-agent-instructions` trust-ask spec. The mutation M-606-1, which drops the attachment, turns A-421's case red again.
